...
**New:**
- [`src/SharpInference.Diffusion/Quality/QualityPreset.cs`](../src/SharpInference.Diffusion/Quality/QualityPreset.cs) — 5-value enum (Maximum/High/Medium/Low/Custom)
- [`src/SharpInference.Diffusion/Quality/QualityProfile.cs`](../src/SharpInference.Diffusion/Quality/QualityProfile.cs) — record with BackboneDType / TextEncoderDType / VaeDType + Validate()
- [`src/SharpInference.Diffusion/Quality/QualityProfileApplier.cs`](../src/SharpInference.Diffusion/Quality/QualityProfileApplier.cs) — generic dict-mutating cast helper that skips norms/biases/pos_embeds when target is FP8
- [`src/SharpInference.Diffusion/Quality/FluxQualityLoader.cs`](../src/SharpInference.Diffusion/Quality/FluxQualityLoader.cs) — Flux-specific applier over `FluxCheckpointConverter.ConvertedWeights`
- [`src/SharpInference.Diffusion/Quality/SdxlQualityLoader.cs`](../src/SharpInference.Diffusion/Quality/SdxlQualityLoader.cs) — same shape for SDXL
...
**New:**
- [`src/SharpInference.Diffusion/Adapters/ControlNetFile.cs`](../src/SharpInference.Diffusion/Adapters/ControlNetFile.cs) + [`ControlNetLoader.cs`](../src/SharpInference.Diffusion/Adapters/ControlNetLoader.cs)
- [`src/SharpInference.Diffusion/Adapters/IpAdapterFile.cs`](../src/SharpInference.Diffusion/Adapters/IpAdapterFile.cs) + [`IpAdapterLoader.cs`](../src/SharpInference.Diffusion/Adapters/IpAdapterLoader.cs)
...
**New:**
- [`src/SharpInference.Cuda/CublasLtApi.cs`](../src/SharpInference.Cuda/CublasLtApi.cs) — P/Invoke for cuBLASLt (handle, matmul desc, layout, preference, dispatch)
- [`src/SharpInference.Cuda/Fp8GemmExecutor.cs`](../src/SharpInference.Cuda/Fp8GemmExecutor.cs) — single-handle wrapper, `IsSupported` gates SM ≥ 8.9

**Modified:**
- [`src/SharpInference.Cuda/CudaBackend.cs`](../src/SharpInference.Cuda/CudaBackend.cs) — added `EnableNativeFp8Gemm` opt-in flag, `Fp8Executor` lazy property, and gated dispatch in `Linear` (FP8×FP8 → F16 with native GEMM when enabled + supported)
...
**New:**
- [`tests/SharpInference.Diffusion.Tests/SdxlSsimTests.cs`](../tests/SharpInference.Diffusion.Tests/SdxlSsimTests.cs) — strict gate 0.90 when `init_noise_seed42.bin` present, loose 0.30 fallback
- [`tests/SharpInference.Diffusion.Tests/FluxSsimTests.cs`](../tests/SharpInference.Diffusion.Tests/FluxSsimTests.cs) — Dev (10 step) + Schnell (4 step) at strict 0.85 / loose 0.30
- [`tests/SharpInference.Diffusion.Tests/T5EncoderDiffTests.cs`](../tests/SharpInference.Diffusion.Tests/T5EncoderDiffTests.cs) — CPU avg_err < 1e-4, GPU avg_err < 1e-3 vs HuggingFace transformers reference

**Modified:**
- [`src/SharpInference.Diffusion/Requests/TextToImageRequest.cs`](../src/SharpInference.Diffusion/Requests/TextToImageRequest.cs) — added `Tensor? InitialNoise` (pipeline takes ownership, disposes after use; eliminates RNG mismatch with PyTorch for SSIM tests)
- [`src/SharpInference.Diffusion/Pipelines/SdxlPipeline.cs`](../src/SharpInference.Diffusion/Pipelines/SdxlPipeline.cs) — `BuildInitialLatent` now uses `TakeOrCreateNoise` helper that consumes injected noise when present
- [`src/SharpInference.Diffusion/Pipelines/FluxPipeline.cs`](../src/SharpInference.Diffusion/Pipelines/FluxPipeline.cs) — same wiring on the unpacked-noise path
...
**Modified:**
- [`src/SharpInference.Core/Tensors/DType.cs`](../src/SharpInference.Core/Tensors/DType.cs) — added `Q5_K` (176 bytes / 256 elements)
- [`src/SharpInference.ModelHandler/Gguf/GgufLoader.cs`](../src/SharpInference.ModelHandler/Gguf/GgufLoader.cs) — corrected `MapGgufType`: was mapping `14 → Q4_K` (wrong; 14 is Q6_K). Now `12 → Q4_K, 13 → Q5_K, 30 → BF16` per ggml.h. Throws on unknown IDs.
- [`src/SharpInference.ModelHandler/Gguf/GgufDequantizer.cs`](../src/SharpInference.ModelHandler/Gguf/GgufDequantizer.cs) — fixed Q4_K scale extraction (was `dmin * (scales[2*j+8] >> 4)` due to operator precedence bug; now uses canonical ggml `get_scale_min_k4` static helper). Added Q5_K dequantizer using both low-nibble and high-bit reconstruction.
...
**Modified:**
- [`src/SharpInference.Diffusion/Models/Denoisers/QwenImageConfig.cs`](../src/SharpInference.Diffusion/Models/Denoisers/QwenImageConfig.cs) — new `V1` preset matching actual `Qwen/Qwen-Image` (60 layers, hidden=3072, 24 heads). `V1_7B` retained as alias; clarified the suffix was a misnomer.
- [`src/SharpInference.Diffusion/Models/TextEncoders/LlamaStyleEncoderConfig.cs`](../src/SharpInference.Diffusion/Models/TextEncoders/LlamaStyleEncoderConfig.cs) — added `Qwen2_5_VL_7B` preset (28 layers, hidden=3584, GQA 28:4) for Qwen-Image's text encoder.
...
- [`docs/Research/BENCHMARKING.md`](Research/BENCHMARKING.md) — procedure for SharpInference vs ComfyUI it/s comparison
