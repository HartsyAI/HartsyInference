...
| **Hunyuan-GameCraft 1.0** (Tencent) | 12.5B HunyuanVideo MM-DiT + CameraNet (Plücker rays) + 33-ch composite history. PCM+CFG 8-step. Reusable `.pt` pickle loader + N-axis rope. No license gate (engine MIT, user-supplied weights — and the HF repo itself is public/ungated, confirmed 2026-07-21). **Catalogued, not yet loadable via `WorldService`**: its real checkpoint set (DiT + Llava-Llama3-8B + CLIP-ViT-L + 3D VAE) is **~51GB minimum** — the actual blocker is size, not licensing, and no multi-checkpoint loader exists yet regardless. Selecting `-m hunyuan-gamecraft` now fails fast with a clear message instead of silently mis-loading as Oasis. |
