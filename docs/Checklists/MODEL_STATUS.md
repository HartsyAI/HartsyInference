...
| **Boogu-Image 0.1 (Base / Turbo / Edit)** | 🔧 | DiT built (2026-06-21) | structural tests pass | **DiT + scheduler + converter + pipelines built end-to-end 2026-06-21** (structural; numerics validation-pending). 10B OmniGen2/Lumina-2 lineage: `BooguImageTransformer` (hidden 3360, 8 dual-stream + 32 single-stream + 3×2 refiner blocks, GQA 28:7, head_dim 120) **reusing** `OmniGen2Block` (single/refiner — byte-identical sandwich-norm block; added a precomputed-RoPE-table `Forward` overload) + `OmniGen2Rope` (added `BuildTableFromPositions` for the edit `pe_shift` offsets) + `DiTUtils` (added `PatchifyNCHW`/`UnpatchifyToNCHW`) + new `BooguImageDoubleBlock` (joint cross-attention + image self-attention dual stream) + `BooguImageConfig.V01`. New `BooguFlowMatchScheduler` (v1 logistic static time-shift, **ascending** t: noise→data). `BooguImageCheckpointConverter` (transformer + FLUX VAE + Qwen3-VL language tower + vision tower buckets). `BooguImagePipeline` (`GenerateFromEmbeddings` T2I single CFG; `EditFromEmbeddings` double guidance with VAE-encoded reference latents). Reuses FLUX `VaeEncoder`/`VaeDecoder` (`VaeConfig.Flux`) and the `LlamaStyleEncoder` Qwen3-VL-8B language tower. 7 structural smoke tests pass (full T2I + edit DiT forward finite; v1 scheduler math; vision tower + multimodal encoder). **Qwen3-VL-8B vision tower built 2026-06-21** (`Qwen3VlVisionConfig` + `Qwen3VlImageProcessor` smart-resize/CLIP-norm/merge-block patch flatten + `Qwen3VlVisionEncoder`: patch-embed, bilinear-interpolated learned pos-embed, 2D split-half RoPE, 27 full-attention blocks, final merger + **deepstack** post-shuffle mergers at taps [8,16,24]) + `Qwen3VlMultimodalEncoder` (HF `get_rope_index` 3D positions → interleaved **M-RoPE** table, vision-token splice into LM embeds, deepstack injection) wired through a new additive `LlamaStyleEncoder.EncodeEmbedsMrope`/`LookupEmbeddings`. Also a reusable subsystem for Lance/OmniGen2 edit. **Fixed a latent engine-wide CPU SDPA bug** while validating: a rank-2 `[Sq,Skv]` causal mask (the `LlamaStyleEncoder` pattern) was mis-indexed for heads > 0 (OOB read → flaky NaN); `AttentionKernels` now branches on mask rank. **Remaining:** Python-parity dumps for numeric validation. Research doc: [`docs/Research/BOOGU_IMAGE.md`](../Research/BOOGU_IMAGE.md). |
