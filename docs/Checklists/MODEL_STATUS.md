...
| **Hunyuan-GameCraft 1.0** (Tencent) — Phase 10 | ❌ | not started | not started — **license-restricted** | 12.5B HunyuanVideo MM-DiT (19 double + 38 single blocks) + CameraNet (Plücker 6-channel ray maps) + 33-channel composite history input `[noisy(16) + ref_history(16) + mask(1)]`. PCM + CFG distilled 8-step variant. Llava-Llama-3-8B + CLIP-ViT-L text encoders. HunyuanVideo 3D causal VAE. Weights are PyTorch `.pt` pickle (90 GB total), needs one-off conversion. **License: Tencent Hunyuan Community — forbidden in EU/UK/SK, 100M MAU cap.** HartsyInference will not bundle weights or auto-download; license-acceptance gate enforced at load time. Research doc: [`docs/Research/HUNYUAN_GAMECRAFT_ARCHITECTURE.md`](../Research/HUNYUAN_GAMECRAFT_ARCHITECTURE.md); build plan: [`docs/Checklists/PHASE_10_INTERACTIVE.md § 7`](PHASE_10_INTERACTIVE.md). |
