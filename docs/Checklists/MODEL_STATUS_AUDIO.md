...
| **Kyutai TTS** (tts-1.6b-en_fr) | 🔬 | Cores verified (backbone 1.3e-4, depformer 32/32, conditioner ~1e-8). **Mimi decode reconcile DONE 2026-07-15 — bit-exact (corr 1.000000 vs moshi).** The DSM `tokenizer-*.safetensors` is moshi-native, not HF: new `MimiDsmWeights.Adapt` remaps upsample/quantizer(`rvq_first`/`rvq_rest`)/SEANet(`decoder.model.N`, `.conv.conv`→`.conv`); `MimiTransformer` now detects the DSM layout (fused `in_proj_weight` split + `.transformer.` prefix) AND uses **interleaved RoPE** (moshi native) vs the HF split-half — the interleave was the last 0.993→1.0 gap. Full e2e now runs with a real `kyutai/tts-voices` embedding (`speaker_wavs [1,512,T]`→[1,T,512]) at correct pacing (13 words→7.68s). Added top-k/temp sampling (audio 0.8/250) to the depformer (was greedy argmax). **BUT output still not intelligible** (whisper hallucinates on it) — a deeper generation-composition bug remains (text scheduling / delay alignment / a forward under real weights); the isolated-core parities don't cover the frame-loop composition. NEXT: frame-by-frame A/B vs moshi `TTSModel` to find the divergence. NOT deployed to Swarm (not worth deploying until intelligible). |
