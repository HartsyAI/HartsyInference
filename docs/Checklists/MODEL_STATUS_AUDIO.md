...
| **Bark / Chatterbox / VibeVoice / FishSpeech** | ✅ | Swarm e2e word-correct (2026-07-13/14). Bark/Chatterbox/VibeVoice correct but slow (host/AR-bound) → perf-pass follow-up. |
...
| **Zonos-v0.1** (transformer) | 🚧 | **Engine parity-verified end-to-end; stochastic-gen tuning open (2026-07-16).** Every component matches the reference on real weights: ResNet293 speaker encoder (mel corr **1.0**, 128-d embedding corr **1.0** CPU / **0.999999** CUDA vs `SpeakerEmbeddingLDA`), prefix conditioner cond+uncond corr **1.0**, phoneme tokenizer (189-symbol) exact, backbone prefill logits corr **1.0** + argmax-exact for all 9 codebooks, **decode logits bit-exact (maxAbs ~3e-5, 0 argmax flips) for 33 AR steps**, and **greedy generation matches Python bit-for-bit** (301 frames, Whisper identical "Oh is this a test…", GPU F32, RTF 9.2). Fixed 3 backbone parity bugs by parameterizing the reused `DiaAttention`/`DiaMlp` (**interleaved RoPE**, **`up·silu(gate)` MLP half**, **`1/√head_dim` scale**), a delay-revert off-by-one, added `Tensor.CastTo` F64→F32 (LDA weights are doubles), collapsed to a **single backbone** (was 16 GB dual → 8 GB, matches reference weight-sharing), added `PreloadWeights` (RTF 136→9.2), and wired **auto-F32** (`HighPrecisionGemm`; TF32's ~1e-3/forward accumulates over the AR loop → degeneration). **Open:** stochastic (min-p/temp/rep) generation degenerates where greedy succeeds — Python's stochastic (seed-dependent) produces full speech; the engine's does not (sampling-trajectory/robustness under investigation). Extension `zonos_tts` wired (`ZonosModel.cs` + `ZonosTts` facade). Golden `zonos_golden.py`; tests `ZonosSpeaker/Conditioning/Generate/Phoneme/E2eTests`. |
