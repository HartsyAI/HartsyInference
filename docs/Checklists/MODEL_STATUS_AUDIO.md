...
| **StyleTTS2** (LibriTTS) | 🚧 | **Build in progress 2026-07-15.** ✅ **StyleEncoder reconciled + VERIFIED corr 1.000000** vs the Python ref (StarGAN-v2 ResBlk learned-downsample + spectral-norm σ-fold + odd-width replicate-pad). ✅ All 6 submodules load; ✅ `LoadFromCheckpoint` + 178-symbol tokenizer + espeak-IPA phonemizer + clone pipeline wired. **Blocker:** the LibriTTS decoder is `type: hifigan` (4-stage 10·5·3·2, Snake, harmonic-noise source), **not** Kokoro's iSTFTNet → needs a dedicated StyleTTS2 **HiFiGAN generator** (engine has `SnakeResBlock` + harmonic source to reuse). Diffusion (Random mode) still a scaffold. See `AUDIO_TTS_BRINGUP_PLAN.md`. |
