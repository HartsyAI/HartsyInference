...
- [x] **[VG] DONE (2026-07-15) gen-perf campaign Rounds 1–8** ([THREED_GENPERF_PLAN.md](THREED_GENPERF_PLAN.md)) — **87 → 9.2 s (30 steps / grid 128 on the 4090), within 1.6× of Python hy3dgen fp16 (5.76 s)**. All bit-exact or coherence-gated: cuDNN fused SDPA + DiT CUDA-graph + F16 activations, then the flagship freebie — a fused `Concat` kernel (a per-slice `cuMemcpyDtoDAsync` loop was ~280k memcpy nodes/forward → dit-loop 27.7 → 7.5 s, bit-identical mesh) — plus DINOv2-giant host loops → device (cond 4.1 → 0.87 s), fused DiT adaLN + QKV-split-norm kernels, and a device FourierEmbed for the VAE. Batched CFG measured and ruled out (no per-forward overhead left to amortize). New shared ops: `IBackend.{LayerNormModulate,QkvSplitNorm,FourierEmbed}` + `dit_concat2` (parity-covered in `CudaOpBisectTests`, all corr 1.0).
