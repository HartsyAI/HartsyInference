...
| **HeartMuLa** (oss-3B LM) | 🔬 | LM verified vs upstream `heartlib` (bf16/3060): backbone+c0 head **argmax exact** (corr 0.9996) · depth decoder cb1–7 corr 0.9998 (6/7 argmax exact; the 1 flip has a **0.000 top-2 gap** = bf16 tie, not a bug) | LM (CSM-shaped, torchtune llama3_2) reconciled to the **real key layout** + config fixed (see below). `HeartMulaCheckpointConverter` remaps torchtune→HF keys + splits the fused `audio_embeddings`/`audio_head`. **HeartCodec is NOT the scaffolded WaveNet** — the real codec is a 24+6-layer adaLN-single DiT flow-matching estimator + RVQ + SQ-Codec `ScalarModel` (818 keys); the C# `HeartCodecDecoder`/`HeartCodecCfmEstimator` are wrong-architecture and unimplemented → no audio decode yet. Probe: `/tmp/hmprobe`, oracle: `/tmp/heartmula_ref/oracle.py`. |
...
| **Dia-1.6B** | 🔬 | full transformer corr 1.0: encoder (12L) + decoder (18L, cross-attn / 9-ch / fused head, teacher-forced) | DenseGeneral transpose adapter + split-half RoPE + attn scale 1.0 + `DacDecoder`-style mlp/logits rename + KV-cache `AdvanceLength` fix. Only DAC wiring (shared/✅) + delay-AR remain. See [[dia-fishspeech-build]]. |
...
| **YuE** (music, Stage-1) | 🔬 | **Stage-1 7B LM verified**: teacher-forced logits maxAbs **2.5e-5**, **corr 1.0**, **argmax 8/8 match** (incl. xcodec cb0 tokens) vs a standalone Llama-2-GQA oracle (CUDA + `HighPrecisionGemm`, bf16 host-resident; hidden maxAbs 5.3e-5) | LM (`m-a-p/YuE-s1-7B-anneal-en-cot`, plain HF Llama keys) reconciled: real config is **GQA `num_key_value_heads=4`** (k/v_proj `[512,4096]`) and **vocab 83968**, both were wrong in `YueConfig.V1` (MHA 32 / vocab 100000). Tokenizer fixed: markers are SP **control symbols** `<SOA>`/`<EOA>`/`<stage_1>` (not `[SOA]` …) → resolved by pinned ids + Decode round-trip (old `EncodeToIds` path threw at load). **e2e audio BLOCKED**: the C# `XCodec` is **wrong-architecture** for the real `xcodec_mini_infer` codec (DAC factorized RVQ `quantizer.quantizers.*.in_proj` vs real EMA-VQ `quantizer.vq.layers.*._codebook.embed`, 12 codebooks, missing the mandatory `fc_post2` 1024→256 before `decoder_2`) — `XCodec.LoadWeights` proven to throw `KeyNotFound`. Stage-2 (multi-codebook) intentionally out of scope. Probe `/tmp/yueprobe`, oracle `/tmp/yueprobe/ref/oracle.py`. |
...
| YuE (XCodec) | The C# `XCodec` (DAC-style: factorized RVQ `quantizer.quantizers.*.in_proj/out_proj`, 8 codebooks, no `fc_post2`) is **wrong-architecture** for the real `xcodec_mini_infer` codec (SoundStream: EMA-VQ `quantizer.vq.layers.{0..11}._codebook.embed [1024,1024]`, 12 codebooks, `dac2.Encoder(64,ratios,256)`/`decoder_2`, mandatory `fc_post2` 1024→256). The converter even *drops* `fc_post2` | `XCodec.LoadWeights` proven to throw `KeyNotFound: quantizer.quantizers.0.in_proj.weight` on the real (converted) weights; `decode()` path read from upstream `soundstream_hubert_new.py` | **not fixed** — XCodec needs a rewrite (out of scope; e2e audio decode blocked, like HeartCodec) |
