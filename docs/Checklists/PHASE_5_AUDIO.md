...
- [x] Timing conditioner — [`StableAudioNumberEmbedder.cs`](../../src/HartsyInference.Diffusion/Models/Denoisers/StableAudioNumberEmbedder.cs) (clamp/normalize → `LearnedPositionalEmbedding` Fourier → Linear(257→768)); T5 reuse from HartsyInference.Diffusion still pending wiring — the DiT takes raw (pre-projection) cross-attn/global cond tensors, composing T5+timing tokens is the caller's (pipeline's) job. **Real-weight parity VERIFIED (2026-07-18)** — [`StableAudioNumberEmbedderParityTests.cs`](../../tests/HartsyInference.Diffusion.Tests/StableAudioNumberEmbedderParityTests.cs) + [`gen_timing_reference.py`](../../tests/python-reference/stable_audio_open_small_parity/gen_timing_reference.py) (inlines `stable_audio_tools.NumberConditioner` verbatim to skip its heavy unrelated deps) → **cosine 1.000000, maxAbsDiff 9.5e-7**.
