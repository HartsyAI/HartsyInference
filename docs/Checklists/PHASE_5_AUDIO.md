...
> **Research scope:** all 27 audio research docs landed 2026-05-17 (see `docs/Research/`).
...
### Moonshine — BUILT (1st-gen + 2nd-gen streaming)
...
#### Moonshine 2nd-gen streaming (`UsefulSensors/moonshine-streaming-{tiny,small,medium}`) — BUILT + REAL-WEIGHT VERIFIED (2026-07-19)
> Part of the "Moonshine Voice" toolkit (github.com/moonshine-ai/moonshine) — a genuinely different
> architecture from 1st-gen, not a drop-in upgrade: mel-free frame-CMVN + asinh-compression + causal-conv
> front end (not the original's raw 3-conv stem), per-encoder-layer SLIDING-WINDOW attention with NO RoPE
> at all in the encoder (the original's encoder RoPE is absent here), untied LM head, and a learned
> absolute positional embedding added to the ENCODER states (not decoder tokens) before cross-attention.
> Spec extracted directly from the real `transformers>=5.14` `modeling_moonshine_streaming.py` (ground
> truth, cross-checked line-by-line against the source, not just the config/weight shapes) — see
> `MoonshineStreamingEncoder.cs`'s class doc for the full architecture delta list. The DECODER block
> itself (RoPE self-attn → cross-attn (no RoPE) → SwiGLU MLP, weight-only LayerNorm) is architecturally
> IDENTICAL to 1st-gen's `MoonshineDecoder`, so it's reused unchanged via a thin `MoonshineStreamingDecoder`
> wrapper — only `TieWordEmbeddings=false` (untied `proj_out`, now wired into `MoonshineDecoder`) plus the
> encoder-side positional-embedding-add are new.
- [x] [`MoonshineConfig.cs`](../../src/HartsyInference.Audio/Models/Moonshine/MoonshineConfig.cs) — `StreamingTiny`/`StreamingSmall`/`StreamingMedium` presets (dims verified from the real `config.json` of each; small/medium's encoder hidden_size/head_dim genuinely differ from the decoder's — new `Encoder*` fields carry those) + `SlidingWindows` per-layer `(left,right)` spans + `TieWordEmbeddings` now actually wired (was a dead config field before this pass)
- [x] [`MoonshineStreamingOps.cs`](../../src/HartsyInference.Audio/Models/Moonshine/MoonshineStreamingOps.cs) — frame CMVN, learned asinh compression, the encoder's "unit-offset gamma" LayerNorm (`LayerNorm(elementwise_affine=false) * (gamma+1)` — distinct from the decoder's plain weight-only norm), dense sliding-window mask builder
- [x] [`MoonshineStreamingEncoder.cs`](../../src/HartsyInference.Audio/Models/Moonshine/MoonshineStreamingEncoder.cs) — causal-conv front end (`backend.Conv1d` left-pad-only) + N sliding-window-attention layers (no RoPE) + plain (non-gated) GELU MLP
- [x] [`MoonshineStreamingDecoder.cs`](../../src/HartsyInference.Audio/Models/Moonshine/MoonshineStreamingDecoder.cs) — thin wrapper: adds the learned positional embedding to encoder states (+ optional re-projection when encoder/decoder hidden sizes differ) before delegating to the reused `MoonshineDecoder`
- [x] [`MoonshineStreamingPipeline.cs`](../../src/HartsyInference.Audio/Pipelines/MoonshineStreamingPipeline.cs) — full-utterance batch encode → greedy KV-cached decode, mirrors `MoonshinePipeline`. **Not yet implemented**: true chunked/incremental encoder streaming that exploits the bounded per-layer sliding window to avoid re-processing already-seen audio (the HF reference itself is batch-only; this is genuine follow-up work, not something to port).
- [x] **Real-weight parity VERIFIED** — [`MoonshineStreamingParityTests.cs`](../../tests/HartsyInference.Audio.Tests/MoonshineStreamingParityTests.cs) + [`gen_reference.py`](../../tests/python-reference/moonshine_streaming_parity/gen_reference.py): loads the real `moonshine-streaming-tiny` checkpoint into both our port and the real `transformers` `MoonshineStreamingForConditionalGeneration` → **encoder cosine 0.999998, decoder logits cosine 0.999999**. Only Tiny is weight-verified; Small/Medium share the identical architecture with dims read directly from their own real `config.json` (high confidence, not yet independently parity-tested). **Gotcha hit + documented in the test script**: the HF reference silently SKIPS the sliding-window mask entirely when no `attention_mask` is passed (a reference-code quirk — `per_layer_attention_mask` is only built inside `if attention_mask is not None:`) — the parity fixture generator passes an explicit all-ones mask to force the intended windowed path; without it the reference falls back to full unrestricted attention and parity looks badly wrong (cosine ~0.86) despite the port being correct.
- [x] **Engine-wired (2026-07-20)** — registered as `moonshinestreaming` in [`SttCatalog.cs`](../../src/HartsyInference.Engine/Audio/Stt/SttCatalog.cs) (variant `tiny`/`small`/`medium` → `UsefulSensors/moonshine-streaming-{tiny,small,medium}`, defaults to tiny); until this pass the pipeline was built and parity-verified but unreachable from `TranscribeService`/the CLI/Server/SwarmUI extension — only 1st-gen `moonshine` was registered. New end-to-end test [`MoonshineEndToEndTests.Transcribe_Jfk_OnMoonshineStreamingTiny_MatchesCanonicalText`](../../tests/HartsyInference.Audio.Tests/MoonshineEndToEndTests.cs) exercises the full autoregressive greedy-decode loop (the parity tests above only check the encoder and a single decoder step) — passes word-perfect on the JFK clip, CPU backend, ~22s.
...
- [x] **SentencePiece text decoder BUILT (2026-06-21)** — [`KyutaiSttTokenizer.cs`](../../src/HartsyInference.ModelAssets.Tokenizers/KyutaiSttTokenizer.cs): `Decode(ids)` → text (ML.Tokenizers `SentencePieceTokenizer`, ▁→space) + `Encode`. Consumer supplies the model's `text.model`. Compile-verified (SentencePiece can't be hand-crafted tiny for a unit test); runtime decode validated when the real spm is present.
...
- [x] **BERT WordPiece tokenizer BUILT (2026-06-21)** — [`BertWordPieceTokenizer.cs`](../../src/HartsyInference.ModelAssets.Tokenizers/BertWordPieceTokenizer.cs) (mBERT-cased, raw ids, no special tokens) + [`AudioTextFrontend.BarkText`](../../src/HartsyInference.Audio/Frontends/AudioTextFrontend.cs) applies the `+10048` offset. Consumer supplies `vocab.txt`. 2 tests pass.
- [ ] `BarkSpeakerPrompt.cs` (3-stream speaker-prompt history) + checkpoint validation (`suno/bark`).

### CosyVoice 2 — ✅ E2E VERIFIED ON REAL WEIGHTS + SWARM-DEPLOYED (non-streaming), 2026-07-17
> Zero-shot voice clone works end-to-end on the real `FunAudioLLM/CosyVoice2-0.5B` checkpoint; gallery gen + whisper is word-perfect (`Audio Models/CosyVoice/2-0.5b`, ~34 s/clip). Files under [`src/HartsyInference.Audio/Models/CosyVoice/`](../../src/HartsyInference.Audio/Models/CosyVoice/). **Load recipe (see `CosyVoiceE2eTests` + `cosyvoice2-e2e-bringup` memory):** LM `llm.pt` / flow `flow.pt` / vocoder `hift.pt` use their DEFAULT engine key maps (verified correct against the real checkpoints); the **frozen** S3 speech tokenizer + CAM++ speaker load from **chatterbox `s3gen.safetensors`** (`tokenizer.*` / `speaker_encoder.*`), because CosyVoice ships those only as ONNX whose export fuses Conv+BN (CAM++ head `head.bn*` vanish) and mangles S3 names — the same frozen models chatterbox ships clean-named. **Architecture correction:** the LM is *not* the research doc's "single extended-vocab softmax" — the real `llm.pt` keeps the Qwen text embedding and adds separate `speech_embedding` / `llm_decoder` / `llm_embedding` heads; built to match. Three engine fixes landed: `CosyVoicePipeline` wraps gen in `HighPrecisionGemm` (AR needs F32) + one-time `PreloadWeights`, and `CosyVoiceFlow` trims the prompt-mel region so the vocoder doesn't replay the reference clip. **Perf pass DONE (2026-07-17):** warm gallery gen **28.9 → 5.1 s (5.7×)**, RTF 4.61 → 1.34. Bottleneck was NOT the LM (~1 s, already resident) but the **CFM flow decoder** (`CausalConditionalDecoder`, 20×/gen): flow 24.6 → 5.2 s by moving the estimator fully on-device (new `audio_mish_f32` PTX kernel + `IBackend.Mish`; `ExactGelu`→`GeluErf`, head reshapes→`Permute0213`, time-emb→`BroadcastAdd`, packing→`RepeatTime`/`Concat`). Pure residency refactor — CosyVoice + shared-consumer Chatterbox both re-verified word-perfect. **Follow-ups:** CUDA-graph capture of the estimator step (flow now launch-bound); streaming (5:15 interleave); CosyVoice 1.
...
- [x] **Checkpoint validation + e2e** — `llm.pt`/`flow.pt`/`hift.pt` default key maps confirmed against real weights; S3/CAM++ loaded from chatterbox `s3gen.safetensors` (frozen); env-gated `CosyVoiceE2eTests` (`COSY_DIR`/`COSY_REF_WAV`/`COSY_OUT_WAV`, CUDA) writes a whisper-word-perfect clip. Extension `CosyVoiceModel`/`CosyVoiceProvider` updated + Swarm-deployed.
...
- [x] [`SparkTtsTokenizer.cs`](../../src/HartsyInference.ModelAssets.Tokenizers/SparkTtsTokenizer.cs) — composes `BpeTokenizer` + `ByteLevelCodec` + the `added_tokens.json` map (`<|…|>` split). **Reproduces the validated prompt ids exactly.**
...
### Zonos-v0.1 (transformer) — ✅ SWARM GALLERY VOICE-CLONE VERIFIED (07-17, whisper verbatim)
...
- [x] [`ZonosConditioning.cs`](../../src/HartsyInference.Audio/Models/Zonos/ZonosConditioning.cs) — 7-conditioner prefix + CFG-uncond branch (uncond_vectors; language index = id−(−1)). **cond + uncond prefix corr 1.000000.** **Returns channels-last `[1, P, 2048]`** (the `[B,seq,hidden]` layout `ZonosPipeline.Generate`/`Prefill` consume) — a prior channels-first `[1,2048,P]` return fed the backbone a transposed prefix → instant EOS; per-token guard added. [`ZonosPhonemeTokenizer.cs`](../../src/HartsyInference.Audio/Models/Zonos/ZonosPhonemeTokenizer.cs) — espeak IPA → fixed 189-symbol table. **Exact vs reference ids; espeak stress parity ($u+ words keep primary stress).**
...
- [x] Extension `zonos_tts` (`ZonosModel.cs`) wired + **installed as `Audio Models/Zonos/transformer`**: model + DAC + ResNet293 `.pt` + LDA `.pt`, 24 kHz ref → 16 kHz resample, voice-clone required. **Gallery gen (`GenerateText2Image`) whisper-verbatim 07-17.** **Perf pass DONE 07-17: GPU-resident decode → ~6× (203 → 32 ms/frame stochastic on the 4090, RTF 2.9× slower than real time)** via `DiaAttention.SelfForwardFlash` + `FixedKvCache` in-place append + GQA-native `FlashAttention` (gated on new `IBackend.FlashDecodeSupported`); greedy stays bit-parity. Now GPU-compute-bound on F32 Linear GEMVs (F16 next lever, risky). Bench: [`benchmarks/results/zonos_tts_2026-07-17.md`](../../benchmarks/results/zonos_tts_2026-07-17.md). Minor follow-ups: speaker-encoder host syncs + MLP host-SwiGLU.
...
### VibeVoice (1.5B / 7B / Streaming-0.5B) — multi-speaker BUILT + Swarm word-correct + PERF PASS DONE; streaming pipeline pending
...
> **Perf pass 2026-07-17 — RTF 0.78 (was ~206 s/clip; 134 s → 6.47 s on an 8.27 s clip, 20.7×, faster-than-real-time).** Three rounds, **zero new kernels** (all reused `IBackend` ops): (1) VAE causal convs were CPU `float*` loops interleaved with GPU FFN ops (device↔host round-trip per ConvNeXt block) → routed `SConv1d`/`SConvTranspose1d` → `backend.Conv1d`/`ConvTranspose1d`, channels-first RMSNorm → `Transpose2D`+`RmsNorm`+`Transpose2D`, layer-scale → groups=C kernel-1 `Conv1d` (`gamma`→`[C,1,1]`), streaming combine/tail → `Concat`/`SliceLastDim` (→9.08 s; prefill 43.8→0.33 s / 133×). (2) Batched CFG — cond+uncond → one N=2 diffusion-head forward (→7.21 s). (3) Diffusion-head host-glue → GPU (`SliceAlongLastDim`/`AdaLnModulate`/`AdaLnGatedAdd`/`RmsNormNoAffine` → `SliceLastDim`/`AddScalar`+`Mul`+`Add`/`RmsNorm`-ones; →6.47 s). Added `VibeVoicePipeline.PreloadWeights`. Behavior-preserving: corr **0.999585** vs the host path, identical Whisper transcript, 15/15 unit tests pass. Four stages now balanced (~31/31/24/11 %); remaining lever is CUDA-graph capture (major rewrite). `benchmarks/results/vibevoice_tts_2026-07-17.md`.
...
### ZipVoice (k2-fsa/ZipVoice) — ✅ BACKBONE REAL-WEIGHT PARITY VERIFIED (2026-07-19); tokenizer + sampling loop out of scope
> Flow-matching zero-shot voice-cloning TTS on a "Zipformer" (U-Net-shaped Conformer variant) backbone, ported
> from `k2-fsa/ZipVoice`'s real Python source (`zipvoice/models/modules/zipformer.py` + `scaling.py`) read
> line-by-line. Files under [`Models/ZipVoice/`](../../src/HartsyInference.Audio/Models/ZipVoice/) +
> [`ZipVoicePipeline.cs`](../../src/HartsyInference.Audio/Pipelines/ZipVoicePipeline.cs).
- [x] [`ZipVoiceConfig.cs`](../../src/HartsyInference.Audio/Models/ZipVoice/ZipVoiceConfig.cs) — architecture config, values read verbatim from the real checkpoint's `model.json` (`fm_decoder`: 5 U-Net stages `[1,2,4,2,1]` downsampling, dim 512; `text_encoder`: 1 stage, dim 192; shared rel-pos attention dims).
- [x] [`ZipformerScaling.cs`](../../src/HartsyInference.Audio/Models/ZipVoice/ZipformerScaling.cs) — SwooshL/SwooshR activations + `BiasNorm` (`scale = exp(log_scale) / rms(x - bias)`, no additive bias in the output).
- [x] [`ZipformerAttentionPrimitives.cs`](../../src/HartsyInference.Audio/Models/ZipVoice/ZipformerAttentionPrimitives.cs) — `CompactRelPositionalEncoding`, rel-pos attention scores + rel_shift gather, value-mixing, `NonlinAttention` (`s | x | y` chunk order: `s` gates `x` before time-mixing, `y` gates after).
- [x] [`ZipformerFeedForwardConv.cs`](../../src/HartsyInference.Audio/Models/ZipVoice/ZipformerFeedForwardConv.cs) — feed-forward + depthwise conv module.
- [x] [`ZipformerResample.cs`](../../src/HartsyInference.Audio/Models/ZipVoice/ZipformerResample.cs) — bypass, `SimpleDownsample` (right-pad-by-repeat + softmax-weighted average), `SimpleUpsample`.
- [x] [`ZipformerEncoderLayer.cs`](../../src/HartsyInference.Audio/Models/ZipVoice/ZipformerEncoderLayer.cs) / [`ZipformerEncoderStage.cs`](../../src/HartsyInference.Audio/Models/ZipVoice/ZipformerEncoderStage.cs) — full per-layer sub-block order (ff1 → nonlin_attn → self_attn1 → conv1 → ff2 → bypass_mid → self_attn2 → conv2 → ff3 → norm → bypass) and the per-stage downsample→layers→upsample→bypass-combine pattern.
- [x] [`TTSZipformer.cs`](../../src/HartsyInference.Audio/Models/ZipVoice/TTSZipformer.cs) — shared backbone (`in_proj` → time-embed MLP if applicable → N stages → `out_proj`), used for both `fm_decoder` and `text_encoder`.
- [x] [`ZipVoiceModel.cs`](../../src/HartsyInference.Audio/Models/ZipVoice/ZipVoiceModel.cs) — ties `embed` + `text_encoder` + `fm_decoder`; "average upsampling" duration/frame-to-token-index logic (`BuildFrameToTokenIndex`, pure integer, no trained duration predictor) + `ForwardVelocity` (`[xt | text_condition | speech_condition]` concat feeding the 300-dim `fm_decoder.in_proj`).
- [x] [`ZipVoicePipeline.cs`](../../src/HartsyInference.Audio/Pipelines/ZipVoicePipeline.cs) — Euler sampling + CFG + Vocos decode orchestration.
- [x] **Real-weight parity VERIFIED** — [`ZipVoiceParityTests.cs`](../../tests/HartsyInference.Audio.Tests/ZipVoiceParityTests.cs) + [`gen_reference.py`](../../tests/python-reference/zipvoice_parity/gen_reference.py): constructs the real `TTSZipformer` directly from the real `k2-fsa/ZipVoice` source (imported standalone, bypassing `zipvoice.utils.common`'s `tensorboard` dependency) with the real checkpoint's `fm_decoder`/`text_encoder` weights, on deterministic random input (seed 0). **`fm_decoder` cosine 1.000000 (maxAbsDiff 6.4e-6), `text_encoder` cosine 1.000000 (maxAbsDiff 7.5e-8)**, both on the first run — no C# bugs found in either backbone. The pure-integer `BuildFrameToTokenIndex(9, 41)` also matches the Python `prepare_avg_tokens_durations`/`get_tokens_index` reference exactly (element-for-element). Confirmed en route: the real `TTSZipformer.forward()` takes `x` **batch-first** `(batch, seq_len, in_dim)` (permutes to seq-first internally) — matches the existing C# `Forward(backend, x, t, timestep)` convention, so no shape-convention bug either.
- [ ] `ZipVoiceTokenizer.cs` (English espeak-IPA g2p) — **out of scope for parity**: exactness vs `piper_phonemize` is a separate, accepted scope limitation, not verified here.
- [x] **Engine-wired + Swarm-verified (2026-07-20)** — registered as `zipvoice` in [`TtsCatalog.cs`](../../src/HartsyInference.Engine/Audio/Tts/TtsCatalog.cs) ([`ZipVoiceModel.cs`](../../src/HartsyInference.Engine/Audio/Tts/Models/ZipVoiceModel.cs), mirrors F5-TTS's zero-shot clone contract). The pipeline/backbone were parity-verified 07-19 but the Engine had no catalog entry for it — unreachable from the CLI/Server/SwarmUI extension until now. Real generation confirmed on the live Swarm instance via `/API/GenerateText2Image` (`Audio Models/ZipVoice/base`): cloned the JFK reference voice onto "The quick brown fox...", produced a valid non-silent 24 kHz mono WAV. **Slow** — 11.4 min for a ~10 s clip on this pass; no GPU-residency work done on ZipVoice yet (unlike Stable Audio below), a real follow-up perf-pass candidate.
- [ ] End-to-end pipeline validation (Euler sampling loop + Vocos decode + real audio output) — not yet run against real reference audio; only the two Zipformer backbones and the duration-index math are weight-verified so far.
...
- [x] Voice BPE tokenizer — [`AceStepLyricTokenizer`](../../src/HartsyInference.ModelAssets.Tokenizers/AceStepLyricTokenizer.cs), verified against the real shipped tokenizer: `[lang]` prefix (zh→`[zh-cn]`), spaces→`[SPACE]`=2, HF Whitespace pre-tokenization, `[UNK]`=1 fallback; vocab/merges exported to `Models/Music/ACE-Step-v1-3.5B/`
...
- [x] Timing conditioner — [`StableAudioNumberEmbedder.cs`](../../src/HartsyInference.Diffusion/Models/Denoisers/StableAudioNumberEmbedder.cs) (clamp/normalize → `LearnedPositionalEmbedding` Fourier → Linear(257→768)); T5-base reuse wired in `StableAudioPipeline` — the DiT takes raw (pre-projection) cross-attn/global cond tensors, `StableAudioPipeline.Generate` composes [T5 tokens ‖ timing token] before calling it. **Real-weight parity VERIFIED (2026-07-18)** — [`StableAudioNumberEmbedderParityTests.cs`](../../tests/HartsyInference.Diffusion.Tests/StableAudioNumberEmbedderParityTests.cs) + [`gen_timing_reference.py`](../../tests/python-reference/stable_audio_open_small_parity/gen_timing_reference.py) (inlines `stable_audio_tools.NumberConditioner` verbatim to skip its heavy unrelated deps) → **cosine 1.000000, maxAbsDiff 9.5e-7**.
- [ ] `dpmpp-3m-sde` scheduler (Open 1.0 v-prediction)
- [x] Pingpong scheduler (Open Small rectified-flow, 8-step, no CFG) — [`StableAudioPingPongScheduler.cs`](../../src/HartsyInference.Diffusion/Schedulers/StableAudioPingPongScheduler.cs). NOT `FlowMatchPingPongScheduler` (that one's SD3-shift sigma grid is ACE-Step-specific) — ground-truthed against `stable_audio_tools.inference.sampling.sample_rf(sampler_type="pingpong")`'s actual logSNR-linspace schedule (`logsnr=linspace(-6,2,steps+1); t=sigmoid(-logsnr)`), which is NOT the naive linear `τ=i/steps` the research doc's pseudocode implied. Unit-tested against a hand-computed Python reference ([`StableAudioPingPongSchedulerTests.cs`](../../tests/HartsyInference.Diffusion.Tests/StableAudioPingPongSchedulerTests.cs)).
- [x] `StableAudioPipeline` wiring (T5 → DiT → Oobleck decode → WAV) — [`StableAudioPipeline.cs`](../../src/HartsyInference.Diffusion/Pipelines/StableAudioPipeline.cs). Fixed a pre-existing shared-engine bug hit along the way: `T5Block.Forward` unconditionally disposed `combinedMask`, but `BuildAttentionMask`'s batch==1/no-external-mask fast path returns the T5TextEncoder-owned cached `positionBias` tensor BY REFERENCE — corrupting it after layer 0 for any T5 v1.0 shared-bias config (T5-base/T5-large) called without an attention mask. Fixed in `T5Block.cs` (only dispose when not reference-equal to `positionBias`).
- [x] **SWARM-DEPLOYED + VERIFIED END-TO-END (2026-07-18)** — wired into `SwarmUI-AudioLab` (`StableAudioProvider`, `MusicModels.StableAudio`, `stableaudio_music` handler; HF auto-download from `FastVideo/stable-audio-open-small-Diffusers` + `google-t5/t5-base`, same pattern as MusicGen). Real `/API/ProcessAudio` call on the live Swarm instance (RTX 4090) → valid 3.00s stereo 44.1kHz WAV, RMS 8274, all-finite, in ~3s wall time. Two `UseLocalHartsy=true` dev-deploy gaps fixed (raw file `Reference`s don't pull transitive NuGet deps): added explicit `Content Include` copies for `Microsoft.ML.Tokenizers.dll` + `Google.Protobuf.dll` in `SwarmUI-AudioLab.csproj` (T5Tokenizer's SentencePiece parser needs both; latent for MusicGen too, just never exercised under local-engine dev mode before).
- [x] **Engine catalog binding was missing (fixed 2026-07-20)** — the `StableAudioProvider` extension UI entry existed since 07-18, but the Engine refactor never carried over a `MusicCatalog` entry for it (`stableaudio` was absent from `MusicCatalog.Registry`) — registered via new [`StableAudioMusicModel.cs`](../../src/HartsyInference.Engine/Audio/Music/Models/StableAudioMusicModel.cs). AudioLab's own `AudioEngineBridge._bindings` was *also* missing the `stableaudio_music → stableaudio` entry (the provider was registered but silently unreachable) — fixed in the extension's `AudioEngineBridge.cs`.
- [x] **GPU-residency perf pass (2026-07-20)** — `StableAudioDit`'s `ToMultiHead`/`FromMultiHead`/`RepeatKv` and `StableAudioRope.Apply` were host `float*` loops running every self-attention Q/K per block per step (16 layers × 8 steps); `StableAudioPingPongScheduler.Step` read/wrote the full loop-carried latent via `DataPointer` every one of the 8 outer steps (a synchronous D2H-then-H2D round trip on the main denoise tensor — the established costliest pattern in this codebase, see `sync-h2d-stream-drain` precedent). Replaced with existing `IBackend` ops with zero new kernels needed: `Permute0213`/`RepeatKvHeads` for the reshapes, `ApplyRopeSingle` for RoPE (required re-ordering to apply LayerNorm+RoPE on the pre-permute `[B,L,H,D]` layout — verified layout-order-invariant since LayerNorm only reduces the last dim), and chained `Scale`/`Add` for the ping-pong step math (no fused kernel exists for the 3-tensor lerp-with-fresh-noise, chaining is still GPU-resident and avoids the host round trip). **Re-verified bit-parity after the rewrite**: `StableAudioDitParityTests` cosine 1.000000 (unchanged), all `StableAudioPingPongSchedulerTests` pass against the CPU backend. CUDA-graph capture of the step loop is a natural follow-up now that it's fully host-glue-free, not yet done.
- [x] **Re-verified on Swarm after the fix + perf pass (2026-07-20)** — real `/API/GenerateText2Image` call on the live Swarm instance (`Audio Models/StableAudio/open-small`, RTX hardware): valid non-silent stereo 44.1 kHz WAV, 11.89 s, in **2.85 s generation time** (8 pingpong steps, cuDNN conv1d engine engaged). The 07-18 note above (`ProcessAudio`, ~3s wall) predates the Engine-refactor regression that made this unreachable at all — this entry supersedes it as the current, confirmed-working state.
