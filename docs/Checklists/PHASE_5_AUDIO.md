...
> **Perf pass 2026-07-17 — RTF 1.10 (was ~206 s/clip; 134 s → 9.08 s on an 8.27 s clip, 14.8×).** The acoustic/semantic VAE causal convs ran as CPU `float*` loops interleaved with GPU FFN ops (full device↔host round-trip per ConvNeXt block). Routed `SConv1d`/`SConvTranspose1d` → `backend.Conv1d`/`ConvTranspose1d`; channels-first RMSNorm → `Transpose2D`+`RmsNorm`+`Transpose2D`; ConvNeXt layer-scale → groups=C kernel-1 `Conv1d` (`gamma`→`[C,1,1]`, folded at load, no weight mutation); streaming combine/tail → `Concat`/`SliceLastDim`. Added `VibeVoicePipeline.PreloadWeights`. **Zero new kernels** (all reused ops). Prefill (voice-ref encode) 43.8 s → 0.33 s (133×). Behavior-preserving: output corr **0.999624** vs the host path, identical Whisper transcript, 15/15 VibeVoice unit tests pass. Diffusion head now the top cost (~53 %, launch-bound → CUDA-graph is the remaining lever). `benchmarks/results/vibevoice_tts_2026-07-17.md`.
