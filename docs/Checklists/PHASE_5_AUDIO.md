...
- [x] Extension `zonos_tts` (`ZonosModel.cs`) wired + **installed as `Audio Models/Zonos/transformer`**: model + DAC + ResNet293 `.pt` + LDA `.pt`, 24 kHz ref → 16 kHz resample, voice-clone required. **Gallery gen (`GenerateText2Image`) whisper-verbatim 07-17.** Perf follow-up open (~120 ms/frame; speaker-encoder syncs + F32 GEMM).
