...
### 48. Lens Solid-Black Output — SageAttention's F16 V Prologue Overflows Lens's Un-Normalized Value Stream

**Problem**: Microsoft Lens produced **solid-black** 1024² images through both the SwarmUI API and the CLI, on both checkpoint variants (`lens_turbo_bf16` at 4 steps/cfg 1.0 and `lens_bf16` at 20 steps/cfg 5.0) — 16/16 failures in the 2026-07-26 image benchmark, while ComfyUI rendered the identical checkpoints correctly. Nothing was logged: the pipeline "succeeded" and wrote a black PNG.

**How it was found**: stage stats at every pipeline boundary put the first degeneracy squarely inside the denoise loop — steps 0 and 1 were healthy (latent std 0.98 → 0.93), step 2's `noisePred` was **100% NaN** (524288/524288), and everything downstream (BN un-normalize → unpatchify → VAE decode) faithfully propagated it. Per-block probes narrowed it to block 45 of forward 3; per-op probes inside that block showed every input clean (Q/K RMS-normed at max_abs 18.0/14.3, V at max_abs 71583, zero NaN) and the **SDPA output** carrying exactly **4130 bad elements — one per token** (4096 image + 34 text). One bad element per token is the fingerprint of a single `(head, dim)` column of V going INF: `out[s,h*,d*] = Σ_j p[s,j]·V[j,h*,d*]` is bad for *every* query `s` when one `V[j*,h*,d*]` is inf. `max|V|` across the three forwards ran **18940 → 31281 → 71583**, straddling F16's 65504 at exactly the forward that broke.

**Root cause**: **SageAttention's INT8 flash path casts V to F16 and it is default-on, regardless of `allowF16`.** `CudaBackend.ScaledDotProductAttention`'s F32/no-mask branches dispatch to `SageAttentionInt8` for `head_dim ∈ {64,128}` whenever `UseSageAttn` (i.e. `HARTSY_SAGE_ATTN != 0`, default on) — and its v1 prologue `LaunchSageVF16T` materializes a `[B,H,D,Skv]` **F16** transpose of V. Lens RMS-norms Q and K but **not** V, and its residual stream is genuinely enormous by design (verified against the ComfyUI reference: block-0/1/2 image max_abs 560 / 5024 / 3.16e7 vs our 580 / 5026 / 3.09e7 — the explosion is the architecture, not a port bug), so V grows past 65504 from the third denoise step on. The `allowF16: false` the call site already passed is honored by the cuDNN branch but **not consulted by either Sage branch** — so the model's own assertion that its V is unbounded had no effect.

**Fix** ([LensTransformerBlock.cs](../../src/HartsyInference.Diffusion/Models/Denoisers/DiTBlocks/LensTransformerBlock.cs)): attention is exactly linear in V, so the joint V is scaled by `ValueF16Damp = 1/256` before `ScaledDotProductAttention` and the attention output is scaled back by 256 after — two device-side `backend.Scale` passes, no host sync, ~0.2 s over a whole generation. The factor is a power of two (exponent shift only, no mantissa drift of its own) and sized off the measured worst case: `max|V|` peaks at **139264** in blocks 44-46, sampled against the diffusers reference over the full 4-, 20- and 50-step Lens schedules at 1024² — the damp brings that to 544, ~120× under 65504. Explicit `Scale` calls rather than ZImage #-style `Fp8ScaleFactor` folding (see `F16SandwichDamp`) because Lens's `to_v`/`to_out` carry biases the GEMM alpha would not scale correctly, and because `LoraStack` refuses to merge LoRA into any weight with `Fp8ScaleFactor != 1`.

**Verified**: `lens_turbo_bf16` @ 4 steps/cfg 1.0 and @ 20 steps/cfg 5.0 (deep schedule + the CFG two-forward path), and `lens_bf16` @ its 20-step/cfg-5 defaults — all sharp, on-prompt, and clean through `benchmarks/swarm_image_bench/quality_gate.py` (4/4 unflagged; the pre-fix PNG flags `near-black` + `near-uniform`). Post-fix output is visually indistinguishable from a `HARTSY_SAGE_ATTN=0` control, confirming Sage's INT8 Q/K quantization is fine for Lens once V no longer overflows. Regression guard: `LensGenerationTests.AssertNotNearBlack`.

**Lesson**:
- **`allowF16: false` is not a guarantee — audit which backend branches actually read it.** The two F32 Sage branches in `CudaBackend.ScaledDotProductAttention` ignore the flag entirely; only the cuDNN branch gates on it. A model whose V is un-normalized cannot currently opt out of an F16 V, so it has to defend itself.
- **Q/K RMS-norm bounds the *scores*, never V.** "Bounded scores → F16 attention is safe" is the justification on nearly every `allowF16: true` call site in the codebase; it says nothing about the value stream, and V is the one attention input with no normalizer in front of it.
- **A step-dependent NaN is a threshold crossing, not a logic bug.** When step N-1 is clean and step N is entirely NaN, chart the magnitude of the suspect tensor across steps and look for a format limit (65504, 3.4e38) between them — here 31281 → 71583 named the culprit before any kernel was read.
- **One bad element per token in an attention output localizes to one `(head, dim)` column of V.** The count itself is the diagnosis: `softmax·V` smears a single inf V element across every query row at that exact channel.
- This is the second instance of the same class (see `ZImageBlock.F16SandwichDamp`): an un-normalized activation ingested as F16 by a backend fast path. Any new arch with a hot, unnormalized residual stream needs a magnitude sweep against F16 range before it is trusted on the default attention path.
