...
- [ ] **End-to-end GPU image generation pending** — the validation run hit a SIGSEGV during `CudaBackend` cuBLAS init from inside this dev shell (Flatpak sandbox: host CUDA libs at `/run/host/usr/local/cuda-13.2/...` are not loadable from the sandboxed runtime even with `LD_LIBRARY_PATH` overrides). All SD3.5-specific code paths (converter, auto-detect, JointBlock with dual attention, Sd3Transformer, Sd3Pipeline) verified to load and route correctly — only the cuBLAS/CUDA driver bring-up fails, and that is unrelated to SD3.5. Run from a non-sandboxed terminal (where Flux / Z-Image / Flux2 tests already passed on the same hardware) to produce the first image.
