...
- [x] `ZImageContextRefinerBlock.cs` — RMSNorm + QK-norm MHA + RMSNorm + SwiGLU FFN (no AdaLN, no RoPE — caption-only refinement). Fused QKV.
...
- [x] `ZImageTransformer.cs` — top-level orchestrator. `t_embedder.mlp.{0,2}` (sinusoidal × 1000 → Linear(256→1024) → SiLU → Linear(1024→256)), `cap_embedder` (RMSNorm + Linear 2560→3840), `x_embedder` (patch Linear 64→3840), context_refiner stack on caption, noise_refiner stack on padded image (image-only RoPE), concat, 30× main `layers` with full RoPE, `final_layer` (SiLU → Linear scale-only → LayerNormNoAffine → modulate → Linear → unpatchify). Pads cap and image to multiples of 32 with `cap_pad_token` / `x_pad_token`
- [ ] Qwen3-4B text encoder — handled by `LlamaStyleEncoder` + `Qwen3Tokenizer` in a separate task; ZImagePipeline accepts pre-computed `[B, capLen, 2560]` embeddings
...
- [~] End-to-end Turbo generation test — pipeline plumbing complete; output is recognizable humanoid figure matching prompt but with heavy high-frequency banding artifacts. 8 NFE @ 512×512 takes ~2:44 wall on RTX 3060. See `PHASE_3_DEVIATIONS.md` #25–27 for the 10 bugs found and fixed (single-file naming, dispose-after-noop pad, 8-bug semantic diff vs diffusers). Remaining glitch artifacts likely come from missing attention mask on pad tokens, RoPE phase, or dynamic-vs-static scheduler shift.
- [ ] End-to-end Base generation test — 28 steps, CFG=4 with negative prompt, validates the CFG branch on a real un-distilled model
