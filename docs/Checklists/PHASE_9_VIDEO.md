...
- [x] LTX-2.3 (Lightricks, 22B **audiovisual**) — vendored diffusers `transformer_ltx2.py`/`pipeline_ltx2.py`. **T2V+A COMPLETE END-TO-END (2026-06-18)** — structurally CPU-verified (tiny-config dual-stream forward → finite video+audio velocities), numerics validation-pending. Dual-stream DiT (`LtxVideo2Transformer` + `LtxVideo2Block`/`LtxVideo2Attention`/`LtxVideo2Rope`, 48 layers: gated self-attn + QK-RMSNorm + interleaved RoPE / text cross-attn / bidirectional audio↔video cross-attn / gelu FFN, all AdaLN-Single modulated by **8 global `LTX2AdaLayerNormSingle` tables**) + per-modality text connectors (`LtxVideo2TextConnectors`: Gemma-3-12B all-49-layer features → video `[L,4096]`/audio `[L,2048]`) + video VAE (`LtxVideo2VaeDecoder`) + audio VAE (`LtxAudioVaeDecoder`) + BigVGAN vocoder (`LtxAudioVocoder`) + `LtxVideo2Pipeline` (Gemma 49-layer encode → connectors → flow-match Euler dual-stream denoise + 2-way velocity-space CFG → RGB frames + 48 kHz waveform). `LtxVideo2CheckpointConverter` (per-component prefix routing: DiT `model.diffusion_model.*` stripped, connectors kept-prefixed, `vae.`/`audio_vae.` stripped, `vocoder.` kept, Gemma `text_encoder.*`). Gemma tower reuses `LlamaStyleEncoder.Gemma3_12B` + `EncodeMultiLayer`. Tests: tiny-config transformer forward + 19-case converter routing. Remaining: first-run numeric validation (needs the 22B checkpoint + GPU) + I2V/conditioning variants.
...
- [x] Wan **full variant matrix** (2026-06-19) — the shared `WanVideoTransformer`/`WanVideoBlock`/`WanVideoConfig` + `WanVideoPipeline` now cover **every** Wan2.1 & Wan2.2 model, structurally CPU-verified (numerics validation-pending). Engine additions: (1) config presets `T2V_1_3B`/`T2V_14B`/`I2V_14B_480p`/`I2V_14B_720p`/`T2V_A14B`/`I2V_A14B` (per-variant in/out channels, Wan2.1 8× VAE compression, flow shift); (2) **I2V CLIP image conditioning** — `condition_embedder.image_embedder` (norm/ff/norm + pos_embed) in the DiT + per-block `add_k_proj`/`add_v_proj`/`norm_added_k` image cross-attention (text+image contexts split by `imageContextLen`, per-head outputs summed), reusing `ClipVisionEncoderConfig.ViTH14`; (3) **Wan2.2 A14B MoE** — dual high/low-noise experts switched by `BoundaryRatio·1000` timestep boundary (both preloaded); (4) `IWanVaeDecoder`/`IWanVaeEncoder` so the pipeline drives either the Wan2.1 16-ch or Wan2.2 48-ch VAE; (5) `GenerateImageToVideoClip` (36-ch concat conditioning `[noise, mask, cond-latent]`) alongside the existing TI2V-5B expand_timesteps I2V. Tests: image-conditioned transformer forward + MoE dual-expert + I2V-CLIP pipeline smoke (5 Wan pipeline + 6 Wan transformer tests green). The generic `WanVideoCheckpointConverter` (incl. `img_emb` renames) already routes all variants; A14B loads two transformer files. Remaining: SwarmUI compat-class wiring (engine NuGet republish) + first-run numeric validation.
...
- [x] [`LtxVideoCheckpointConverter.cs`](../../src/HartsyInference.ModelAssets/CheckpointConverters/LtxVideoCheckpointConverter.cs) (2026-06-10) — single-file (`model.diffusion_model.*` + `vae.*`, original naming → diffusers rename tables) and diffusers-folder layouts; FP8 scale folding; per-channel-stats → `latents_mean`/`latents_std`. Pure `RouteKey` unit-tested.
...
- [x] **Wan LoRA support (2026-06-10)** — `LoraFormat.KohyaWan` (kohya/musubi-tuner underscored: `lora_unet_blocks_{i}_self_attn_q.*`) + `LoraFormat.DiffusersWan` (ComfyUI-style `diffusion_model.blocks.*`, PEFT or kohya suffixes, `.diff` entries skipped) via one [`WanLoraMapper`](../../src/HartsyInference.ModelAssets/Lora/Mappers/WanLoraMapper.cs) that reuses the checkpoint converter's verbatim rename table; diffusers-PEFT Wan (`transformer.blocks.*`) rides the existing architecture-agnostic passthrough unchanged. `WanVideoGenerationTests` auto-merges `WAN_LORA_PATH` (strength via `WAN_LORA_STRENGTH`) before `LoadWeights` — e.g. a lightx2v distill LoRA for few-step inference. Numeric merge gate in `LoraWanTests`. See `docs/Design/LORA_KEY_MAPPING.md` § F6/F7.
