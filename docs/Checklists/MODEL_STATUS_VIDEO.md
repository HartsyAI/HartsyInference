...
`numerics.fp8Native` on the 4090: **~3× faster steps on T2V-14B fp8 (1.65 s/step vs ~5.0 s at 320×192/20 steps)** via `Fp8GemmExecutor` (cublasLtMatmul, fp8 weights consumed directly + dynamic absmax e4m3 input quant). Numerics match the F16-cast baseline at the same config (near-identical per-frame profiles, slightly dimmer — within the config's CFG-dark regime). **Production recipe for Ada+: `WAN_NO_CACHE_CASTS=1 numerics.fp8Native`.** Stays opt-in until the fp8-CFG parity item resolves; then evaluate default-on for SM≥8.9.
