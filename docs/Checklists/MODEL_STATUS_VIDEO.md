...
| **Native fp8 GEMM — VALIDATED on Ada (2026-07-02, first exercise)** | `numerics.fp8Native=true` on the 4090: **~3× faster steps on T2V-14B fp8 (1.65 s/step vs ~5.0 s at 320×192/20 steps)** via `Fp8GemmExecutor` (cublasLtMatmul, fp8 weights consumed directly + dynamic absmax e4m3 input quant). ([details](#native-fp8-gemm--validated-on-ada-2026-07-02-first-exercise)) |
...
- [x] **Built, correct, and opt-in (`numerics.ltx2DiffusionVae`).** `LtxVideo25DiffusionDecoder` used to be
...
- [x] ~~fp32 whole-clip VAE activation ceiling~~ **BF16 VAE activations landed (2026-08-02)** — reference
      precision, CUDA default, `numerics.seedvr2VaeF32` reverts; pixel/latent boundaries stay F32 and the
...
- [x] ~~DiT perf pass~~ **Device-resident DiT landed (2026-08-02)** — tokens stay `[L,C]` backend tensors
      ...
      phase split at 960×540 (`diagnostics.logLevel=Debug` prints it): 4090 encode 1.0 s / dit 6.3 s /
...
cliff). One real fix shipped along the way: `AudioGuidanceRescale` (an `LtxVideo2Config` field, default 1.0; the `HARTSY_LTX2_AUDIO_RESCALE` switch it once had is gone and has no knob)
...
- [x] **fl2va, ref2va, and reference-video/audio conditioning all DONE and real-weight adherence-verified**
      ...
      combination on the original single-family profiles. Hybrid-capable profiles now use independent spans,
      but their real-weight canary is still open below. The former Qwen3-VL dense `[S,S]` causal mask / repeated-
      KV ceiling was removed in favor of implicit-causal GQA FlashAttention; focused parity is present, while
      expansion real-weight validation remains open. Design rationale: `docs/Research/MINIMAX_H3.md`.
...
flow (8-step base at half grid + ×2 latent upsample + 3-step refine; `numerics.ltx2TwoStage=false` for
...
`numerics.fp8Native=true` on the 4090: **~3× faster steps on T2V-14B fp8 (1.65 s/step vs ~5.0 s at 320×192/20 steps)** via `Fp8GemmExecutor` (cublasLtMatmul, fp8 weights consumed directly + dynamic absmax e4m3 input quant). Numerics match the F16-cast baseline at the same config (near-identical per-frame profiles, slightly dimmer — within the config's CFG-dark regime). **Production recipe for Ada+: the `numerics.fp8Native=true` knob, with `CudaBackend.CacheWeightCasts` set false.** (That second one was the `WAN_NO_CACHE_CASTS` environment variable; it is a backend property now, with no knob and no variable.) Stays opt-in until the fp8-CFG parity item resolves; then evaluate default-on for SM≥8.9.
