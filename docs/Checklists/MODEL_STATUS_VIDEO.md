...
| **LTX-2.5 22B** (in bring-up) | Variant detection, the `keyframes_abs_pos_embedding` marker and diffusion-VAE key routing are in; **not runnable yet** — still needs the Gemma 4 12B text encoder, the `NADiffusionDecoder` (new `Na3d` primitive) and distilled sampling. Header verification proved the 2.3→2.5 transformer diff is only `ff_bias:false` + `use_keyframes_abs_pos_embedding:true`, and that the conv video VAE and audio VAE are structurally identical to 2.3 (zero code). Real generation is blocked on this box by the 26 GB bf16 Gemma 4 checkpoint, not the DiT. ([research](../Research/LTX_2_5.md)) |
