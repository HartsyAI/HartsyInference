...
| **LTX-2.5 22B** (components built, **not yet runnable end-to-end**) | Every component is ported and parity-checked, but the pipeline still constructs the Gemma-3 tower and the conv decoder, so a full 2.5 bundle is refused with a targeted error rather than mis-decoded. Done: variant detection from checkpoint metadata (the 2.3→2.5 transformer diff is only `ff_bias:false` + `use_keyframes_abs_pos_embedding:true`), the keyframe marker, diffusion-VAE key routing, the distilled 8-step schedule, and both catalog ids. `NADiffusionDecoder` matches the reference at **relL2 2.3e-7** and decodes the real checkpoint; the Gemma 4 encoder matches at **3.9e-7 sliding / 1.1e-6 global** on a tiny config, and its tokenizer is bit-exact against the HuggingFace library on the real 262k vocab. Conv video VAE and audio VAE are structurally identical to 2.3 (zero code). Blocked from real generation on this box by the 26 GB bf16 Gemma 4 checkpoint, not the DiT. Two open items are recorded in the research doc: a suspected missing final norm in `LlamaStyleEncoder.EncodeMultiLayer` that also affects the shipping 2.3 path, and a left-vs-right padding difference from upstream. ([research](../Research/LTX_2_5.md)) |
