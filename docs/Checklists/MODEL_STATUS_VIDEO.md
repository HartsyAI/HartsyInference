...
| **Native fp8 GEMM — VALIDATED on Ada (2026-07-02, first exercise)** | `numerics.fp8Native` on the 4090: **~3× faster steps on T2V-14B fp8 (1.65 s/step vs ~5.0 s at 320×192/20 steps)** via `Fp8GemmExecutor` (cublasLtMatmul, fp8 weights consumed directly + dynamic absmax e4m3 input quant). ([details](#native-fp8-gemm--validated-on-ada-2026-07-02-first-exercise)) |
...
flow (8-step base at half grid + ×2 latent upsample + 3-step refine; `numerics.ltx2TwoStage` for
...
`numerics.fp8Native` on the 4090: **~3× faster steps on T2V-14B fp8 (1.65 s/step vs ~5.0 s at 320×192/20 steps)** via `Fp8GemmExecutor` (cublasLtMatmul, fp8 weights consumed directly + dynamic absmax e4m3 input quant). Numerics match the F16-cast baseline at the same config (near-identical per-frame profiles, slightly dimmer — within the config's CFG-dark regime). **Production recipe for Ada+: the `numerics.fp8Native` knob, with `CudaBackend.CacheWeightCasts` set false.** (That second one was the `WAN_NO_CACHE_CASTS` environment variable; it is a backend property now, with no knob and no variable.) Stays opt-in until the fp8-CFG parity item resolves; then evaluate default-on for SM≥8.9.
