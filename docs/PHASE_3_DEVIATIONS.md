...
## 2. TimestepEmbedding Sinusoidal Order — Sin/Cos Swapped

**File:** `src/SharpInference.Diffusion/Models/Denoisers/UNetBlocks/TimestepEmbedding.cs`

**Before:** `embPtr[b * dim + i] = Cos(angle); embPtr[b * dim + halfDim + i] = Sin(angle);`
**After:** `embPtr[b * dim + i] = Sin(angle); embPtr[b * dim + halfDim + i] = Cos(angle);`

**Reason:** Diffusers `get_timestep_embedding()` outputs `torch.cat([emb.sin(), emb.cos()], dim=-1)` — sin first, cos second. Our code had the order reversed. Since every ResNet block in the UNet conditions on the timestep embedding, this corrupted all noise predictions across all denoising steps.

## 3. CLIP Text Encoder — Missing Final LayerNorm
...
## 4. CUDA SDPA Softmax — PTX Kernel (Resolved)
...
## 5. CUDA Conv2D — Im2Col + cuBLAS SGEMM
...
## 6. CUDA GroupNorm/LayerNorm — Three-Pass Kernels
