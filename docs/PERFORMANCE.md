...
| Chroma1-Radiance (pixel-space, 20 st, cfg 3.5) | **22.4 s** (fp8) | 21.07 s (fp8) / 24.68 s (BF16) | **1.06× — honest fp8-vs-fp8 (07-18)**: still slightly behind Comfy's best fp8. Removed a wasteful per-step CopyInto (re-patchify the uncond pass instead of a fresh-tensor H2D) = 22.53→22.37; but the 4090 runs at **~100 % util (GPU-compute-bound)**, so the profiled "overhead" (CopyInto/H2D-miss) overlaps compute and barely moves the wall, and a CUDA step-graph would be wall-neutral. The residual ~1.3 s gap is raw GPU compute (fp8 GEMM + SDPA + head-permutes) ~6 % behind Comfy's tuned torch kernels — closing it needs kernel-level work (uncertain payoff). Progression: F32 31.18 (the 54.4 s handoff was stale) → **F16 backbone** 30.27 (the deferred "Radiance head audit": img damped at the ForwardCore F16 cast, un-damped at the NeRF `param_generator` GEMM alpha — coherent+exact; only −1 s because a SYNC profile showed **Linear = 58 %, BF16-weight-HBM-bound**) → **fp8 weight requant** **22.53** (`HARTSY_RADIANCE_FP8`: `QuantizeDitBlocksToFp8` on block Linears; native fp8 tensor-core GEMM). **Honest fp8-vs-fp8** (`--fp8_e4m3fn-unet --fast fp8_matrix_mult` on Comfy): Comfy **21.07** vs ours **22.53** → we closed the gap 1.23×→**1.07×** but Comfy's fp8 tensor-core path is still ~1.5 s ahead. (Comfy fp8 *without* fp8_matrix_mult = 26.37 s — slower than its own BF16.) Follow-ups to actually beat it: fp8-quant load = ~76 s host absmax loops (persist fp8 repack to disk), + the profiled H2D_MISS_BIG (1.3 s) / CopyInto (0.67 s) / fp8 the NeRF head. (Changes are working-tree on `1.0.0-alpha.51`, not yet released.) |
...
launch-bound**. That shapes which optimizations pay off:
- **CUDA-graph decode** captures the single-frame backbone + depth steps as replayable graphs (bit-identical audio)
  and removes the ~8 ms/frame of launch overhead → ~5%, the honest ceiling for a launch-overhead lever here.
- **Q8_0 weight quantization** is the real lever — it cuts the bytes streamed per token. Measured **1.41× faster**
  (64.8 vs 91.5 ms/frame), pushing HeartMuLa past real-time. The fused Q8 GEMV is actually *faster* than cuBLAS
  bf16 at M=1 when the weights are GPU-resident (microbench: 0.65–0.73×); the catch is residency — the CSM decode
  must `PreloadWeights` the (device-matmul) weights, or they re-upload every step (that thrash made an earlier build
  ~8× *slower*). Weights are quantized once to a disk GGUF cache (streaming, no OOM) and mmapped thereafter.
...
| `HARTSY_HEARTMULA_QUANT` | Quantize the HeartMuLa LM weights (`q8_0`/`q4_k`/`q5_k`/`q6_k`) to a disk-cached GGUF once (streaming convert, no OOM), then mmap from cache. The quant weights are pinned GPU-resident so the fused quant GEMV reads them from VRAM. **Measured 1.41× faster** at `q8_0` (64.8 vs 91.5 ms/frame) + ~1/2 the VRAM. Off by default (opt-in per gen via env); `q8_0` is the safe/near-lossless choice |
