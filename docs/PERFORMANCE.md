...
| dp4a int8-activation decode GEMV | `HARTSY_DP4A_ON` | **On** | SM ≥ 6.1 (`__dp4a`) | LLM decode (M≤8) GEMVs on Q4_K/Q5_K/Q6_K/Q8_0/Q4_0/Q5_0 weights quantize the activation to int8 (Q8_1) once per call and dot with `__dp4a` (4 int8 MACs/instruction) instead of per-element float dequant — the kernels reach 83-93% of DRAM roofline. Llama-3.2-1B 159→197 tok/s, Qwen3-4B 71→93 (RTX 3060, graph-on) — faster than llama.cpp on both. Numerics bounded by the Q8_1 rounding error (ground-truth-tested per dtype; byte-identical greedy output on the two flagship benchmark checkpoints) | Fused float dequant-GEMV kernels |
