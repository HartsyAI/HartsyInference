...
| **Q8_0 disk-quant** (a `-q8` / `-q8_0` model id) | **64.8** | **15.4** | **1.41× faster**, ~1/2 VRAM, past real-time |
...
- **Weight quantization** ([`CsmWeightCache`](../../src/HartsyInference.Audio/Models/Csm/CsmWeightCache.cs),
  select it with a `-q8`/`-q4` model-id suffix, which `HeartMulaMusicModel.ResolveQuant` maps to q8_0/q4_k — NOT the `numerics.audioLmQuant` knob, which only reaches YuE) — the real lever, **1.41× faster** (64.8 vs 91.5 ms/frame) + ~1/2 the VRAM.
