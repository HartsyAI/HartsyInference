...
> Status: Complete (full inference code + all configs captured from `etched-ai/open-oasis`; safetensors key dump still required) | Last Updated: 2026-05-24 | Needed Before: SharpInference.Interactive (`OasisPipeline`, Phase 10 — world models)
...
For SharpInference, Oasis-500m is the **Phase 10 reference world model**: tiny (~3.4 GB total FP16), fully MIT, end-to-end in pure C# without any new heavy primitives, and trivially validated frame-by-frame against the upstream Python (because both action stream and prompt frames are deterministic given a fixed seed).
...
**No `config.json` is shipped.** The architecture is hard-coded in `etched-ai/open-oasis/dit.py` and `vae.py`. A SharpInference loader must hard-code these constants too (or read them from a `oasis_500m.json` we ship in our `model-data/` directory; recommended).
...
When the **second** world model lands in SharpInference, **that** is where the new VQ-codebook module class belongs:

```
src/SharpInference.Interactive/Models/Tokenizers/
...
This produces a sigmoid-shaped β schedule between effectively 0 and 0.999, smoother than the cosine schedule near `t=0`. SharpInference should reproduce this exactly; no off-the-shelf scheduler we ship today returns it.
...
1. **DiT** ([facebookresearch/DiT](https://github.com/facebookresearch/DiT/blob/main/models.py)) — the base spatial DiT block with adaLN-zero modulation, sinusoidal-MLP timestep embedding, `FinalLayer` shape. SharpInference's existing `DiTBlock` family already implements this exactly.
...
`rotary_freqs` buffers are non-persistent — not in the safetensors file; SharpInference computes them at load time from `H=18, W=32, head_dim=64, max_freq=576`.
...
SharpInference's `OasisPipeline` should accept either a `float[T, 25]` array (one-hot, already encoded) or an `OasisAction[]` struct array with explicit fields. A small `OasisActionEncoder` helper handles the camera normalisation.
...
- [Facebook DiT](https://github.com/facebookresearch/DiT) — adaLN-zero modulation + sinusoidal timestep + FinalLayer. SharpInference's existing DiT blocks match.
...
These are the right precedents to study when SharpInference's *second* world model arrives — but they are out of scope for the Oasis port itself.
...
6. **Behaviour on prompt sizes other than 360 × 640.** The VAE's `Attention` registers `rotary_freqs` as a non-persistent buffer derived from `frame_height=18, frame_width=32` (hard-coded). Other resolutions will break unless you re-initialise. For the SharpInference port: **enforce 360 × 640** at API level (the model card doesn't claim anything else works).
7. **Latent shape after VAE encode.** The code shows `vae.encode(x).mean` returning `(B, 576, 16)` — i.e. *token-major*, not channel-major. Then rearranged to `(B, T, C, H, W)` via `"(b t) (h w) c -> b t c h w"`. SharpInference's tensor reshape utilities will need to follow this convention exactly when round-tripping.
8. **Whether the same `vit-l-20.safetensors` checkpoint can serve as a standalone image VAE** for other SharpInference modalities (e.g. compress arbitrary 360 × 640 images for free). Likely yes — the VAE is task-agnostic — but its 16-channel latent space is **not** the same as SDXL's, so it's not interoperable.
9. **The "Oasis 7B" production model.** Whether Etched/Decart will release it under MIT later. If so, the same SharpInference port should accept it as a config override (`hidden_size`, `depth`, possibly a discrete tokenizer).
10. **Real-time interactive surface in SharpInference.** Phase 10 will need a streaming API: `OasisLivePipeline.PushAction(OasisAction)` → `Tensor frame` per call, with internal sliding-window state. This is non-trivial — the DDIM loop assumes batch generation; for live use we want pre-baked per-step kernels and a persistent latent buffer.
...
A brand-new package, **`SharpInference.Interactive`**, is appropriate for world models (this is Phase 10 in `BUILD_ORDER.md`). Inside:

```
src/SharpInference.Interactive/
├── SharpInference.Interactive.csproj
├── Models/
│   ├── Tokenizers/
...
In **`SharpInference.ModelHandler`**:
...
Compared to existing SharpInference infrastructure, Oasis introduces:
...
This is the **smallest production-quality DiT** in SharpInference. It is the right model for: smoke-testing the world-model pipeline, validating new attention backends (since both spatial and temporal attention are exercised), demonstrating the C# real-time interactive surface, and onboarding new contributors.
