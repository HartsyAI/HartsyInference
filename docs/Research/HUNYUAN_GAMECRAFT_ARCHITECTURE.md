...
- **Any documentation or example must lead with these restrictions.** Do not write code samples that implicitly download the weights.
...
15. **Interactive entry point.** A command taking `--reference-image`, `--prompt`, and action keys from stdin (`w/a/s/d`) to produce chunks on demand, displaying the license warning before doing anything. This would be a `hartsy world` subcommand: the `samples/` tree this step originally proposed no longer exists.
