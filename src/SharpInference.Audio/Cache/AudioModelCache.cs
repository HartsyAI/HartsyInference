...
namespace SharpInference.Audio.Cache;

/// <summary>Resolves HuggingFace model artifacts to a local file path, downloading them
/// on first use into a shared on-disk cache at <c>~/.cache/sharpinference/models/</c>.
///
/// <para>The cache layout mirrors HuggingFace conventions so a given repo lives under
/// <c>~/.cache/sharpinference/models/{owner}--{name}/</c>. Multi-file repos (Whisper has
/// <c>model.safetensors</c>, <c>config.json</c>, <c>tokenizer.json</c>, etc.) all land in
/// the same directory.</para>
///
/// <para>Override the cache root with the <c>SHARPINFERENCE_MODEL_CACHE</c> environment
...
public static class AudioModelCache
{
    ...
    /// <summary>The active cache root. Honors <c>SHARPINFERENCE_MODEL_CACHE</c>;
    /// defaults to <c>~/.cache/sharpinference/models</c>.</summary>
    ...
    {
        string? env = Environment.GetEnvironmentVariable("SHARPINFERENCE_MODEL_CACHE");
        if (!string.IsNullOrEmpty(env)) return env;
        string home = Environment.GetFolderPath(Environment.SpecialFolder.UserProfile);
        return Path.Combine(home, ".cache", "sharpinference", "models");
    }
    ...
    private static HttpClient CreateHttpClient()
    {
        ...
        client.DefaultRequestHeaders.UserAgent.Add(
            new ProductInfoHeaderValue("SharpInference.Audio", "0.1"));
