...
public interface IVideoService
{
    ...
    /// <summary>Executes an already validated in-memory plan against the exact request instance that produced it.
    /// The native implementation uses this to ensure HTTP preflight, cache identity, construction, and execution
    /// share one immutable resolution. External implementations may fall back to their ordinary generation path.</summary>
    Task<VideoGenerationResult> GenerateAsync(VideoPlan plan, VideoRequest request,
        IProgress<StepPreview>? progress = null, CancellationToken cancel = default) =>
        GenerateAsync(plan.Model, request, progress, cancel);
