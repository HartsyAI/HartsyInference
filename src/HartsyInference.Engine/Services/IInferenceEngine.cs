...
/// <summary>The single in-process entry point for running inference: owns the compute backend and the loaded-model cache, and exposes one typed service per capability. A consumer (CLI, HTTP API, SwarmUI extension, direct library use) constructs one, calls the service it needs, and disposes it.</summary>
public interface IInferenceEngine : IDisposable
{
    ...
    /// <summary>Drops every loaded model across all modalities and returns their device memory, leaving the engine usable — the next request reloads. For a host's "free memory" control; <see cref="IDisposable.Dispose"/> is the full teardown that also releases the backend.</summary>
    void FreeMemory();

    /// <summary>Whether a handler is wired for <paramref name="modality"/>.</summary>
    bool IsSupported(Modality modality);

    /// <summary>Cache keys of every image/video recipe pipeline currently constructed and resident. Keys are the internal cache identity (checkpoint path + composition signature), not catalog ids — good enough to answer "what's loaded" for ops, not to look a model back up by name. Text/audio/vision loaded-state isn't tracked centrally yet; each of those services owns its loaded-model bookkeeping internally.</summary>
