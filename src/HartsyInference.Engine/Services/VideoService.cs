...
public sealed class VideoService : IVideoService, IVideoPlanningService
{
    ...
    /// <summary>Resolves SwarmUI's literal <c>&lt;weight[N]:text&gt;</c>/<c>&lt;alternate:...&gt;</c>/
    /// <c>&lt;fromto[N]:...&gt;</c> tags upstream of every video recipe pipeline, so no pipeline ever sees tag garbage
    /// — the same treatment as <c>ImagesService</c>, and through the same helper. The emphasis grammar survives only
    /// for a family whose recipe declares a <see cref="PromptWeightingMode"/>; no video recipe consumes a multi-variant
    /// <c>ConditioningSchedule</c> yet, so the scheduling tags always collapse to their step-0 value.</summary>
    private static VideoRequest NormalizePromptTags(VideoRequest request, PromptWeightingMode weighting) => request with
    {
        Prompt = PromptFeatureFlattening.Prepare(request.Prompt, weighting),
        NegativePrompt = PromptFeatureFlattening.Prepare(request.NegativePrompt, weighting),
    };
        ...
        IProgress<StepPreview>? progress = null, CancellationToken cancel = default)
    {
        ...
        VideoRequest effectiveRequest = NormalizePromptTags(executionPlan.EffectiveSettings.Apply(executionRequest),
            InferenceEngine.VideoPromptWeightingFor(executionPlan.Model));
