...
/// <summary>Resolves the audio-LM quant policy: <c>HARTSY_AUDIO_LM_QUANT</c> (<c>q4k</c>|<c>q8</c>|<c>off</c>) wins when set; otherwise Q4_K single-device (today's fit behavior, byte-identical) and Off when a layer-split placement pools VRAM — the point of sharding is not having to quantize.</summary>
