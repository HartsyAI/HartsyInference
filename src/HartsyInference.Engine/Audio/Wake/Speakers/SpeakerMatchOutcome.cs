...
/// <summary>Why a speaker lookup ended the way it did. A caller gating on a required speaker must distinguish "this is somebody else" from "there was nothing to decide with" — the first is a rejection, the second is a configuration or capture problem that should be logged rather than silently treated as an intruder.</summary>
