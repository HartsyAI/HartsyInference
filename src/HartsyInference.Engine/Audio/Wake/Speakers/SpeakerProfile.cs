...
/// <summary>One enrolled household speaker: the raw enrollment embeddings kept for re-derivation, and the centroid scored against at recognition time. Arrays are shared, not copied — treat them as immutable.</summary>
public sealed record SpeakerProfile
{
    ...
    /// <summary>The individual enrollment embeddings, each L2-normalized. Persisted so the centroid can be re-derived (or adapted) later without asking the household to record again.</summary>
    public required float[][] EnrollmentEmbeddings { get; init; }

    /// <summary>The phrase repeated during enrollment when it was text-dependent, else null. Recorded because a text-dependent profile is only trustworthy when scored on that same phrase.</summary>
    ...
    /// <summary>How many utterances went into the centroid; below <see cref="SpeakerProfileStore.RecommendedEnrollmentUtterances"/> the centroid is dominated by the acoustics of a single recording.</summary>
