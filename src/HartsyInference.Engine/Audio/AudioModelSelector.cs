...
/// <summary>Splits a <see cref="ModelSpec"/> into the catalog descriptor id and the per-model variant hint the
/// descriptors resolve — the Engine-native form of the extension's (provider id, <c>__model_id</c>) pair. The
/// request token is <c>id</c> or <c>id:variant</c> (e.g. <c>whisper:large-v3</c>, <c>acestep:turbo</c>); with no
