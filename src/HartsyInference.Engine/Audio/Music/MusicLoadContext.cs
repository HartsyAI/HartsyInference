...
/// <summary>Load-time context for music model descriptors: the primary backend plus, when the engine placement pools VRAM across GPUs, the ordered shard devices and the resolved LM precision policy. Single-device with <see cref="AudioLmQuant.Q4K"/> is byte-identical to the pre-placement behavior.</summary>
internal sealed record MusicLoadContext
{
    ...
    /// <summary>Ordered shard stages (selector + resolved backend) when the engine placement has ≥2 <c>ShardDevices</c>; null = single-device. Backends belong to the engine pool — do not dispose.</summary>
    ...
    /// <summary>Runner-cache key suffix so a quant/placement change never reuses a stale runner. Empty for the single-device Q4_K default, keeping existing cache keys byte-identical.</summary>
