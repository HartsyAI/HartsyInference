...
/// every key to the diffusers naming <c>VaeDecoder</c> expects and upcasts 16-bit weights to F32, plus the encoder-half
/// construction every img2img-capable recipe needs.
/// <para>The encoder and decoder halves live in one weight dictionary, so a recipe that already staged weights for its
/// decoder builds its encoder from the same object at no extra I/O — see <c>SdxlRecipe</c> for the canonical shape.</para></summary>
public static class LoaderVaeUtils
{
    ...
    /// <summary>Whether <paramref name="weights"/> carries an encoder half at all. Some published checkpoints ship a
    /// decode-only VAE, which is a property of the file rather than of the family — recipes use this to decide between
    /// a hard failure and constructing an encoder-less pipeline that refuses img2img at generate time.</summary>
    ...
    /// <summary>Builds the encoder matching a decoder already loaded from <paramref name="weights"/>, or throws naming
    /// <paramref name="family"/> when the file is decode-only — so a missing encoder half surfaces at load time rather
    /// than as a failed generation much later.</summary>
    ...
    /// <summary>Builds the encoder matching a decoder already loaded from <paramref name="weights"/>, or returns null
    /// when the file is decode-only. Callers must pass the null through to the pipeline so img2img is refused by name;
    /// dropping it silently is the one failure mode that still looks like a successful generation.</summary>
    ...
    /// <summary>The Qwen-Image 3D-causal counterpart of <see cref="TryBuildEncoder"/>. Kept separate because that VAE
    /// normalizes latents per channel (<c>latents_mean</c> / <c>latents_std</c>), which the generic
    /// <see cref="VaeEncoder"/> has no path for — families on <c>VaeConfig.QwenImage</c> must come through here.</summary>
