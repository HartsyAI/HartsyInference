...
/// <summary>Strips the structural <c>&lt;tag&gt;</c> layer off a prompt before it reaches the tokenizer, so region/stage tag characters don't leak into the base conditioning. Encoder-level syntax the engine itself honors — weighting <c>(x:1.3)</c>, alternation <c>[a|b]</c>, <c>&lt;break&gt;</c>, embed markers — is deliberately left intact.</summary>
public static class PromptConditioningResolver
{
    ...
    /// <summary>The video-stage text: the <c>&lt;video&gt;</c> sub-prompt when present (image-to-video's alternate motion prompt), else the tag-stripped global text.</summary>
