...
/// <summary>Engine-native parser for the structural prompt tag syntax (<c>&lt;region:x,y,w,h,strength&gt;</c>, <c>&lt;object:…&gt;</c>, <c>&lt;segment:…&gt;</c>, <c>&lt;clear:…&gt;</c>, <c>&lt;extend:…&gt;</c>, plus the stage tags <c>&lt;base&gt;</c>/<c>&lt;refiner&gt;</c>/<c>&lt;video&gt;</c>/<c>&lt;videoswap&gt;</c>/<c>&lt;pixeldecoder&gt;</c>). Text before any tag — and after a <c>&lt;region:end&gt;</c> — accumulates into <see cref="GlobalPrompt"/>; each other tag opens a section that subsequent text appends to. Unrecognized <c>&lt;…&gt;</c> runs (<c>&lt;break&gt;</c>, <c>&lt;embed:…&gt;</c>, and by the time this parser runs, weighting/scheduling — see <c>PromptTagFlattening</c>/<c>PromptTagScheduling</c>, which run upstream in each service and convert <c>&lt;weight[N]:…&gt;</c> to plain <c>(text:N)</c> parens) are preserved verbatim so encoder-level syntax reaches the tokenizer untouched.</summary>
