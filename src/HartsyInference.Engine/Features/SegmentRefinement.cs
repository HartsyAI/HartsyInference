...
public static class SegmentRefinement
{
    ...
    /// <summary>Returns <paramref name="prompt"/> with every <c>&lt;segment:X&gt;</c>/<c>&lt;clear:X&gt;</c> tag AND the text that accumulates into it (up to whichever tag reopens a different recognized section, or the end of the prompt) removed — everything else, including <c>&lt;region:&gt;</c>/<c>&lt;object:&gt;</c> tags and their own content, is preserved byte-for-byte so a pipeline that re-parses the result on its own (e.g. <c>RegionalPromptResolver.HasRegionParts</c>) sees exactly what it would have seen from the untouched prompt. Mirrors <see cref="PromptRegionParser.Parse"/>'s own split/accumulate loop rather than a regex, because "what belongs to a segment" is defined by that accumulator rebinding, not by the tag's own span — text after <c>&lt;segment:X&gt;</c> up to the next tag is the segment's sub-prompt, not the base prompt. Case-sensitive prefix match, same as the parser itself (a stray-cased <c>&lt;Segment:&gt;</c> is untouched here for the same reason <see cref="PromptRegionParser"/> would fall through and treat it as ordinary prompt text, not a tag).</summary>
    ...
    /// <summary>True when <paramref name="prompt"/> carries at least one <c>&lt;segment:&gt;</c> part — cheap pre-check so a request with no segments never pays for a <see cref="PromptRegionParser"/> parse plus a CLIPSeg weight-directory lookup.</summary>
    ...
    /// <summary>Runs every <c>&lt;segment:X&gt;</c> part in <paramref name="resolved"/>'s prompt against <paramref name="generated"/> (the already-composited base result), returning the final image with every matched segment refined in place. Segments whose CLIPSeg match is empty (nothing scored above the part's <see cref="PromptRegionParser.Part.Strength"/> threshold) or whose grown/oversized mask covers the whole canvas are logged and skipped, not treated as an error — the same tolerance <see cref="InpaintOnlyMasked.Prepare"/> already has for a mask that selects nothing.</summary>
