...
/// <remarks>The order is the point. Folding happens inside the container, before the key transform renames anything,
/// because a transform that renames <c>.weight</c> has no rule for <c>.weight_scale</c> — folding after it pairs
/// nothing, drops the scale, and the component runs at <c>1/scale</c> with no symptom until the image does.</remarks>
