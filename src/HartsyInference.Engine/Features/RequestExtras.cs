...
/// <summary>Typed reads out of <c>ImageRequest.Extra</c> for the knobs the flat contract does not name. The contract is
/// frozen, so features whose resolvers need extra parameters (ControlNet union type, IP-Adapter model/weight/window,
/// Redux blending) source them here under documented keys and fall back to sensible defaults when absent.
/// <para>Keys: <c>controlnet.union_type.{index}</c> (string, per-slot union control type, e.g. "canny");
...
/// <c>redux.apply_start</c> (number, default 0.0); <c>flux.tools_control_image</c> (<see cref="Requests.ImageData"/>,
/// FLUX.1 Canny/Depth's already-preprocessed control hint — absent means the checkpoint is driven as vanilla Flux
/// or FLUX.1 Fill instead).</para></summary>
public static class RequestExtras
{
    ...
    /// <summary>FLUX.1 Canny/Depth's already-preprocessed control hint (<see cref="Requests.ImageData"/>) — the host
    /// runs the edge/depth annotator (it needs host-app image types) and hands back the finished map. Absent means
    /// no Tools control image was supplied; the recipe still resolves the checkpoint's actual variant from the
    /// loaded weights, so an absent hint on a genuine Tools checkpoint surfaces as <see cref="Diffusion.Pipelines.FluxPipeline"/>'s
    /// own clear "requires a control image" error rather than a silent fallback.</summary>
    ...
    /// <summary>The <see cref="Requests.ImageData"/> at <paramref name="key"/>, or null when absent or not actually
    /// an <see cref="Requests.ImageData"/> (the bag is <c>object</c>-valued; a host bug that puts the wrong type
    /// under this key should surface as "no image", not an <see cref="InvalidCastException"/> mid-generation).</summary>
