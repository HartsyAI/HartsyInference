...
public sealed class Flux1RecipePipeline : IRecipePipeline
{
    ...
    /// <summary>Wraps the constructed Flux.1 pipeline plus its tokenizers, taking ownership of every disposable. <paramref name="isDev"/> selects the step fallback and whether the embedded distilled guidance is applied.</summary>
    public Flux1RecipePipeline(FluxPipeline pipeline, ClipTokenizer clipTokenizer, T5Tokenizer t5Tokenizer, bool isDev, List<SafeTensorsLoader> loaders)
    {
        _pipeline = pipeline;
    ...
    public ImageResult Generate(ImageRequest request, IProgress<StepPreview>? progress, CancellationToken cancel)
    {
        ...
        int steps = request.Steps <= 0 ? (_isDev ? 20 : 4) : request.Steps;
