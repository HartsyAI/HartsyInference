...
/// <summary>Routes LTX-2 dev-family requests whose checkpoint is a distilled build to the distilled sampling contract. The dev and distilled 2.5 transformers are byte-indistinguishable in structure (same model version, config, and tensor keys), so the FILENAME is the only signal there is — the same remap-table approach SwarmUI uses for Hunyuan 1.5 distilled. This is what makes the distilled contract reachable from SwarmUI, which can only send the dev family id.</summary>
public static class LtxVideo2DistilledRouting
{
    ...
    /// <summary>True when the checkpoint's file name, leaf directory name, or (for a directory) any contained <c>*.safetensors</c> file name contains "distilled". The directory scan is load-bearing: distilled runs stage a directory (the distilled file is transformer-only; VAE/Gemma are merged from siblings), so the leaf name alone would make routing depend on what the user called their folder.</summary>
    ...
    /// <summary>A dev-family LTX-2 id whose checkpoint is a distilled build returns <see cref="DistilledFamilyId"/>; everything else passes through unchanged. Logs when it fires (once per distinct path) — the remap changes the sampling contract, and that must be visible in the log.</summary>
