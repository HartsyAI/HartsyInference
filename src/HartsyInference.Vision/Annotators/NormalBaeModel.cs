...
public sealed unsafe class NormalBaeModel
{
    ...
    /// <summary>Folds inference BatchNorm (eps 1e-3, the tf_ variants' value) into the preceding conv:
    /// <c>w' = w·γ/√(σ²+ε)</c>, <c>b' = (b−μ)·γ/√(σ²+ε) + β</c>.</summary>
    private static (Tensor W, Tensor B) FoldBn(IReadOnlyDictionary<string, Tensor> w, string convKey, string bnKey)
    {
        ...
        {
            float scale = pg[o] / MathF.Sqrt(pv[o] + BnEps);
