...
/// <summary>Drives a sliding window of resident blocks across a sequential model's forward pass: prefetches blocks ahead of compute, awaits them just-in-time, and evicts blocks that have fallen behind the retain window. SharpInference equivalent of ComfyUI's "lowvram" model-streaming. Single-threaded; call from the inference thread that owns the backend's CUDA context.</summary>
