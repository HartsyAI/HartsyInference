...
//     src/HartsyInference.Cuda/Kernels/dit/dit_f16.cu src/HartsyInference.Cuda/Kernels/dit/dit_f16.ptx compute_80
//   cp src/HartsyInference.Cuda/Kernels/dit/dit_f16.ptx src/HartsyInference.Cuda/Ptx/
...
// Subset-emitting twin of dit_qkv_split_norm_head_major_f16. SEPARATE kernel on purpose: the fused call above must stay
...
__global__ void dit_qkv_split_norm_head_major_subset_f16(
    __half* __restrict__ q, __half* __restrict__ k, __half* __restrict__ v,
    const __half* __restrict__ qkv, const float* __restrict__ qW, const float* __restrict__ kW,
    unsigned int tokens, unsigned int heads, unsigned int headDim, unsigned int seq, float eps,
    unsigned int packStride, int qSlot, int kSlot, int vSlot)
{
    ...
    unsigned int W = heads * headDim;
    unsigned int d = threadIdx.x;
    const __half* base = qkv + token * (unsigned long long)packStride * W;
    unsigned long long b = token / seq, s = token % seq;
    unsigned long long outOff = ((b * heads + h) * seq + s) * headDim;
    ...
    if (d < headDim)
    {
        if (qSlot >= 0) qv = __half2float(base[qBase + d]);
        if (kSlot >= 0) kv = __half2float(base[kBase + d]);
    }
    float qsum = qv * qv;
    float ksum = kv * kv;
    for (unsigned int i = d + blockDim.x; i < headDim; i += blockDim.x)
    {
        float qi = qSlot >= 0 ? __half2float(base[qBase + i]) : 0.0f;
        float ki = kSlot >= 0 ? __half2float(base[kBase + i]) : 0.0f;
        qsum += qi * qi;
        ksum += ki * ki;
    }
    qred[d] = qsum;
    ...
    if (d < headDim)
    {
        if (qSlot >= 0) q[outOff + d] = __float2half(qv * qred[0] * qW[d]);
        if (kSlot >= 0) k[outOff + d] = __float2half(kv * kred[0] * kW[d]);
        if (vSlot >= 0) v[outOff + d] = base[vBase + d];
    }
    for (unsigned int i = d + blockDim.x; i < headDim; i += blockDim.x)
    {
        if (qSlot >= 0) q[outOff + i] = __float2half(__half2float(base[qBase + i]) * qred[0] * qW[i]);
        if (kSlot >= 0) k[outOff + i] = __float2half(__half2float(base[kBase + i]) * kred[0] * kW[i]);
