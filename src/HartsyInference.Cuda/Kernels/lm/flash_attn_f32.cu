...
// F16-storage KV variant: identical to lm_flash_attn_f32 above (same online-softmax algorithm, same F32
// accumulate — only the two K/V reads differ) except the KV cache buffers are __half (halved VRAM) and
// upconverted to float on load via __half2float. Q, out, scores, softmax, and the accumulator all stay
// F32 — this is a storage/bandwidth change, not a numerically-new kernel. v1 scope: engaged only on the
// monolithic path (split-K and graph-decode force this KV dtype off and fall back here — see CudaBackend).
extern "C" __global__ void lm_flash_attn_f16kv_f32(
    float* __restrict__ out,
    const float* __restrict__ Q,
    ...
    const int* __restrict__ dPos)
{
    ...
    {
        size_t kvBase = (((size_t)b * Hkv + hkv) * Lk + (unsigned int)k) * D;
        sdata[tid] = (tid < D) ? qv * __half2float(K[kvBase + tid]) : 0.0f;
        __syncthreads();
        for (unsigned int s = blockDim.x >> 1; s >= 32; s >>= 1)
        {
            if (tid < s) sdata[tid] += sdata[tid + s];
        ...
        float p = __expf(score - newM);
        l = l * corr + p;
        float vval = (tid < D) ? __half2float(V[kvBase + tid]) : 0.0f;
        acc = acc * corr + p * vval;
        m = newM;
        __syncthreads();
    }
