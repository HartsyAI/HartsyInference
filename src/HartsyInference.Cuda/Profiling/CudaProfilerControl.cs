...
/// <summary>Wrappers around <c>cuProfilerStart</c> / <c>cuProfilerStop</c>. Used in conjunction with <c>nsys profile --capture-range=cudaProfilerApi</c> to bound an Nsight Systems trace to the steady-state measurement window only — typically the inner steps of a denoise loop, after warmup has settled.</summary>
/// <remarks>
/// <para>Example:</para>
/// <code>
/// // Warmup (not captured)
...
public static class CudaProfilerControl
{
    ...
    /// <summary>RAII helper: <c>using (CudaProfilerControl.Window())</c> calls <see cref="Start"/> on entry and <see cref="Stop"/> on exit.</summary>
