...
/// <summary>Native FP8 GEMM via cublasLtMatmul, gated on Ada+ (SM 8.9+) GPUs. Single-tensor allocation lifecycle: a per-call descriptor + layout set is created and torn down inside <see cref="Run"/>. The handle and workspace are owned by the caller and reused across calls.
///
/// <para><b>Hardware requirement.</b> FP8 tensor-core paths exist only on Ada (SM 8.9+) and Hopper (SM 9.0+). On Ampere (SM 8.0/8.6/8.7), the constructor returns <see cref="IsSupported"/> = false and callers must fall back to the cast-then-FP16-GEMM path in <see cref="CudaBackend"/>.</para>
///
/// <para><b>Untested locally.</b> The author's hardware is RTX 3060 (SM 8.6 — Ampere). This path was written from cuBLASLt documentation + reference samples but has not been exercised end-to-end on Ada hardware. The dispatch is gated on <see cref="IsSupported"/> + an explicit opt-in flag in <see cref="CudaBackend"/> until validated on a real Ada GPU.</para></summary>
