...
public sealed class GptOssTokenizer : IDisposable
{
    ...
    /// <summary>Token count of the chat-template wrapper (system + assistant thinking) — used by <see cref="HartsyInference.Diffusion.Pipelines.LensPipeline"/> to strip the prefix off the encoder hidden states. Constant per upstream pipeline.</summary>
