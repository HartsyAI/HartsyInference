...
/// <summary>TRELLIS image-to-3D configuration (<c>microsoft/TRELLIS-image-large</c>): a two-stage flow-matching system — a dense sparse-structure stage (16³ latent → 64³ occupancy → active-voxel coords) then a sparse SLAT stage over those voxels.</summary>
public sealed record TrellisConfig
{
    #region Conditioner (DINOv2 ViT-L/14 with registers)
    /// <summary>DINOv2 conditioner hidden dim (<c>dinov2_vitl14_reg</c> = 1024); patchtokens pass through a final <c>layer_norm</c> before use as cross-attention condition.</summary>
    ...
    /// <summary>SS-VAE decoder latent channels (8) → occupancy out channels (1); channel schedule [512,128,32], 2 res-blocks + 2 middle, ×2 upsample per stage → 64³.</summary>
    ...
    /// <summary>SLAT-flow resolution (64), in/out channels (8), patch size (2), io res-blocks (2) with io_block_channels [128]; DiT-L width/blocks/heads (1024 / 24 / 16), APE pos, qk_rms_norm.</summary>
    ...
    /// <summary>Gaussian-splat decoder: gaussians per active voxel (32), voxel size, softplus scaling bias/opacity bias per the ckpt <c>representation_config</c>.</summary>
