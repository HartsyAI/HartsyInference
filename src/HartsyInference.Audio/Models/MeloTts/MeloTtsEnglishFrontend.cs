...
public sealed class MeloTtsEnglishFrontend
{
    ...
    public sealed record Result(int[] PhoneIds, int[] ToneIds, int[] LangIds, int[] Word2Ph);

    /// <summary>Runs the g2p and packs ids. <paramref name="addBlank"/> interleaves the blank symbol between every
    /// phoneme (always true for the public checkpoints).</summary>
    public Result Process(string text, bool addBlank = true)
    {
        ...
        return new Result(phoneIds, toneIds, langIds, w2p);
    }
    ...
    private static string TextNormalize(string text) => text.ToLowerInvariant();
