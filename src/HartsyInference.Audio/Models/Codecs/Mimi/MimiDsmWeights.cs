...
/// <summary>Adapts a moshi-native "DSM" Mimi checkpoint (the <c>tokenizer-*.safetensors</c> shipped with <c>kyutai/tts-1.6b-en_fr</c> and the STT models) to the HF <c>transformers</c> MimiModel key layout that the decode-path loaders expect.</summary>
/// <remarks>The moshi checkpoint wraps every conv in a streaming module (<c>.conv.conv</c> / <c>.convtr.convtr</c>),
...
/// <c>input_proj</c>) are passed through unchanged; the decode path just ignores them.</para></remarks>
internal static class MimiDsmWeights
{
    ...
    /// <summary>Returns a new dict with the DSM decode-path keys renamed to the HF layout; non-matching keys are copied through. Tensor instances are shared (borrowed from the caller's loader).</summary>
