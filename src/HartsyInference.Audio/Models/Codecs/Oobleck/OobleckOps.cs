...
/// <summary>Shared load-time transforms for the Oobleck VAE: weight-norm fusion (reusing the codec-wide
/// <see cref="WeightNormFusion"/> helpers) and the logscale snake parameter exponentiation that
/// distinguishes Oobleck's Snake1d from DAC's (parameters are stored as logs; the runtime value is
/// <c>exp(param)</c>, computed once here so the hot path uses <see cref="HartsyInference.Core.Backends.IBackend.Snake"/> directly).</summary>
internal static unsafe class OobleckOps
{
    /// <summary>Loads a snake's <c>{prefix}.alpha</c> / <c>{prefix}.beta</c> (stored <c>[1, dim, 1]</c>,
    /// logscale) as exponentiated <c>[dim]</c> tensors ready for the snake-beta kernel.</summary>
    ...
    /// <summary>Fuses a weight-normed ConvTranspose1d's <c>{prefix}.weight_g</c> / <c>.weight_v</c>.
    /// diffusers wraps transpose convs with torch's DEFAULT dim-0 weight_norm, so the norm is per
    ...
    /// from EnCodec's per-output-channel <c>WeightNormFusionT</c> convention.</summary>
