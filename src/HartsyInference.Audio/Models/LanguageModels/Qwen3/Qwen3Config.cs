...
/// <summary>Configuration for a Qwen3 decoder. Qwen3 vs Qwen2: adds <b>per-head q_norm/k_norm</b> (RMSNorm over
/// head_dim, applied post-projection before RoPE), <b>decouples head_dim from hidden_size</b> (q/k/v projections
/// are sized <c>n_heads·head_dim</c>, not hidden), and drops QKV bias. Otherwise Llama-family (RMSNorm, SwiGLU,
/// GQA, RoPE). Used by Qwen3-TTS's talker + code-predictor. See <c>docs/Research/QWEN3_TTS_ARCHITECTURE.md</c>.</summary>
public sealed record Qwen3Config
{
    ...
    /// <summary>Qwen3-TTS talker backbone (1.7B): hidden 2048 / 28L / 16 heads / 8 KV / head_dim 128 /
    /// SwiGLU 6144 / RoPE θ=1e6. (0.6B = hidden 1024 / intermediate 3072.)</summary>
