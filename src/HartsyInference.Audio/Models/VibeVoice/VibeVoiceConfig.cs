...
/// <summary>Configuration for VibeVoice (Microsoft Research, 2025) — long-form multi-speaker TTS built on Qwen2.5 + next-token-diffusion.</summary>
/// <remarks>Fields verified against the public safetensors layout of <c>microsoft/VibeVoice-1.5B</c>,
/// <c>microsoft/VibeVoice-Large</c>, and <c>microsoft/VibeVoice-Realtime-0.5B</c>.
///
...
/// semantic VAE is present and whether the LM is split.</para></remarks>
public sealed record VibeVoiceConfig
{
    /// <summary>Master model_type discriminator: <c>"vibevoice"</c> for multi-speaker (1.5B / 7B), <c>"vibevoice_streaming"</c> for the 0.5B real-time variant.</summary>
    ...
    /// <summary>Semantic VAE (encoder only, 128-d, deterministic mean); <c>null</c> on the streaming variant which has no semantic feedback.</summary>
    public VibeVoiceTokenizerConfig? SemanticTokenizer { get; init; }

    /// <summary>Qwen2.5 LM backbone shape; the actual forward is provided by the dotLLM dependency at pipeline-construction time, this record only sizes connectors and the diffusion head.</summary>
    ...
    /// <summary>Semantic latent channels; always 128 for the multi-speaker checkpoints, unused on the streaming variant.</summary>
    public int SemanticVaeDim { get; init; } = 128;

    /// <summary>Streaming-only: number of upper Qwen layers used for TTS (the lower <c>num_hidden_layers - tts_backbone_num_hidden_layers</c> are text-only); ignored on the multi-speaker variant. Default 20 per <c>VibeVoice-Streaming-0.5B</c>.</summary>
    public int TtsBackboneNumHiddenLayers { get; init; } = 20;

    /// <summary>Classifier-free guidance scale for the diffusion head: <c>eps = uncond + CfgScale*(cond - uncond)</c> from a positive (text+voice conditioned) and a negative (voice/acoustic-only, text-masked) LM stream. 1.3 is the upstream demo default; 1.0 disables guidance (single-stream).</summary>
    public float CfgScale { get; init; } = 1.3f;

    /// <summary>AR token selection. Upstream uses greedy (<c>do_sample=False</c>) once CFG is enabled — the diffusion head stops cleanly; set true to restore temperature/top-p stochastic sampling.</summary>
    ...
    /// <summary><c>VibeVoice-1.5B</c> preset — Qwen2.5-1.5B backbone, 65k context, up to 4 speakers, ~90 min output.</summary>
    ...
    /// <summary><c>VibeVoice-Large</c> (7B) preset — Qwen2.5-7B backbone, 32k context, up to 4 speakers, ~45 min output.</summary>
    ...
    /// <summary><c>VibeVoice-Streaming-0.5B</c> preset — Qwen2.5-0.5B split-LM, single speaker, no semantic VAE, binary EOS classifier.</summary>
...
/// <summary>Shared shape for both the acoustic VAE (encoder+decoder) and the semantic VAE (encoder only), differing only in <see cref="VaeDim"/>, <see cref="FixStd"/> and <see cref="StdDistType"/>.</summary>
/// <remarks><see cref="EncoderRatios"/> is the downsample chain in the public config order;
/// the encoder forward reverses it internally (Python's <c>list(reversed(...))</c>).
/// Total downsample = product = 3200 → 7.5 Hz on 24 kHz input.</remarks>
public sealed record VibeVoiceTokenizerConfig
{
    ...
    /// <summary>Fixed std for the Gaussian sampling at the VAE output: 0.5 (acoustic), 0 (semantic — deterministic).</summary>
    public float FixStd { get; init; } = 0.5f;

    /// <summary>Sampling distribution: <c>"gaussian"</c> draws a per-batch std multiplier from <c>N(0, fix_std / 0.8)</c> on top of the fixed std (acoustic); <c>"none"</c> returns the deterministic mean (semantic); <c>"fix"</c> uses the fixed std.</summary>
    ...
    /// <summary>Per-stage downsample factors, forward-order; the encoder consumes <c>reversed(ratios)</c> internally so low-rate dims are crossed last. Product = 3200.</summary>
    public IReadOnlyList<int> EncoderRatios { get; init; } = [8, 5, 5, 4, 2, 2];

    /// <summary>Per-stage upsample factors for the acoustic decoder; same as <see cref="EncoderRatios"/> for the canonical checkpoints, null on the semantic VAE (encoder only — no decoder).</summary>
    public IReadOnlyList<int>? DecoderRatios { get; init; } = [8, 5, 5, 4, 2, 2];

    /// <summary>Per-stage ConvNeXt block depths, dash-separated matching the Python config <c>"3-3-3-3-3-3-8"</c> — 7 entries (stem stage + 6 downsample stages), the last stage runs 8 blocks at the smallest temporal rate.</summary>
    public string EncoderDepths { get; init; } = "3-3-3-3-3-3-8";

    /// <summary>Optional override for decoder depths; <c>null</c> → reverse of encoder depths. Acoustic decoder follows this default; semantic VAE has no decoder.</summary>
    public string? DecoderDepths { get; init; }

    /// <summary>Mixer type inside <see cref="VibeVoiceConvNeXtBlock"/>; always <c>"depthwise_conv"</c> for the official checkpoints — full Conv1D with groups=channels.</summary>
    public string MixerLayer { get; init; } = "depthwise_conv";

    /// <summary>Normalization in convolutional layers; <c>"none"</c> means no weight-norm / spectral-norm wrapper (just plain Conv1d).</summary>
    ...
    /// <summary>If true, skip the pre-head normalization between the last stage and the final 1×1 conv; the published checkpoints set this true.</summary>
    ...
    /// <summary>Initial value for <c>nn.init.normal_(weight, std=...)</c> in the Python reference; inference doesn't use it, kept for parity / parameter-count audits.</summary>
    public float WeightInitValue { get; init; } = 1e-2f;

    /// <summary>Per-corpus normalization scale applied to input audio before encoding; 0.0 disables it (no normalization) for the published checkpoints.</summary>
...
/// <summary>Shape of the Qwen2.5 LM that VibeVoice wraps — only the dimensions needed to size the speech connectors and the diffusion head; the actual LM forward is supplied by the dotLLM dependency.</summary>
public sealed record VibeVoiceDecoderConfig
{
    ...
    /// <summary>Qwen2.5 vocab size — 151936 (1.5B / 0.5B) or 152064 (7B); VibeVoice does NOT extend this vocab, speech tokens are remapped onto Qwen's existing <c>&lt;|vision_start|&gt;</c> / <c>&lt;|vision_end|&gt;</c> / <c>&lt;|vision_pad|&gt;</c>.</summary>
    public required int VocabSize { get; init; }

    /// <summary>Whether <c>lm_head.weight</c> is tied to <c>embed_tokens.weight</c>; true for 1.5B, false for 7B and the 0.5B realtime variant.</summary>
    ...
    /// <summary>Qwen2.5-0.5B preset (streaming-only variant); layer count is total — the streaming model splits this into lower (4) + upper (20) at runtime per <see cref="VibeVoiceConfig.TtsBackboneNumHiddenLayers"/>.</summary>
...
/// <summary>Shape of the per-token diffusion denoising head: 4-layer FFN-only transformer with AdaLN-Zero modulation, RMSNorm, SwiGLU FFN, zero-init final projection.</summary>
/// <remarks>Hidden dim tracks the LM's hidden size; latent dim equals <see cref="VibeVoiceConfig.AcousticVaeDim"/>.</remarks>
public sealed record VibeVoiceDiffusionHeadConfig
{
    /// <summary>Per-layer width, equal to LM hidden_size — the head reads LM hidden states directly via the cond projection without dimensionality reduction.</summary>
    public int HiddenSize { get; init; } = 1_536;

    /// <summary>Number of <see cref="VibeVoiceDiffusionHead"/> layers; 4 in the published checkpoints across all three variants.</summary>
    public int HeadLayers { get; init; } = 4;

    /// <summary>SwiGLU FFN expansion ratio; 3.0× in the published checkpoints (lower than the typical 4× of DiT blocks).</summary>
    ...
    /// <summary>Latent channel count; mirrors <see cref="VibeVoiceConfig.AcousticVaeDim"/> — denoting "this head denoises acoustic VAE latents". 64.</summary>
    public int LatentSize { get; init; } = 64;

    /// <summary>Convenience copy of <see cref="VibeVoiceConfig.AcousticVaeDim"/>; the Python config stores both — we keep the field to ease round-trip JSON loading.</summary>
    public int? SpeechVaeDim { get; init; } = 64;

    /// <summary>Noise-prediction target; always <c>"v_prediction"</c> for the published VibeVoice checkpoints, <c>"epsilon"</c> is supported by the upstream code but unused.</summary>
    ...
    /// <summary>DDPM steps consumed at inference (DPM-Solver-multistep walks through these); upstream's own inference sets <c>set_ddpm_inference_steps(num_steps=10)</c>, so 10 is the documented default — 5 is reported to degrade quality. Overrideable per request.</summary>
    ...
    /// <summary>Training-only oversample factor for the diffusion loss, unused at inference; kept here so we can round-trip the config JSON without dropping fields.</summary>
    public int DdpmBatchMul { get; init; } = 4;

    /// <summary>Default preset matching the published checkpoints — caller overrides <see cref="HiddenSize"/> per variant.</summary>
