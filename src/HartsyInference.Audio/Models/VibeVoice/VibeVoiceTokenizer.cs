...
/// <summary>Thin tokenizer wrapper for VibeVoice. Qwen2.5 and Qwen3 share the same
/// 151 936-entry byte-level BPE vocab — the existing <see cref="Qwen3Tokenizer"/>'s
/// embedded vocab.json / merges.txt loads cleanly for Qwen2.5 too.
///
...
public sealed class VibeVoiceTokenizer : IDisposable
{
    ...
    /// <summary>Per-frame diffusion sentinel. Re-uses Qwen's <c>&lt;|vision_pad|&gt;</c>
    /// (ID 151 654). The LM emits one of these per 7.5 Hz acoustic frame.</summary>
    ...
    /// <summary>Negative-prompt sentinel used by the streaming-0.5B CFG path
    /// (<c>&lt;|image_pad|&gt;</c>, ID 151 655). Reserved for that variant only.</summary>
    ...
    /// <summary>Encodes a single string into a variable-length token-id array.
    /// Does NOT add BOS / EOS — the multi-speaker processor controls token injection
    /// (it splices in <c>&lt;|vision_*|&gt;</c> sentinels and the final
    /// <c>&lt;|endoftext|&gt;</c> via the prompt template).</summary>
    ...
    /// <summary>Decodes a token-id sequence back to text. Useful for logging the prompt
    /// the LM actually received.</summary>
