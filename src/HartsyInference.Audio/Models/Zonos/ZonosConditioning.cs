...
public sealed unsafe class ZonosConditioning : IDisposable
{
    ...
    /// <summary>Assembles the conditioning prefix and returns it channels-first <c>[1, 2048, P]</c>.</summary>
    public Tensor BuildPrefix(IBackend backend, ReadOnlySpan<int> phonemes, Tensor speaker128, float[] emotion8,
        float fmax, float pitchStd, float speakingRate, int languageId)
    {
        ...
        // 7. Language integer embedding.
        float* le = (float*)_languageEmbed!.DataPointer;
        long lsrc = (long)languageId * DModel;
    ...
    {
        Tensor?[] top = [_phonemeEmbed, _speakerW, _speakerB, _languageEmbed, _projW, _projB, _normW, _normB];
