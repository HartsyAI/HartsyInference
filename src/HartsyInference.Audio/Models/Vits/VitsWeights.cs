...
/// <summary>Weight-loading helpers for VITS modules. Most VITS convolutions are PyTorch <c>weight_norm</c>'d
/// (stored as <c>weight_g</c> + <c>weight_v</c>); this fuses them at load (or takes a pre-fused
/// <c>weight</c>), so model code only ever sees a plain conv weight.</summary>
public static class VitsWeights
{
    /// <summary>Loads conv weight <paramref name="key"/>: prefers a fused <c>{key}.weight</c>, else fuses
    /// <c>{key}.weight_g</c> + <c>{key}.weight_v</c>.</summary>
