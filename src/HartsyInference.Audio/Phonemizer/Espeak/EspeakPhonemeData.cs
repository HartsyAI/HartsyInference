...
/// <summary>Output of running a phoneme's bytecode program (espeak <c>PHONEME_DATA</c>), restricted to the fields the phonemizer needs: the <c>pd_param</c> set-by-program parameters (notably the allophone change/insert/append codes) and the IPA name emitted by <c>i_IPA_NAME</c>; audio-synthesis fields (formants, wave addresses) are ignored.</summary>
