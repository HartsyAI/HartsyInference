...
public static class ActivationKernels
{
    ...
    /// <summary>Leaky ReLU: <c>x if x &gt;= 0 else slope * x</c>.</summary>
    /// <remarks>Used by Kokoro / StyleTTS 2's text encoder (slope=0.2) and several other audio nets where the
    /// non-zero negative gradient prevents dead-neuron failure modes in narrow channel counts.</remarks>
    [MethodImpl(MethodImplOptions.AggressiveOptimization)]
    ...
    /// <summary>Snake activation, with optional snake-beta variant, over channels-first <c>[B, C, T]</c> input.</summary>
    /// <remarks><paramref name="alpha"/> is shape <c>[C]</c> (the leading and trailing singleton dims are
    /// squeezed). When <paramref name="beta"/> is null the divisor is <paramref name="alpha"/>[c] (vanilla snake
    /// from BigVGAN / Stable Audio Oobleck VAE); when supplied the divisor is <paramref name="beta"/>[c] + 1e-8
    /// (snake-beta from BigVGAN-v2).</remarks>
    [MethodImpl(MethodImplOptions.AggressiveOptimization)]
