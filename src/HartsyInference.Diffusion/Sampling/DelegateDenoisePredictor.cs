...
/// halves of the pair at guidance 1.0, which makes the sampler's combine an identity. A family that hands back raw
/// cond/uncond sets the pair's real scale and lets the fused device op do the combine. Getting this backwards
/// double-applies guidance, so the choice is explicit per step rather than inferred.</para></summary>
public sealed class DelegateDenoisePredictor : IDenoisePredictor
{
    ...
    public DelegateDenoisePredictor(PredictionType prediction, Func<Tensor, float, int, DenoisePrediction> predict)
    {
