...
{
    /// <summary>Validates the box is inside <c>[0, 1000]</c> on both axes with <c>min &lt; max</c>. Throws <see cref="HartsyInferenceException"/> otherwise.</summary>
    public void Validate()
    {
        if (YMin < 0 || XMin < 0 || YMax > 1000 || XMax > 1000)
            throw new HartsyInferenceException($"BoundingBox out of [0,1000]: [{YMin},{XMin},{YMax},{XMax}].");
        if (YMin >= YMax || XMin >= XMax)
            throw new HartsyInferenceException($"BoundingBox must have min < max: [{YMin},{XMin},{YMax},{XMax}].");
    }
