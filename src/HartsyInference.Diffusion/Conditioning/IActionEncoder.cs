...
/// <summary>Where a conditioning stream is consumed inside the model — pipelines route each stream produced by an
/// <see cref="IActionEncoder"/> to the matching injection site. See <c>docs/Research/INTERACTIVE_INFERENCE.md</c> § 2.</summary>
...
/// <summary>Translates raw <see cref="ActionInput"/> payloads into the per-frame conditioning streams a specific model
/// consumes. Encoders fill caller-owned buffers (<see cref="Span{T}"/>) — no allocations on the 25–40 ms frame path.
/// Models without action conditioning simply have no encoder. See <c>docs/Research/INTERACTIVE_INFERENCE.md</c> § 2.</summary>
public interface IActionEncoder : IDisposable
{
    ...
    /// <summary>Writes one frame of <paramref name="streamName"/> into <paramref name="destination"/> (length must be
    /// the stream's <see cref="ActionStreamSpec.FloatsPerFrame"/>).</summary>
