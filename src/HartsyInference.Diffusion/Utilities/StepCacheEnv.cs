...
/// <summary>Env-knob parsing for the across-step feature cache (`vram.stepCache`, `vram.stepCacheCap`) —
...
{
    /// <summary>Reads vram.stepCache: unset/0 = off (0f); "1"/"true" = the 0.10 default drift threshold
    ...
    public static float ReadThreshold()
    {
        ...
        if (!float.TryParse(value, System.Globalization.CultureInfo.InvariantCulture, out float threshold) || threshold < 0f)
            throw new ArgumentException($"vram.stepCache must be a non-negative float or 1/true; got '{value}'.");
        return threshold;
    }
    ...
    /// individual env knobs (vram.stepCachePoly / _LATE / _CAP) always override their component,
    ...
    /// <summary>Reads vram.stepCachePoly ("c0,c1,c2,..." — TeaCache-style gate calibration polynomial,
    /// lowest power first) and vram.stepCacheCalib (a CSV path: run uncached and log indicator→residual
    ...
    public static float[]? ReadPoly()
    {
        ...
        for (int i = 0; i < parts.Length; i++)
            if (!float.TryParse(parts[i], System.Globalization.CultureInfo.InvariantCulture, out c[i]))
                throw new ArgumentException($"vram.stepCachePoly malformed at '{parts[i]}'.");
        return c;
    }

    public static string? ReadCalibFile() => EngineKnobs.StepCacheCalib.Value;

    /// <summary>Reads vram.stepCacheLate: fraction of the schedule (measured from the END) where cache
    ...
    public static float ReadLateWindow()
    {
        ...
        if (!float.TryParse(value, System.Globalization.CultureInfo.InvariantCulture, out float late)
            || late < 0f || late > 1f)
            throw new ArgumentException($"vram.stepCacheLate must be a float in [0,1]; got '{value}'.");
        return late;
    }

    /// <summary>Reads vram.stepCacheCap (max consecutive cached steps), default 3.</summary>
    public static int ReadCap()
    {
        ...
        if (!int.TryParse(value, out int cap) || cap < 1)
            throw new ArgumentException($"vram.stepCacheCap must be a positive integer; got '{value}'.");
        return cap;
    }
}
