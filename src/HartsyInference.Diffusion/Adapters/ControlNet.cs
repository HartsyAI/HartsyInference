...
/// <summary>ControlNet adapter for spatial conditioning of UNet-based denoisers (SD1.5, SDXL). The architecture is a near-mirror of the base UNet's encoder half (timestep + ADM embedding, conv_in, down blocks, mid block) with two ControlNet-specific additions: a hint encoder that downsamples the control image to the latent grid, and a tower of 1×1 zero-initialized convolutions that gate every skip-connection output before it leaves the adapter. The forward pass produces residuals — one per UNet skip-connection injection point plus one for the mid block — which the caller (a pipeline) adds onto the base UNet's corresponding tensors before the up blocks consume them. With the zero-init scheme, an untrained / zeroed-out ControlNet contributes nothing and the base model's output is unchanged; a trained ControlNet steers the generation toward the conditioning signal proportional to <c>conditioningScale</c>. Implementation reuses <see cref="DownBlock"/>, <see cref="UNetResNetBlock"/>, <see cref="CrossAttentionBlock"/>, <see cref="TimestepEmbedding"/>, and <see cref="AdditionEmbedding"/> verbatim — the diffusers ControlNet uses the same module shapes as the base UNet, so checkpoint key names line up 1:1 (<c>down_blocks.{i}.resnets.{j}.*</c>, <c>mid_block.resnets.{i}.*</c>, etc.). Only the hint encoder and the zero conv tower are new. <b>Flux ControlNet</b> uses a different architecture (DiT blocks, no UNet down/mid) and lives in the separate <see cref="FluxControlNet"/> adapter. This class covers <see cref="ControlNetBaseModel.Sd15"/> and <see cref="ControlNetBaseModel.Sdxl"/>.</summary>
