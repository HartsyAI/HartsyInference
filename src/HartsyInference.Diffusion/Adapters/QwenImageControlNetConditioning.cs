...
/// <summary>One Qwen-Image DiT ControlNet's contribution to a generation: the loaded adapter, the control
/// image, a strength multiplier, and a step window. The Qwen counterpart of
/// <see cref="FluxControlNetConditioning"/> — pipelines accept a list so users can stack ControlNets.
/// Caller retains ownership of both the adapter and the image.</summary>
public sealed record QwenImageControlNetConditioning
{
    ...
    /// pipeline VAE-encodes and 2×2-packs it once per generation (Qwen DiT ControlNets condition on the packed
