...
/// <summary>The union-specific modules of a xinsir-style union SDXL ControlNet (diffusers <c>ControlNetUnionModel</c>): a learned per-control-type <c>task_embedding</c> table, a small pre-LN transformer (<c>transformer_layes</c> — sic, the checkpoint key carries the typo) that fuses pooled condition features with the pooled latent feature, a zero-initialized <c>spatial_ch_projs</c> Linear that turns the fused condition token into a per-channel bias, and a <c>control_add_embedding</c> MLP that injects a sinusoid-encoded multi-hot control-type vector into the timestep embedding.
///
/// <para>Replaces the plain ControlNet's <c>hidden = conv_in(latent) + cond_hint</c> injection with <c>hidden = conv_in(latent) + cond_hint + alpha</c> where <c>alpha</c> is the transformer-fused channel bias, and adds <c>control_emb</c> onto the timestep embedding before the ADM addition embedding — everything downstream (down blocks, mid block, zero convs) is the standard SDXL ControlNet path.</para>
///
/// <para>The fusion transformer is a faithful port of the reference: diffusers feeds the <c>[B, tokens, C]</c> token stack straight into <c>nn.MultiheadAttention</c> with the default <c>batch_first=False</c>, so attention runs over the diffusion-batch dim independently per token — the tokens NEVER interact (LayerNorm/MLP are per-token too). Two consequences we exploit, both bit-identical to running the full stack (verified against the diffusers oracle at B=1 and B=2): the pooled-latent token the reference appends is dead compute for the single-condition case (its transformed value is discarded — <c>spatial_ch_projs</c> only consumes the condition tokens), and the condition token can be run through the layers alone as a <c>[1, B, C]</c> stack with B as the SDPA sequence dim. This also keeps the forward off <see cref="IBackend.Split"/>, whose CUDA implementation is a host fallback (mid-forward sync trap).</para></summary>
