...
{
    /// <summary>Build the per-cross-attn-layer scale array from a base scale and weight-type string. Returns an array of length <paramref name="layerCount"/> ready to pass into <c>UNet.Forward</c>'s <c>ipaScalePerLayer</c> parameter (after multiplying by any per-step gating factor).
    /// <para>The array is indexed in the IPA CHECKPOINT order (diffusers' <c>attn_processors</c> enumeration): indices <c>[0, downLayers)</c> are the down (encoder) blocks, <c>[downLayers, layerCount - midLayers)</c> the up (decoder) blocks, and the final <c>midLayers</c> entries the mid block — matching how <c>UNet.Forward</c> consumes the K_ip/V_ip lists.</para></summary>
