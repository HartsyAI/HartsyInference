...
/// <summary>Configuration for Ideogram 4 (<c>ideogram-oss/ideogram4</c>, 9.3B, non-commercial license). Verified verbatim against upstream <c>src/ideogram4/modeling_ideogram4.py</c> (<c>Ideogram4Config</c> + <c>Ideogram4Transformer</c>). Ideogram 4 is a **single-stream unified-sequence DiT**: text tokens (Qwen3-VL multi-layer hidden states) and image-latent tokens occupy ONE length-L sequence, kept apart by 3D MRoPE + an <c>image_indicator</c> embedding + (for packed multi-prompt batches) a block-diagonal <c>segment_ids</c> mask. Distinctive vs every other DiT in this codebase: <list type="bullet"> <item>**Scale-only AdaLN (NO shift)** + <c>tanh</c>-gated residuals + **sandwich norms** (a norm before AND after each sublayer). Per-block modulation is <c>Linear(512 → 4*hidden)</c> → <c>chunk(4)</c> = <c>(scale_msa, gate_msa, scale_mlp, gate_mlp)</c>.</item> <item>**3D sectioned MRoPE** <c>(24, 20, 20)</c>, θ=5e6, standard HF <c>rotate_half</c> on <c>cat(freqs, freqs)</c> — see <see cref="DiTBlocks.Ideogram4Mrope"/>.</item> <item>**Fused QKV** <c>Linear(hidden → 3*hidden, bias=False)</c>, per-head QK-RMSNorm, <c>o</c> proj bias=False, SwiGLU <c>w2(silu(w1)·w3)</c> bias=False.</item> <item>Text/image features placed into the unified sequence by **masked addition** (text positions zero <c>x</c>, image positions zero <c>llm_features</c>), not concatenation.</item> <item>Final layer: <c>LayerNorm(no affine)</c> + scale-only AdaLN (<c>Linear(512 → hidden)</c>, double-SiLU input) → <c>Linear(hidden → 128)</c>.</item> </list> Conditioning is Qwen3-VL-8B-Instruct (language tower) with 13 layers tapped — see <see cref="LlamaStyleEncoderConfig.Qwen3_VL_8B"/> and <see cref="QwenActivationLayers"/>. VAE is the Flux.2 KL autoencoder (<c>VaeConfig.Flux2</c>), but latent un-normalization uses fixed constants (<see cref="Ideogram4LatentNorm"/>), NOT the VAE BatchNorm.</summary>
