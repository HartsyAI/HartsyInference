...
{
    /// <summary>Opt-in (off by default): set <c>HARTSY_DIT_PROFILE=1</c> to enable. Adds per-block device
    /// syncs around the attention and MLP sublayers (and a per-step cfgΔ probe) so the pipeline can report
    /// where step time goes — those syncs perturb timing, so keep it off for real runs. Read once at startup.</summary>
