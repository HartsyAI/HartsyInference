...
/// <summary>Opt-in per-forward timing accumulators for the Ideogram 4 block, gated by the <c>diagnostics.ditProfile=true</c> environment variable. When enabled, the block syncs the device around its attention and MLP sublayers and adds the elapsed milliseconds here; the pipeline resets at each step and logs the totals. Off by default (zero overhead) since the syncs would otherwise serialize the host and device.</summary>
public static class Ideogram4Profile
{
    /// <summary>Opt-in (off by default): set <c>diagnostics.ditProfile=true</c> to enable. Adds per-block device syncs around the attention and MLP sublayers (and a per-step cfgΔ probe) so the pipeline can report where step time goes — those syncs perturb timing, so keep it off for real runs. Read once at startup.</summary>
