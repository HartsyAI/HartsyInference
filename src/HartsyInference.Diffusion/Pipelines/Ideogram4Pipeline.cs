...
public sealed unsafe class Ideogram4Pipeline : DiffusionPipelineBase
{
        ...
        RegionalPlan? regionalPlan = null)
    {
        ...
        for (int i = steps - 1; i >= 0; i--)
        {
            ...
            double cfgDelta = Math.Sqrt(sd / nv) / (Math.Sqrt(sn / nv) + 1e-9);
            ...
            string profile = HartsyInference.Diffusion.Models.Denoisers.DiTBlocks.Ideogram4Profile.Enabled
                ...
                : "";
            Logs.Info($"[Ideogram4] step {steps - i}/{steps} t={tVal:F3} gw={gw:F1} {stepSw.ElapsedMilliseconds}ms | VRAM {vram} | D2H syncs {syncs} | cfgΔ {cfgDelta:F3}{profile}");
