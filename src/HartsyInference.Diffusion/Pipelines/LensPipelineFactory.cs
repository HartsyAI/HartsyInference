...
/// <summary>Wires <see cref="LensCheckpointConverter"/> output into a ready-to-run <see cref="LensPipeline"/>. Mirrors the manual wiring that <c>Flux2GenerationTests</c> does for Flux.2: build the <see cref="LensTransformer"/> + <see cref="LensGptOssEncoder"/> + Flux.2 <see cref="VaeDecoder"/>, call each component's <c>LoadWeights</c>, and extract the patchified-latent BatchNorm <c>running_mean</c> / <c>running_var</c> the Lens (and Flux.2) latent space is normalized in. Returns a <see cref="LensPipelineBundle"/> so the caller has a single disposable handle over the pipeline, its components, the BN copies, and any loaders opened here.
...
public static class LensPipelineFactory
{
    ...
    /// <summary>Builds a pipeline from already-converted per-component weights (e.g. from <see cref="LensCheckpointConverter.Convert"/> or <see cref="LensCheckpointConverter.ConvertComfy"/>). The caller retains ownership of the weight tensors inside <paramref name="weights"/>; the returned bundle owns only what this factory allocates (the BN F32 copies + the component shells).</summary>
    /// <param name="withTextEncoder">When <c>false</c>, skips loading the GPT-OSS encoder — the resulting pipeline supports <see cref="LensPipeline.GenerateFromEmbeddings"/> only.</param>
    ...
    /// <summary>Loads + converts the three ComfyUI Lens files (<c>Comfy-Org/Lens</c>: DiT, GPT-OSS text encoder, Flux.2 VAE) and wires them into a pipeline. The bundle keeps the loaders alive — the passthrough VAE tensors are memory-mapped views into <paramref name="vaePath"/>.</summary>
    /// <param name="onTransformerWeights">Invoked with the converted transformer dict after conversion and BEFORE <c>LoadWeights</c> runs, so a caller can mutate it in place. This exists for the Engine's LoRA merge, which must land before the weights reach the model (device caches are identity-keyed, so a post-load merge would leave layers serving the pre-merge tensors) — and which cannot be done here, because the merge machinery lives in the Engine package and this one must not depend on it.</param>
    ...
    /// <summary>Loads + converts a single-file Lens checkpoint (HF diffusers-native combined dump) and wires it into a pipeline. The bundle keeps the loader alive (passthrough tensors are mmap views).</summary>
