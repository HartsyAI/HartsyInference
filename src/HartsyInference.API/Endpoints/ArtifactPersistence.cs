...
/// <summary>Writes generated artifacts to disk for the HTTP routes, matching where the CLI already puts them.
/// A failed write must never fail a generation that already succeeded, so every path logs and returns null
/// rather than throwing — the artifact is still in the response body either way.</summary>
public static class ArtifactPersistence
{
    ...
    /// <summary>Envelope-free overload for the OpenAI-compat routes, whose request schema has no place to carry
    /// <c>save</c>/<c>outputDir</c> — they always save to the output root.</summary>
