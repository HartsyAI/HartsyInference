...
/// <summary>Tracks HTTP-visible interactive-world sessions by a server-assigned id. Genuinely new infrastructure — the CLI never needs this because it holds one <see cref="IWorldSession"/> for the life of a single process invocation; an HTTP server can have many concurrent sessions across many clients and must be able to find one again on a later request (an action POST, a reconnecting stream) and reclaim it if the client vanishes.</summary>
public sealed class WorldSessionRegistry : IDisposable
{
    ...
    /// <summary>Creates a registry that evicts sessions idle for longer than <paramref name="idleTimeout"/>, checked on that same interval.</summary>
    ...
    /// <summary>Looks up a session by id, touching its last-activity time so it doesn't idle-time-out mid-use. Null when the id is unknown or was already evicted/closed.</summary>
    ...
    /// <summary>Stops the sweep timer and disposes every still-open session — called by the DI container at host shutdown since this is registered as a singleton.</summary>
    public void Dispose()
    {
