...
/// <summary>Wraps a VkInstance with deterministic disposal. Headless compute — no surface/swapchain extensions. Validation layer is opt-in via SHARPINFERENCE_VK_VALIDATION env var.</summary>
public sealed class VulkanInstance : IDisposable
{
    ...
    /// <summary>Creates a Vulkan 1.3 instance. Validation layer enabled if SHARPINFERENCE_VK_VALIDATION=1.</summary>
    public VulkanInstance(bool? enableValidation = null)
    {
        VulkanLibraryResolver.Register();

        bool enableVal = enableValidation
            ?? Environment.GetEnvironmentVariable("SHARPINFERENCE_VK_VALIDATION") == "1";
        ...
        _appName = new PinnedUtf8String("SharpInference");
        _engineName = new PinnedUtf8String("SharpInference.Vulkan");
