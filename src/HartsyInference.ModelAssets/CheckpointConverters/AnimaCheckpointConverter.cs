...
/// <summary>Loads + buckets an Anima single-file safetensors checkpoint into transformer, llm_adapter, VAE, and text-encoder dictionaries. The Anima ComfyUI checkpoint stores every weight under a flat <c>net.</c> prefix:
/// <list type="bullet">
/// <item><c>net.x_embedder.*</c>, <c>net.t_embedder.*</c>, <c>net.t_embedding_norm.*</c>, <c>net.blocks.*</c>, <c>net.final_layer.*</c> — main DiT trunk → <see cref="ConvertedWeights.Transformer"/>.</item>
/// <item><c>net.llm_adapter.*</c> — Anima-specific 6-block adapter → <see cref="ConvertedWeights.LlmAdapter"/>.</item>
/// </list>
/// The converter strips the <c>net.</c> prefix from every key and (for the llm_adapter bucket) the <c>llm_adapter.</c> sub-prefix too, so downstream loaders see clean names like <c>blocks.0.self_attn.q_proj.weight</c> and <c>blocks.0.norm_self_attn.weight</c>.
///
/// VAE + text encoder buckets exist for compatibility with full pipeline checkpoints; Anima ships diffusion-only, so those buckets are empty for the canonical <c>anima-preview3-base.safetensors</c>. The converter still recognizes <c>vae.</c> / <c>text_encoder.</c> prefixes in case a future Anima distribution bundles them.
///
/// **Backwards compatibility:** also tolerates diffusers-style key prefixes (<c>model.diffusion_model.</c>, <c>transformer.</c>) in case an earlier Anima dump used them.</summary>
public sealed class AnimaCheckpointConverter
{
    ...
    {
        /// <summary>Main DiT trunk weights (prefix-stripped). Keys look like <c>blocks.0.self_attn.q_proj.weight</c>, <c>x_embedder.proj.1.weight</c>, <c>final_layer.linear.weight</c>.</summary>
        public required Dictionary<string, Tensor> Transformer { get; init; }

        /// <summary>Anima <c>llm_adapter</c> weights (both <c>net.</c> and <c>llm_adapter.</c> prefixes stripped). Keys look like <c>blocks.0.self_attn.q_proj.weight</c>, <c>norm.weight</c>, <c>out_proj.weight</c>, <c>embed.weight</c>.</summary>
