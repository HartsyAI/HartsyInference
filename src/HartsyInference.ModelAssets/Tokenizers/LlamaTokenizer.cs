...
/// <summary>Llama 3 / 3.1 byte-level BPE tokenizer used as a text encoder for HiDream and other diffusion models that condition on Llama-3.1-Instruct hidden states. Wraps <see cref="BpeTokenizer"/> with <c>vocab.json</c> + <c>merges.txt</c> shipped under the <c>meta-llama/Llama-3.1-8B-Instruct</c> tokenizer. Vocab size 128,256; <c>BosTokenId</c> = 128000 (<c>&lt;|begin_of_text|&gt;</c>); <c>EosTokenId</c> = 128001 (<c>&lt;|end_of_text|&gt;</c>); <c>PadTokenId</c> = 128004 (<c>&lt;|finetune_right_pad_id|&gt;</c>). HiDream uses Llama as a feature extractor — encode the raw prompt with BOS prefix, no chat template required.
/// <para>Note: Llama 3 tokenizers ship as a <c>tokenizer.json</c> file, not raw vocab.json+merges.txt. Pass the path to either form; the constructor accepts whichever the user has on disk. The path constructor expects the vocab.json + merges.txt extracted from <c>tokenizer.json</c> (same content, just split into the two-file form). HuggingFace's <c>tokenizers</c> library produces both representations interchangeably.</para></summary>
public sealed class LlamaTokenizer : IDisposable
{
    ...
    /// <summary>Right-padding token id used by Llama-3 fine-tunes (<c>&lt;|finetune_right_pad_id|&gt;</c>). Falls back to <see cref="EosTokenId"/> if a model uses pad=eos.</summary>
    ...
    /// <summary>Creates a Llama 3 tokenizer from the canonical <c>vocab.json</c> + <c>merges.txt</c> embedded in this assembly (same pattern as <see cref="ClipTokenizer"/> / <see cref="Qwen3Tokenizer"/>). Throws if the assets aren't embedded in this build — check <see cref="EmbeddedTokenizerResources.HasLlama3Assets"/> first if you want to fall back gracefully.</summary>
    ...
    /// <summary>Encodes text into a fixed-length token-id array. Prefixes BOS, then BPE-encodes the prompt, then right-pads with <see cref="PadTokenId"/>. Truncates from the right if the prompt overflows.</summary>
    ...
    /// <summary>Builds a <c>[seq]</c> attention mask: 1 for real tokens (including the prepended BOS), 0 for trailing pad. Walks from the right to find the last non-pad position.</summary>
