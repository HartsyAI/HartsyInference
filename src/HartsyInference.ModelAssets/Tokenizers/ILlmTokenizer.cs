...
/// <summary>Abstraction over a decoder-LLM tokenizer so the generation pipeline and chat templates work with any model (not just Qwen). Implemented by <see cref="GgufTokenizer"/> (built from GGUF metadata) and by the per-family tokenizers. Encoding is special-token aware so a rendered chat template's control-token literals (e.g. <c>&lt;|im_start|&gt;</c>, <c>&lt;|begin_of_text|&gt;</c>, <c>[INST]</c>) map to their real ids.</summary>
public interface ILlmTokenizer
{
    /// <summary>Encodes text to ids. When <paramref name="addSpecial"/> is true, registered special/control tokens appearing literally in <paramref name="text"/> are emitted as their ids (the rest is BPE); when false, the whole string is ordinary BPE. Chat templates render to a string then encode with <paramref name="addSpecial"/> = true.</summary>
