...
/// <summary>Gemma SentencePiece tokenizer (256k vocab) — the conditioning-encoder tokenizer for Gemma-family text encoders: LTX-2's Gemma 3 12B and Lumina-2's Gemma 2. Wraps Microsoft.ML.Tokenizers' <see cref="SentencePieceTokenizer"/> over Gemma's <c>tokenizer.model</c> protobuf (bundled with LTX-2 at <c>tokenizer/tokenizer.model</c>, or in the Gemma repos).
///
/// <para>Gemma prepends <c>&lt;bos&gt;</c> (id 2) and does NOT append <c>&lt;eos&gt;</c> for encoder conditioning. Special ids: pad=0, eos=1, bos=2, unk=3. Unlike T5 (bos_id=-1, which needs a protobuf patch), Gemma's bos_id is a real token, so no patch is needed.</para>
///
/// <para>The exact max length / whether the diffusion model right-pads is model-specific (validation-gated against the LTX-2 reference); callers pass <paramref name="maxLength"/> and assemble the final sequence.</para></summary>
public sealed class GemmaTokenizer : IDisposable, ILtx2PromptTokenizer
{
    ...
    /// <summary>Encodes a prompt to Gemma token ids (BOS prepended via the SentencePiece config), truncated to <see cref="DefaultMaxLength"/>/the ctor max. No padding or EOS — the pipeline assembles the conditioning sequence (Gemma-family text encoders condition on BOS + content).</summary>
