...
/// <summary>LLaVA-NeXT/1.6 "anyres" image tiling: picks the best-fit resolution from a fixed set of grid pinpoints, resizes+pads the image into it, and splits it into <c>tileSize×tileSize</c> tiles plus a separately-resized base/overview tile of the whole image.</summary>
/// <remarks>Ports HF's <c>image_processing_llava_next.py</c> (<c>select_best_resolution</c>/<c>get_patch_output_size</c>/<c>_get_padding_size</c>/<c>divide_to_patches</c>) exactly, resolved against the real installed <c>transformers</c> source rather than from memory — its own docstrings disagree with each other on whether image sizes are <c>(height, width)</c> or <c>(width, height)</c> (a known upstream inconsistency); this port follows <c>select_best_resolution</c>'s actual body, which is unambiguously <c>(height, width)</c> throughout, confirmed against <c>tests/python-reference/dump_llavanext_vision_ref.py</c>.
/// <para>Uses this repo's own bilinear resize kernel (matching <see cref="VlmImagePreprocessor"/>'s convention) rather than HF's bicubic — the same simplification already accepted, unvalidated pixel-for-pixel, for LLaVA-1.5's base-image resize. Padding is applied in raw <c>[0,255]</c> pixel space with a black (0) fill BEFORE normalization (matching HF's own pipeline order), so padded regions end up at <c>-mean/std</c> per channel after normalize, not exactly 0.</para></remarks>
public static unsafe class LlavaNextImagePreprocessor
{
    /// <summary>Converts raw interleaved RGB (<c>H*W*3</c> bytes, row-major HWC) into a <c>[1, 3, height, width]</c> tensor of raw <c>[0,255]</c> floats (CHW, un-resized, un-normalized) — the native-resolution input <see cref="LlavaNextEncoder.Encode"/> expects, since the unpad step needs the original aspect ratio.</summary>
    ...
    /// <summary>Selects the grid pinpoint that best fits <paramref name="originalSize"/>: maximizes effective (non-wasted) resolution, tie-broken by minimum wasted area; direct port of HF's <c>image_processing_utils.select_best_resolution</c> — both the input and every candidate are <c>(height, width)</c>, and so is the return value.</summary>
    ...
    /// <summary>Aspect-preserving resize-to-fit dimensions of <paramref name="original"/> inside <paramref name="target"/>; direct port of HF's <c>image_processing_utils.get_patch_output_size</c>.</summary>
    ...
    /// <summary>Full anyres tiling pipeline: base/overview tile (whole image, distorted-aspect resize to <paramref name="tileSize"/>²) followed by the best-fit grid's tiles in raster order, matching HF's <c>_get_image_patches</c>; <paramref name="nativeChw255"/> is the un-resized tensor from <see cref="RawToNativeTensor"/>, and each returned tile is normalized and ready for <see cref="SiglipVlmEncoder.Encode"/>.</summary>
    ...
    /// <summary>Bilinear resize (center-pixel sampling, edge-clamped) — same kernel convention as <see cref="VlmImagePreprocessor"/>, generalized to non-square dimensions.</summary>
    ...
    /// <summary>Centers <paramref name="src"/> (<c>srcH×srcW</c>) inside a <c>dstH×dstW</c> canvas filled with <paramref name="padValue"/>; matches HF's <c>_get_padding_size</c> split (floor half one side, ceil half the other).</summary>
