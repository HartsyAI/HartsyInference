...
public sealed record TransformerConfig
{
    ...
    /// <remarks>Default is taken from the <c>vram.lowvramQuant</c> env var (set "1" to enable) so it can be flipped for A/B benchmarking without recompiling (see docs/Checklists/QUANT_GEMM_PERF_PLAN.md); an explicit <c>init</c> value always overrides the env default.</remarks>
    public bool LowVramQuant { get; init; } = EngineKnobs.LowvramQuant.Value;
