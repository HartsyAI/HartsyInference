...
///
/// <para>For SDXL Plus the canonical config is <c>depth=4, num_queries=16, dim=1024, dim_head=64, heads=12 (inner_dim=768), embedding_dim=1280, output_dim=2048, ff_mult=4</c>. The <c>1024 ≠ 768</c> mismatch isn't a bug — Q/K/V are projected from the <c>dim=1024</c> latent space down to a smaller <c>inner_dim=768</c> attention head space, then projected back to <c>1024</c> via <c>to_out</c>. The FFN expands <c>1024 → 4096 → 1024</c> as usual.</para>
///
