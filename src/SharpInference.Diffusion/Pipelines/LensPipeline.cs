...
/// <summary>Microsoft Lens text-to-image pipeline (`microsoft/Lens` RL-tuned, `microsoft/Lens-Turbo` 4-step distilled, `microsoft/Lens-Base` SFT-only). Like <see cref="Kandinsky5Pipeline"/> this pipeline only handles the diffusion side — the GPT-OSS MoE text encoder is not yet implemented as a SharpInference text encoder. Callers must pre-compute the four multi-layer hidden-state captures (at layers 5/11/17/23 of GPT-OSS, each <c>[B, S_txt, 2880]</c>) plus their negative-prompt counterparts; this matches diffusers' <c>LensPipeline.__call__(prompt_embeds=, negative_prompt_embeds=, ...)</c> escape hatch. Once a GPT-OSS MoE encoder lands in SharpInference the pipeline can grow a tokenizer-fed overload without touching the denoising path.
///
...
public sealed unsafe class LensPipeline : DiffusionPipelineBase
{
    ...
    /// <see cref="SharpInference.Tokenizers.GptOssTokenizer.BuildChatInputs"/>).</param>
