...
namespace SharpInference.Diffusion.Quality;
...
public sealed record QualityProfile
{
    ...
    public void Validate()
    {
        if (VaeDType.IsFp8 || VaeDType.IsQuantized)
            throw new SharpInferenceException($"VAE dtype {VaeDType} is not supported. VAE must run at F16/BF16/F32 — FP8/Q8/Q4 VAE produces visible artifacts on smooth gradients.");
        if (!BackboneDType.IsFloatingPoint && !BackboneDType.IsQuantized)
            throw new SharpInferenceException($"Backbone dtype must be a floating-point or quantized format; got {BackboneDType}.");
        if (!TextEncoderDType.IsFloatingPoint && !TextEncoderDType.IsQuantized)
            throw new SharpInferenceException($"Text encoder dtype must be a floating-point or quantized format; got {TextEncoderDType}.");
    }
}
