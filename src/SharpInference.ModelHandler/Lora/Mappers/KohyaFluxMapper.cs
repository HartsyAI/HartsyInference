    ...
    private static unsafe void AddFusedSplit(SafeTensorsLoader loader, string sourceKey, LoraRole role, string[] canonicalKeys, Dictionary<(LoraTarget, string), GroupBuffer> groups, int splitWays)
    {
        ...
        if (role == LoraRole.Up)
        {
            if (source.Shape.Rank != 2)
                throw new SharpInferenceException($"Kohya Flux fused up matrix '{sourceKey}' must be 2D, got rank {source.Shape.Rank}.");
            long totalOut = source.Shape[0];
            long rank = source.Shape[1];
            if (totalOut % splitWays != 0 && splitWays == 3)
                throw new SharpInferenceException($"Kohya Flux fused QKV up matrix '{sourceKey}' shape[0]={totalOut} not divisible by 3.");
            long sliceOut;
            long[] sliceOuts;
            if (splitWays == 3)
            {
            ...
            else if (splitWays == 4)
            {
                ...
                // total = 3H + 4H = 7H
                if (totalOut % 7 != 0)
                    throw new SharpInferenceException($"Kohya Flux fused linear1 up matrix '{sourceKey}' shape[0]={totalOut} not divisible by 7 (expected hidden + 4*hidden Q/K/V/mlp split).");
                long h = totalOut / 7;
                sliceOuts = [h, h, h, 4 * h];
            }
