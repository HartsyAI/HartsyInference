...
public sealed class GgufLoader : IDisposable
{
    ...
    public unsafe void Load(string filePath)
    {
        ...
        // Magic
        if (remaining < 4)
            throw new SharpInference.Core.Exceptions.SharpInferenceException("GGUF file too small for magic number.");
        uint magic = *(uint*)(ptr + offset);
        if (magic != GgufMagic)
            throw new SharpInference.Core.Exceptions.SharpInferenceException($"Invalid GGUF magic: 0x{magic:X8}, expected 0x{GgufMagic:X8}.");
        offset += 4;
        ...
        if (version < 2 || version > SupportedVersion)
            throw new SharpInference.Core.Exceptions.UnsupportedModelException($"Unsupported GGUF version: {version}.", null, "GGUF");
        Version = (int)version;
    ...
    private static unsafe (object Value, long BytesRead) ReadMetadataValue(byte* ptr, long remaining, uint valueType)
    {
        ...
        switch (valueType)
        {
            ...
            default:
                throw new SharpInference.Core.Exceptions.SharpInferenceException($"Unknown GGUF metadata value type: {valueType}.");
        }
    }
    ...
    {
        if (remaining < 8)
            throw new SharpInference.Core.Exceptions.SharpInferenceException("GGUF string: not enough bytes for length prefix.");

        ulong length = *(ulong*)ptr;
        if ((long)length > remaining - 8)
            throw new SharpInference.Core.Exceptions.SharpInferenceException($"GGUF string: length {length} exceeds remaining bytes {remaining - 8}.");
...
public sealed class GgufTensorDescriptor
{
    ...
    /// <summary>SharpInference data type (mapped from GGUF type ID).</summary>
