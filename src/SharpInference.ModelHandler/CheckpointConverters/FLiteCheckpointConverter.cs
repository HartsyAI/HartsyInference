...
public sealed class FLiteCheckpointConverter
{
    ...
    public static (ConvertedWeights weights, LoaderHandle handle) LoadAndConvert(string folderPath)
    {
        if (!Directory.Exists(folderPath))
            throw new SharpInference.Core.Exceptions.SharpInferenceException($"F-Lite folder does not exist: {folderPath}");
    ...
    private static Dictionary<string, Tensor> LoadComponent(string root, string component, List<SafeTensorsLoader> loaders)
    {
        ...
        if (!Directory.Exists(componentDir))
            throw new SharpInference.Core.Exceptions.SharpInferenceException($"F-Lite component directory missing: {componentDir}");

        string[] shards = Directory.GetFiles(componentDir, "*.safetensors");
        if (shards.Length == 0)
            throw new SharpInference.Core.Exceptions.SharpInferenceException($"No safetensors shards in {componentDir}");
        ...
        foreach (string shard in shards)
        {
            ...
            foreach (KeyValuePair<string, Tensor> kvp in loader.GetAllTensors())
            {
                if (merged.ContainsKey(kvp.Key))
                    throw new SharpInference.Core.Exceptions.SharpInferenceException($"Duplicate key '{kvp.Key}' across shards in {componentDir}");
