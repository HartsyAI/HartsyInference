...
/// <summary>SDXL GGUF mapper. SDXL GGUFs (city96 / unsloth) ship with the LDM/CompVis naming that <see cref="HartsyInference.ModelHandler.CheckpointConverters.SdxlCheckpointConverter.Convert"/> already handles — keys like <c>model.diffusion_model.input_blocks.X.Y.weight</c>, <c>conditioner.embedders.0.transformer.text_model.*</c>, <c>first_stage_model.*</c>. Pass-through for now; add rewrites here if a GGUF builder uses an alternate prefix.</summary>
