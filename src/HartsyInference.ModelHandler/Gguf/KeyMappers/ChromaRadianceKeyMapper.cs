...
/// <summary>Chroma Radiance GGUF mapper. Radiance is the Chroma family (BFL naming with <c>distilled_guidance_layer.*</c>)
/// plus the pixel-space IO ends: <c>img_in_patch.*</c> conv patchify and the <c>nerf_blocks.*</c> NeRF head. Must be
/// probed BEFORE <see cref="ChromaKeyMapper"/> in heuristic detection — every Radiance checkpoint also matches the
/// classic-Chroma key signature. <see cref="HartsyInference.ModelHandler.CheckpointConverters.ChromaCheckpointConverter.Convert"/>
/// passes the Radiance-only keys through verbatim.</summary>
