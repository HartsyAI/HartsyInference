...
/// <summary>Converts single-file SD1.5 checkpoints (LDM/CompVis format) to diffusers-format weight dictionaries that can be loaded by HartsyInference model components.</summary>
public sealed class Sd15CheckpointConverter
{
    ...
    /// <summary>Converts one LDM UNet key (after stripping the <c>model.diffusion_model.</c> prefix) to its diffusers name, or null when the key has no diffusers counterpart. Also used by <see cref="ControlNetCheckpointConverter"/> for the encoder half of SD1.5 ControlNets.</summary>
