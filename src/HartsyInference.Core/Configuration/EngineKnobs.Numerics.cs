...
public static partial class EngineKnobs
{
    ...
    public static readonly Knob<bool> Bf16Gemv =
        Bool("numerics.bf16Gemv", true, KnobScope.Runtime, KnobDomain.Numerics, "Uses the fused BF16/F16 decode GEMV kernel for small-m F32-activation matmuls instead of cuBLAS GemmEx.");
    ...
    public static readonly Knob<bool> ChromaFusedQkv =
        Bool("numerics.chromaFusedQkv", false, KnobScope.Construction, KnobDomain.Numerics, "Keeps Chroma's fused BFL qkv weights at conversion so blocks run one qkv GEMM plus a fused split-norm.");
    ...
    public static readonly Knob<bool> ConvrotWide =
        Bool("numerics.convrotWide", false, KnobScope.Runtime, KnobDomain.Numerics, "Selects the f16-staged wide fused ConvRot+quant kernel over the bit-identical rotate-then-quant pair.");
    ...
    public static readonly Knob<bool> ConvCudnn =
        Bool("numerics.convCudnn", true, KnobScope.Runtime, KnobDomain.Numerics, "Kill-switch for cuDNN convolution forward; 0 routes every 2D conv back through im2col+cuBLAS.");
    ...
    public static readonly Knob<bool> CsmCfgBatch =
        Bool("numerics.csmCfgBatch", true, KnobScope.Runtime, KnobDomain.Numerics, "Runs CSM's CFG cond+uncond as one B=2 batched backbone step; 0 restores the two separate stream steps.");
    ...
    public static readonly Knob<bool> CsmCfgGraph =
        Bool("numerics.csmCfgGraph", true, KnobScope.Runtime, KnobDomain.Numerics, "Replays the CSM batched B=2 CFG backbone step as one captured CUDA graph instead of the eager batched step.");
    ...
    public static readonly Knob<bool> DitGraph =
        Bool("numerics.ditGraph", false, KnobScope.Runtime, KnobDomain.Numerics, "CUDA-graph capture of a DiT denoise step: 0 kills both tiers, 1 forces both, unset keeps per-arch defaults.");
    ...
    public static readonly Knob<bool> Dp4aOn =
        Bool("numerics.dp4aOn", true, KnobScope.Runtime, KnobDomain.Numerics, "Kill-switch for the dp4a int8-activation decode GEMV kernels over Q4_K/Q6_K/Q8_0 LLM weights.");
    ...
    public static readonly Knob<bool> EpilogueFusion =
        Bool("numerics.epilogueFusion", true, KnobScope.Runtime, KnobDomain.Numerics, "Uses cuBLASLt's fused bias epilogue for biased Linear instead of a separate BiasAdd kernel; 0 disables.");
    ...
    public static readonly Knob<bool> F5Graph =
        Bool("numerics.f5Graph", false, KnobScope.Runtime, KnobDomain.Numerics, "Enables the experimental F5-TTS DiT CUDA-graph capture; replay currently throws ILLEGAL_ADDRESS, keep off.");
    ...
    public static readonly Knob<bool> Fp8StaticInputScale =
        Bool("numerics.fp8StaticInputScale", true, KnobScope.Runtime, KnobDomain.Numerics, "Quantizes fp8 activations with the checkpoint's .input_scale rather than a per-call absmax; changes numerics.");
    ...
    public static readonly Knob<bool> FusedFfn =
        Bool("numerics.fusedFfn", false, KnobScope.Construction, KnobDomain.Numerics, "At Ideogram-4 checkpoint conversion, fuses each block's SwiGLU w1/w3 into one w13 weight (fp8 pairs requantized).");
    ...
    public static readonly Knob<bool> GraphDecode =
        Bool("numerics.graphDecode", false, KnobScope.Runtime, KnobDomain.Numerics, "Opts LLM decode into the captured CUDA-graph step (greedy, non-JSON, dense GQA only) over the eager loop.");
    ...
    public static readonly Knob<bool> GroupedLinear =
        Bool("numerics.groupedLinear", true, KnobScope.Runtime, KnobDomain.Numerics, "Kill-switch for grouped resident-int8 Linears sharing one activation rotate+quant pass across projections.");
    ...
    public static readonly Knob<bool> Int8FusedMma =
        Bool("numerics.int8FusedMma", true, KnobScope.Runtime, KnobDomain.Numerics, "Kill-switch for the fused int8 GEMM+dequant mma kernel on the shapes it is narrowed to (m >= 1024).");
    ...
    public static readonly Knob<bool> Ltx25Na3dTiled =
        Bool("numerics.ltx25Na3dTiled", true, KnobScope.Runtime, KnobDomain.Numerics, "Uses the query-tiled LTX-2.5 na3d kernel (online softmax over the tile's union window) over the per-query one.");
    ...
    public static readonly Knob<bool> Ltx2DiffusionVae =
        Bool("numerics.ltx2DiffusionVae", false, KnobScope.Construction, KnobDomain.Numerics, "Builds LTX-2.5's diffusion video decoder instead of the ~40x faster conv decoder when the checkpoint has both.");
    ...
    public static readonly Knob<bool> Ltx2Gatefuse =
        Bool("numerics.ltx2Gatefuse", true, KnobScope.Runtime, KnobDomain.Numerics, "Kill switch for folding LTX-2's per-head gate into the int8 activation rotate+quant pass (bit-identical).");
    ...
    public static readonly Knob<bool> Ltx2Ropef16 =
        Bool("numerics.ltx2Ropef16", false, KnobScope.Runtime, KnobDomain.Numerics, "Stores the LTX-2 rope cos/sin tables as F16 instead of F32; faster fused QK kernel, SSIM ~0.9956 change.");
    ...
    public static readonly Knob<bool> Ltx2SageTokenmajor =
        Bool("numerics.ltx2SageTokenmajor", false, KnobScope.Runtime, KnobDomain.Numerics, "Lets a token-major SDPA call permute into head-major so it can take the SageAttention kernel.");
    ...
    public static readonly Knob<bool> Ltx2Tokenmajor =
        Bool("numerics.ltx2Tokenmajor", true, KnobScope.Runtime, KnobDomain.Numerics, "Kill switch for LTX-2's token-major attention route, falling back to the head-major layout.");
    ...
    public static readonly Knob<bool> Mm3CfgBatch =
        Bool("numerics.mm3CfgBatch", true, KnobScope.Runtime, KnobDomain.Numerics, "Runs MiniMax-Music3's AR CFG cond+uncond rows as one batched step; 0 restores separate passes.");
    ...
    public static readonly Knob<bool> Mm3DepthQuant =
        Bool("numerics.mm3DepthQuant", true, KnobScope.Construction, KnobDomain.Numerics, "Also quantizes the MiniMax-Music3 depth decoder; off leaves it at checkpoint precision.");
    ...
    public static readonly Knob<bool> ModulateEmitFp8 =
        Bool("numerics.modulateEmitFp8", true, KnobScope.Runtime, KnobDomain.Numerics, "Kill-switch for the modulate/affine-broadcast kernel emitting fp8 e4m3 activations straight into the next GEMM.");
    ...
    public static readonly Knob<string?> MusicgenGraphOff =
        Str("numerics.musicgenGraphOff", null, KnobScope.Runtime, KnobDomain.Numerics, "Presence-only: setting it to ANY value (even 0) disables CUDA-graph decode in the MusicGen AR loop.");
    ...
    public static readonly Knob<bool> QknormScatter =
        Bool("numerics.qknormScatter", true, KnobScope.Runtime, KnobDomain.Numerics, "Kill-switch for the fused per-head QK-norm + rope + KV-scatter decode epilogue (Qwen3/Gemma-3 shapes).");
    ...
    public static readonly Knob<bool> QkFusion =
        Bool("numerics.qkFusion", true, KnobScope.Construction, KnobDomain.Numerics, "0 disables the partial Q+K weight concat used when V's dtype differs; the all-match QKV fuse is unaffected.");
    ...
    public static readonly Knob<bool> QkScatter =
        Bool("numerics.qkScatter", true, KnobScope.Runtime, KnobDomain.Numerics, "Kill switch for the fused [q|k] GEMV plus rope-scatter-V decode kernel; off restores the composed ops.");
    ...
    public static readonly Knob<bool> QuantAtProducer =
        Bool("numerics.quantAtProducer", true, KnobScope.Runtime, KnobDomain.Numerics, "Kill-switch for norm kernels emitting a Q8_1 sidecar so the dp4a GEMV skips a separate quantize pass.");
    ...
    public static readonly Knob<bool> RadianceFp8 =
        Bool("numerics.radianceFp8", true, KnobScope.Construction, KnobDomain.Numerics, "Requantizes Chroma Radiance DiT blocks to fp8 during checkpoint conversion; 0 keeps the wide BF16 weights.");
    ...
    public static readonly Knob<bool> RopeV2 =
        Bool("numerics.ropeV2", true, KnobScope.Runtime, KnobDomain.Numerics, "Uses the division-free head-major RoPE kernel instead of the older one; bit-identical.");
    ...
    public static readonly Knob<bool> S2vTextCfg =
        Bool("numerics.s2vTextCfg", false, KnobScope.Runtime, KnobDomain.Numerics, "Feeds the conditional audio to Wan-S2V's uncond branch so CFG steers text adherence only; changes output.");
    ...
    public static readonly Knob<bool> SageAttn =
        Bool("numerics.sageAttn", true, KnobScope.Runtime, KnobDomain.Numerics, "Selects the INT8 SageAttention kernel for attention; =0 disables it (any non-0 value leaves it on).");
    ...
    public static readonly Knob<string?> SagePv =
        Str("numerics.sagePv", null, KnobScope.Runtime, KnobDomain.Numerics, "Literal f16acc picks SageAttention v1's F16-accumulate PV variant; any other value silently means off.");
    ...
    public static readonly Knob<bool> SageV0 =
        Bool("numerics.sageV0", false, KnobScope.Runtime, KnobDomain.Numerics, "Forces the older v0 SageAttention flash kernel instead of the register-resident v1 path.");
    ...
    public static readonly Knob<bool> Seedvr2VaeF32 =
        Bool("numerics.seedvr2VaeF32", false, KnobScope.Construction, KnobDomain.Numerics, "Builds the SeedVR2 VAE with F32 activations instead of the default CUDA BF16 (which halves the peak).");
    ...
    public static readonly Knob<bool> SpecDecode =
        Bool("numerics.specDecode", false, KnobScope.Runtime, KnobDomain.Numerics, "Default for prompt-lookup speculative decoding when the request omits it; greedy non-JSON requests only.");
    ...
    public static readonly Knob<bool> SsmDeltaV2 =
        Bool("numerics.ssmDeltaV2", true, KnobScope.Runtime, KnobDomain.Numerics, "Uses the row-parallel SSM delta-rule recurrence kernel; 0 falls back to the legacy block-per-row kernel.");
    ...
    public static readonly Knob<bool> SsmDeltaWarprow =
        Bool("numerics.ssmDeltaWarprow", true, KnobScope.Runtime, KnobDomain.Numerics, "Uses the row-parallel warp-per-row SSM delta-rule step kernel over the legacy block-per-head one.");
    ...
    public static readonly Knob<bool> SsmDeviceStep =
        Bool("numerics.ssmDeviceStep", true, KnobScope.Runtime, KnobDomain.Numerics, "Kill-switch for Qwen3.5's on-device single-token step in both the attention and gated-DeltaNet layers.");
    ...
    public static readonly Knob<bool> SsmGraph =
        Bool("numerics.ssmGraph", true, KnobScope.Runtime, KnobDomain.Numerics, "0 disables CUDA-graph capture/replay of the SSM (Qwen3.5-class) greedy decode step.");
    ...
    public static readonly Knob<bool> VaeF32 =
        Bool("numerics.vaeF32", false, KnobScope.Construction, KnobDomain.Numerics, "Casts VAE weights to F32 at recipe load instead of the default BF16 on BF16-capable backends.");
    ...
    public static readonly Knob<bool> W8a8 =
        Bool("numerics.w8a8", false, KnobScope.Runtime, KnobDomain.Numerics, "Runs large-M float Linears as per-channel-int8 weight x per-row-int8 activation IMMA GEMM; lossy.");
