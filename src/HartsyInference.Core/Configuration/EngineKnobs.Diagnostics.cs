...
public static partial class EngineKnobs
{
    ...
    public static readonly Knob<bool> ThreeDPhase =
        Bool("diagnostics.threeDPhase", "HARTSY_3D_PHASE", false, BoolGrammar.Exact, KnobScope.Runtime, KnobDomain.Diagnostics, "Prints per-stage wall-clock timings (with a device sync) for the Hunyuan3D and TripoSR mesh pipelines.");
    ...
    public static readonly Knob<bool> AssertAmbient =
        Bool("diagnostics.assertAmbient", "HARTSY_ASSERT_AMBIENT", false, BoolGrammar.Exact, KnobScope.Runtime, KnobDomain.Diagnostics, "Throws instead of falling back when a CUDA helper resolves state ambient-lessly with 2+ backends live.");
    ...
    public static readonly Knob<bool> ChromaF16trace =
        Bool("diagnostics.chromaF16trace", "HARTSY_CHROMA_F16TRACE", false, BoolGrammar.Exact, KnobScope.Runtime, KnobDomain.Diagnostics, "Logs min/max/NaN/Inf of Chroma F16 block intermediates (D2H-draining) to locate F16 overflow sites.");
    ...
    public static readonly Knob<bool> DitProfile =
        Bool("diagnostics.ditProfile", "HARTSY_DIT_PROFILE", false, BoolGrammar.Exact, KnobScope.Runtime, KnobDomain.Diagnostics, "Adds per-block device syncs around the Ideogram 4 attention/MLP sublayers and accumulates their timings.");
    ...
    public static readonly Knob<bool> F5Profile =
        Bool("diagnostics.f5Profile", "HARTSY_F5_PROFILE", false, BoolGrammar.Exact, KnobScope.Runtime, KnobDomain.Diagnostics, "Enables sync-bracketed attn/qkv/ffn timers in F5-TTS DiT blocks plus a per-generation sample-loop timing line.");
    ...
    public static readonly Knob<bool> FliteProbe =
        Bool("diagnostics.fliteProbe", "HARTSY_FLITE_PROBE", false, BoolGrammar.Exact, KnobScope.Runtime, KnobDomain.Diagnostics, "Logs per-stage absmax of the F-Lite transformer on the first forward, host-syncing at each probe.");
    ...
    public static readonly Knob<bool> FluxStats =
        Bool("diagnostics.fluxStats", "HARTSY_FLUX_STATS", false, BoolGrammar.Exact, KnobScope.Runtime, KnobDomain.Diagnostics, "Re-enables Flux's per-tensor min/max/mean/NaN host scans, each a forced D2H sync in the denoise loop.");
    ...
    public static readonly Knob<bool> GraphDump =
        Bool("diagnostics.graphDump", "HARTSY_GRAPH_DUMP", false, BoolGrammar.Exact, KnobScope.Runtime, KnobDomain.Diagnostics, "Logs the captured CUDA graph's node-type histogram and any H2D cache miss that happens inside capture.");
    ...
    public static readonly Knob<bool> GraphGateLog =
        Bool("diagnostics.graphGateLog", "HARTSY_GRAPH_GATE_LOG", false, BoolGrammar.Exact, KnobScope.Runtime, KnobDomain.Diagnostics, "Logs which architecture predicate disqualified a model from CUDA-graph decode capture.");
    ...
    public static readonly Knob<bool> H2dTrace =
        Bool("diagnostics.h2dTrace", "HARTSY_H2D_TRACE", false, BoolGrammar.Exact, KnobScope.Runtime, KnobDomain.Diagnostics, "Logs the first host-to-device weight-cache misses with shape, dtype and byte count.");
    ...
    public static readonly Knob<bool> H3Probe =
        Bool("diagnostics.h3Probe", "HARTSY_H3_PROBE", false, BoolGrammar.Exact, KnobScope.Runtime, KnobDomain.Diagnostics, "Logs min/max/mean/rms of MiniMax-H3 tensors at each conditioning and denoise stage.");
    ...
    public static readonly Knob<bool> H3Vprobe =
        Bool("diagnostics.h3Vprobe", "HARTSY_H3_VPROBE", false, BoolGrammar.Exact, KnobScope.Runtime, KnobDomain.Diagnostics, "Reports max|V| per MiniMax-H3 block against F16's 65504 ceiling via a host-side synchronizing scan.");
    ...
    public static readonly Knob<bool> HidreamProbe =
        Bool("diagnostics.hidreamProbe", "HARTSY_HIDREAM_PROBE", false, BoolGrammar.Exact, KnobScope.Runtime, KnobDomain.Diagnostics, "Logs first-forward absmax/rms of HiDream residual streams (host sync) to judge F16-activation viability.");
    ...
    public static readonly Knob<bool> Ltx2Probe =
        Bool("diagnostics.ltx2Probe", "HARTSY_LTX2_PROBE", false, BoolGrammar.Exact, KnobScope.Runtime, KnobDomain.Diagnostics, "Logs absmax/rms/NaN stats for LTX-2 transformer blocks, VAE decode stages and pipeline stage outputs.");
    ...
    public static readonly Knob<bool> NoImage =
        Bool("diagnostics.noImage", "HARTSY_NO_IMAGE", false, BoolGrammar.Exact, KnobScope.Runtime, KnobDomain.Diagnostics, "Suppresses the CLI's inline half-block terminal image preview.");
    ...
    public static readonly Knob<bool> OasisPhase =
        Bool("diagnostics.oasisPhase", "HARTSY_OASIS_PHASE", false, BoolGrammar.Exact, KnobScope.Runtime, KnobDomain.Diagnostics, "Sync-bracketed per-phase GPU timers (SDPA, attention rest, MLP, mod/norm) for the Oasis DiT eager path.");
    ...
    public static readonly Knob<bool> OrpheusProf =
        Bool("diagnostics.orpheusProf", "HARTSY_ORPHEUS_PROF", false, BoolGrammar.Exact, KnobScope.Runtime, KnobDomain.Diagnostics, "Times the Orpheus TTS decode loop's logits/sample/forward phases with per-step device syncs.");
    ...
    public static readonly Knob<bool> Profile =
        Bool("diagnostics.profile", "HARTSY_PROFILE", false, BoolGrammar.Exact, KnobScope.Runtime, KnobDomain.Diagnostics, "Enables the per-label CPU wall-time accumulator over every NvtxRange-wrapped op.");
    ...
    public static readonly Knob<bool> ProfileEach =
        Bool("diagnostics.profileEach", "HARTSY_PROFILE_EACH", false, BoolGrammar.Exact, KnobScope.Runtime, KnobDomain.Diagnostics, "Dumps the accumulated per-op profile table at every backend Sync (per generation) to HARTSY_PROFILE_OUT.");
    ...
    public static readonly Knob<bool> ProfileFine =
        Bool("diagnostics.profileFine", "HARTSY_PROFILE_FINE", false, BoolGrammar.Exact, KnobScope.Runtime, KnobDomain.Diagnostics, "Enables the sub-op NVTX/wall-time ranges (PushFine) nested inside already-profiled ops.");
    ...
    public static readonly Knob<bool> ProfileShapes =
        Bool("diagnostics.profileShapes", "HARTSY_PROFILE_SHAPES", false, BoolGrammar.Exact, KnobScope.Runtime, KnobDomain.Diagnostics, "Splits selected NVTX/profiler op labels by tensor shape so one label's total breaks down by call regime.");
    ...
    public static readonly Knob<bool> ProfileSync =
        Bool("diagnostics.profileSync", "HARTSY_PROFILE_SYNC", false, BoolGrammar.Exact, KnobScope.Runtime, KnobDomain.Diagnostics, "Syncs the compute stream at each NVTX range Dispose so per-op timing is GPU time; serializes the pipeline.");
    ...
    public static readonly Knob<bool> SdxlDebug =
        Bool("diagnostics.sdxlDebug", "HARTSY_SDXL_DEBUG", false, BoolGrammar.Exact, KnobScope.Runtime, KnobDomain.Diagnostics, "Logs inpaint mask-blend agreement stats per step in the SDXL pipeline (host scan of the latent).");
    ...
    public static readonly Knob<bool> TeProbe =
        Bool("diagnostics.teProbe", "HARTSY_TE_PROBE", false, BoolGrammar.Exact, KnobScope.Runtime, KnobDomain.Diagnostics, "Logs per-layer absmax of the Llama-style text encoder's hidden stream, forcing a host sync per layer.");
    ...
    public static readonly Knob<bool> VaeStats =
        Bool("diagnostics.vaeStats", "HARTSY_VAE_STATS", false, BoolGrammar.Exact, KnobScope.Runtime, KnobDomain.Diagnostics, "Enables per-tile/per-image min/max/mean host scans in the VAE decoder (black-tile bring-up instrumentation).");
    ...
    public static readonly Knob<bool> VisionProbe =
        Bool("diagnostics.visionProbe", "HARTSY_VISION_PROBE", false, BoolGrammar.Exact, KnobScope.Runtime, KnobDomain.Diagnostics, "Logs per-stage min/max/mean/NaN stats through the NormalBAE and UperNet segmentation forwards.");
    ...
    public static readonly Knob<bool> VlmDebug =
        Bool("diagnostics.vlmDebug", "HARTSY_VLM_DEBUG", false, BoolGrammar.Exact, KnobScope.Runtime, KnobDomain.Diagnostics, "Prints mean/maxabs stats for VLM vision-encoder stages and the spliced image embeddings to stderr.");
    ...
    public static readonly Knob<bool> WanDebug =
        Bool("diagnostics.wanDebug", "HARTSY_WAN_DEBUG", false, BoolGrammar.Exact, KnobScope.Runtime, KnobDomain.Diagnostics, "Logs per-step velocity/latent stats and free-VRAM for the Wan video and Wan-S2V pipelines.");
    ...
    public static readonly Knob<bool> YueProfile =
        Bool("diagnostics.yueProfile", "HARTSY_YUE_PROFILE", false, BoolGrammar.Exact, KnobScope.Runtime, KnobDomain.Diagnostics, "Accumulates proj+D2H / host-argmax / feed-queue timings across YuE stage-2 residual decode frames.");
    ...
    public static readonly Knob<bool> ZimageF16trace =
        Bool("diagnostics.zimageF16trace", "HARTSY_ZIMAGE_F16TRACE", false, BoolGrammar.Exact, KnobScope.Runtime, KnobDomain.Diagnostics, "Logs min/max/NaN of every Z-Image block intermediate (first ~300 forwards) to locate F16 overflow.");
    ...
    public static readonly Knob<bool> ZimagePredStats =
        Bool("diagnostics.zimagePredStats", "HARTSY_ZIMAGE_PRED_STATS", false, BoolGrammar.TriState, KnobScope.Runtime, KnobDomain.Diagnostics, "Enables Z-Image's host-side prediction scans before the CFG combine to localize non-finite Base outputs.");
    ...
    public static readonly Knob<bool> ZimageStats =
        Bool("diagnostics.zimageStats", "HARTSY_ZIMAGE_STATS", false, BoolGrammar.Exact, KnobScope.Runtime, KnobDomain.Diagnostics, "Enables per-channel latent/VAE min/max/mean host scans in the Z-Image pipeline.");
    ...
    public static readonly Knob<bool> ZipvoiceDebug =
        Bool("diagnostics.zipvoiceDebug", "HARTSY_ZIPVOICE_DEBUG", false, BoolGrammar.Exact, KnobScope.Runtime, KnobDomain.Diagnostics, "Logs ZipVoice frame/token alignment counts and the phonemized reference text before decoding.");
