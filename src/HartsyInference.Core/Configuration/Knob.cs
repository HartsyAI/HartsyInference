...
public sealed class Knob<T>
{
    ...
    /// <remarks>Load-bearing: the historic call sites did two different things with an out-of-range value and
    /// both survive here. Some rejected it (<c>gb &gt; 0 ? gb : 14</c>); others clamped it
    /// (<c>Math.Clamp(w, 1, 16)</c>), where <c>17</c> means 16 and emphatically not the default 4.</remarks>
