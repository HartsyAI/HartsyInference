...
namespace HartsyInference.Core.Schedulers;

/// <summary>Diffusion noise scheduler that determines noise levels at each step and computes the denoising update rule.</summary>
public interface IScheduler
{
    ...
    /// <summary>Initial noise scale factor: 1.0 for most schedulers, but scaled for some (e.g. Karras).</summary>
    ...
    /// <summary>Performs one denoising step: given the model output and current sample, returns the denoised sample for the next step.</summary>
    ...
    /// <summary>Adds noise to a clean sample at the given timestep, for img2img (starting from a partially noisy image).</summary>
    ...
    /// <summary>Scale factor for the model input at the given step: 1/sqrt(sigma^2 + 1) for Euler, 1.0 for most others.</summary>
    /// <param name="stepIndex">Index into the timestep schedule (0-based).</param>
