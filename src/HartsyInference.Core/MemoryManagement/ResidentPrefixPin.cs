...
/// <summary>Cross-generation state for a denoiser's resident block prefix: how many blocks the sizing pinned, the token load it reserved headroom for, and whether those weights are still on the device.</summary>
...
public sealed class ResidentPrefixPin
{
    ...
    /// <summary>Set when a later phase freed only the prefix's tail: the pin stands, but the pool must be trimmed before the next generation tops it back up.</summary>
