...
#
#   tests/blackwell-run.sh --rehearsal                        # on the local card: everything except the Blackwell-only rows
...
if stage_wanted probe; then
    # BackendFactory.ProbeCuda builds the backend and runs a matmul: this is where the 27 hand-written
    # `.version 7.0 / .target sm_70` PTX either JIT onto this card or do not. Answered in seconds.
    if env ${GPU_UUID:+CUDA_VISIBLE_DEVICES=$GPU_UUID} dotnet test "$REPO/tests/HartsyInference.API.Tests" -c Release --no-build --nologo -v q \
        --filter "FullyQualifiedName~GpuProbeTests" --logger "console;verbosity=normal" >"$OUT/logs/probe.log" 2>&1; then
        log "probe: CUDA backend constructs and computes on this card"
    else
...
run_tests() {   # run_tests <project> <filter> <logname>
    ...
    if env ${GPU_UUID:+CUDA_VISIBLE_DEVICES=$GPU_UUID} dotnet test "$REPO/$proj" -c Release --no-build --nologo -v q --filter "$filter" \
...
if stage_wanted models; then
    ...
    if [ -n "$BASE_RUNS" ] && [ -n "$BASE_SHA" ]; then
        ...
        "$AB" --base "$BASE_SHA" --head "$REPO" --out "$ABOUT" --gpu "$GPU_INDEX" --gpu-name "" --expect bounded:0.50 --reps 1 \
    ...
    "$AB" --no-base --head "$REPO" --out "$ABOUT" --gpu "$GPU_INDEX" --gpu-name "" --reps 2 >"$OUT/logs/ab-head-only.log" 2>&1 \
        || { FAILURES=$((FAILURES + 1)); log "head-only core run reported failures — see $OUT/logs/ab-head-only.log"; }
    ...
    "$AB" --base "$REPO" --head "$REPO" --base-set numerics.fp4Native=false --head-set numerics.fp4Native=true \
        --out "$ABOUT" --gpu "$GPU_INDEX" --gpu-name "" --filter zimage,lens-mxfp8 --tag "" --expect bounded:0.90 --reps 2 \
        >"$OUT/logs/ab-fp4native.log" 2>&1 || { FAILURES=$((FAILURES + 1)); log "fp4Native off/on run reported failures — see $OUT/logs/ab-fp4native.log"; }
    grep -E "^\| |knobs" "$OUT/logs/ab-fp4native.log" | tee -a "$LOG" >&2
    # Within-session determinism: the same build twice must reproduce its own bytes on this card.
    HARTSY_MIGRATION_BASELINE="$OUT/migration" "$REPO/tests/migration-baseline.sh" --backend cuda --record --no-build >"$OUT/logs/baseline-record.log" 2>&1
    HARTSY_MIGRATION_BASELINE="$OUT/migration" "$REPO/tests/migration-baseline.sh" --backend cuda --no-build >"$OUT/logs/baseline-compare.log" 2>&1 \
        && log "determinism: this card reproduces its own generations" || { FAILURES=$((FAILURES + 1)); log "determinism: CHANGED between two identical runs — see $OUT/logs/baseline-compare.log"; grep -E "CHANGED|CRASH" "$OUT/logs/baseline-compare.log" | tee -a "$LOG" >&2; }
...
if stage_wanted collect; then
    tar -czf "$OUT.tar.gz" -C "$(dirname "$OUT")" "$(basename "$OUT")" && log "bundle: $OUT.tar.gz ($(du -h "$OUT.tar.gz" | cut -f1))"
    {
