...
public sealed class TtsToSttLoopTests
{
    ...
    /// when the user explicitly opts in via <c>SHARPINFERENCE_RUN_HEAVY=1</c>.</para></summary>
    [Fact]
    public async Task F5_To_Whisper_PlumbingSmoke_GeneratesAndTranscribes()
    {
        ...
        string tinyRefWav = Path.Combine(Path.GetTempPath(), $"sharpinference_tts_ref_{Guid.NewGuid():N}.wav");
        ...
        try
        {
            ...
            string outDir = Path.Combine(Path.GetTempPath(), "sharpinference_tts_to_stt");
    ...
    ///
    /// <para>The test is gated by environment variable <c>SHARPINFERENCE_RUN_HEAVY=1</c>
    /// because of its runtime — regular CI skips it; manual runs (or nightly) opt in.</para>
    ///
    /// <para>The WAVs are written to <c>{TempPath}/sharpinference_tts_to_stt/</c>:
    ...
    {
        if (Environment.GetEnvironmentVariable("SHARPINFERENCE_RUN_HEAVY") != "1")
        {
            _out.WriteLine("Skipped: set SHARPINFERENCE_RUN_HEAVY=1 to run the ~5-8 min full TTS→STT round-trip.");
            return;
        }
