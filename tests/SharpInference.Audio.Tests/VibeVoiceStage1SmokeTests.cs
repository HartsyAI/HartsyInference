...
/// are not in the SharpInference cache. With the cache present they:
/// <list type="bullet">
...
public sealed class VibeVoiceStage1SmokeTests : IDisposable
{
    ...
    private bool Skip()
    {
        if (!_cacheReady) _out.WriteLine("VibeVoice-1.5B cache missing (expected at ~/.cache/sharpinference/models/vibevoice--VibeVoice-1.5B/) — skipping.");
        return !_cacheReady;
    }
