...
using SharpInference.Audio.Models.VibeVoice;
...
/// (<c>SHARPINFERENCE_RUN_VIBEVOICE_E2E=1</c>) so the default regression suite stays
...
public sealed class VibeVoiceEndToEndTests
{
    ...
    private bool Skip()
    {
        ...
        if (Environment.GetEnvironmentVariable("SHARPINFERENCE_RUN_VIBEVOICE_E2E") != "1")
        {
            _out.WriteLine("Set SHARPINFERENCE_RUN_VIBEVOICE_E2E=1 to enable. Skipping.");
            return true;
        }
        return false;
    }
