...
public sealed class WhisperEndToEndTests
{
    ...
    /// <summary>Set <c>SHARPINFERENCE_NETWORK_TESTS=1</c> to opt into download-driven
    ...
    private static bool AllowNetwork =>
        Environment.GetEnvironmentVariable("SHARPINFERENCE_NETWORK_TESTS") == "1";
    ...
    public async Task Transcribe_Jfk_Clip_OnWhisperTiny_MatchesCanonicalText()
    {
        ...
        if (!cached && !AllowNetwork)
        {
            ...
            // Set SHARPINFERENCE_NETWORK_TESTS=1 to actually download and run.
            return;
        }
