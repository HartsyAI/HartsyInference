...
image	krea2	Stable-Diffusion/Krea2/Turbo/krea2_turbo_fp8_scaled.safetensors	image|a red apple on a wooden table|-m krea2 --steps 4 --width 1024 --height 1024 --seed 42
image	flux	Stable-Diffusion/Flux	image|a red apple on a wooden table|-m flux1 --steps 4 --width 1024 --height 1024 --seed 42
image	qwenimage	Stable-Diffusion/QwenImage	image|a red apple on a wooden table|-m qwen-image --steps 4 --width 1024 --height 1024 --seed 42
image	zimage	Stable-Diffusion/ZImage	image|a red apple on a wooden table|-m zimage --steps 4 --width 1024 --height 1024 --seed 42
...
while IFS=$'\t' read -r modality id ckpt spec; do
    ...
    if [ "$ckpt" != "-" ]; then
        path="$MODELS/$ckpt"
        # A DIRECTORY that exists but holds no weights is the common case here — the model tree is laid out
        # ahead of the download. Testing the path alone reports that as a crash, which is worse than useless: it
        # invents a Vulkan failure for a model nothing ever tried to run.
        missing=""
        if [ ! -e "$path" ]; then
            missing="no local checkpoint at $ckpt"
        elif [ -d "$path" ] && [ -z "$(find "$path" -name '*.safetensors' -o -name '*.gguf' 2>/dev/null | head -1)" ]; then
            missing="no weights under $ckpt"
        fi
        if [ -n "$missing" ]; then
            printf '%s\t%s\t%s\t%s\t%s\t%s\n' "$modality" "$id" "$BACKEND" "untested" "0" "$missing"
            printf '%-9s %-14s %-10s %9s  %s\n' "$modality" "$id" "untested" "-" "$missing" >&2
            continue
        fi
    fi
    ...
    start=$(date +%s.%N)
    # stdin from /dev/null: a model whose companion weights are absent asks "Download these now? [y/N]" and waits.
    # With a terminal attached that is a hang until the per-case timeout; the answer here has to be "no, and say so",
    # since a smoke matrix that silently pulls tens of gigabytes is not a smoke matrix.
    # shellcheck disable=SC2086
    if [ "$prompt" = "-" ]; then
        timeout "$TIMEOUT" dotnet "$CLI" "$cmd" ${path:+--model-path "$path"} $args -b "$BACKEND" -o "$OUT/$id.$BACKEND" >"$log" 2>&1 </dev/null
    else
        timeout "$TIMEOUT" dotnet "$CLI" "$cmd" "$prompt" ${path:+--model-path "$path"} $args -b "$BACKEND" -o "$OUT/$id.$BACKEND" >"$log" 2>&1 </dev/null
    fi
    rc=$?
    ...
    if [ $rc -eq 124 ]; then
        ...
        if grep -q "Download these now?" "$log" 2>/dev/null; then
            # The model itself was never exercised, so this says nothing about the backend.
            status="incomplete"; detail="companion weights absent (declined the download prompt)"
        else
            status="crash"
