...
/// (~13 GB RSS peak) and takes minutes, so it additionally requires <c>SHARPINFERENCE_RUN_ACE_E2E=1</c>; run it from
/// a bare terminal, not inside an IDE test runner:
/// <code>SHARPINFERENCE_RUN_ACE_E2E=1 dotnet test tests/SharpInference.Diffusion.Tests -f net10.0 --filter AceStepGenerationTests.Generate</code>
...
public unsafe class AceStepGenerationTests
{
    ...
    public void Generate_ShortInstrumental_WritesWav()
    {
        ...
        if (Environment.GetEnvironmentVariable("SHARPINFERENCE_RUN_ACE_E2E") != "1")
        { _output.WriteLine("SKIPPED: set SHARPINFERENCE_RUN_ACE_E2E=1 (casts the 3.5B DiT to F32, ~13 GB RSS)."); return; }
