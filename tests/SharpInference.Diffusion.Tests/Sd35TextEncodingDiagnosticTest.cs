...
/// <summary>Diagnostic: encodes the same prompt through the C# CLIP-L + CLIP-G + zero-T5 path that <see cref="SharpInference.Diffusion.Pipelines.Sd3Pipeline.EncodePrompt"/> uses, then dumps the intermediate tensors so they can be diffed against a Python reference (dump_sd35_pipeline_inputs.py).</summary>
