...
public sealed class YoloCheckpointTests
{
    ...
    public void YoloV8n_ForwardPass_OutputsExpectedAnchorCount()
    {
        ...
        (SharpInference.Core.Tensors.Tensor input, _) = pre.Preprocess(rgb, 640, 640);
        try
        {
            using SharpInference.Core.Tensors.Tensor output = pipeline.Model.Forward(backend, input);
