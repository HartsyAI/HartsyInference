...
public sealed class Qwen2LanguageModelTests : IDisposable
{
    ...
    /// each — gated behind <c>HARTSYINFERENCE_RUN_QWEN2_HEAVY=1</c> so the default
    ...
    {
        if (Skip()) return true;
        if (Environment.GetEnvironmentVariable("HARTSYINFERENCE_RUN_QWEN2_HEAVY") != "1")
        {
            _out.WriteLine("Set HARTSYINFERENCE_RUN_QWEN2_HEAVY=1 to enable. Skipping.");
            return true;
        }
        return false;
    }
