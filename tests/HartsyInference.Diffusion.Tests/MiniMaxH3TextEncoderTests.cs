...
public sealed unsafe class MiniMaxH3TextEncoderTests
{
    ...
    [Fact]
    public void ContainerKeepsTheBankPackedForNvfp4Linear_WhereTheDefaultFoldWidensIt()
    {
        // The contract the recipe depends on, end to end over the two types that have to agree: what
        // CheckpointSource's normalization emits must still be what Nvfp4Linear recognizes as packed. It stopped
        ...
        try
        {
            ...
            // The control: the same fixture through the default path loads without error and computes correctly —
            // it is only the footprint that differs, which is why this needs an explicit assertion rather than a
            // failure to load.
            Assert.False(widened.IsPacked);
            Assert.Equal(packed.OutFeatures, widened.OutFeatures);
            Assert.Equal(packed.InFeatures, widened.InFeatures);
        }
        finally
        {
            packed.Dispose();
