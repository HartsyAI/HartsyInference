...
/// <summary>Correctness tests for the reusable <see cref="CausalConv3d"/> video-VAE op (no GPU/checkpoint). Verifies the Conv2D-decomposition against the existing <see cref="HartsyInference.Core.Backends.IBackend.Conv2D"/> and the causal temporal property that any streaming video decode relies on.</summary>
