...
/// <summary>Qwen-Image's two GATED sharding-regression tiers, on top of the shared cross-device case in
/// <see cref="DitShardingEngineTests"/>. Split out because they exist for one documented reason:
/// <see cref="DitShardingEngineTests.CoherentCases"/>'s default-regime SSIM for this model is INFORMATIONAL, not a
/// correctness bar — investigated 2026-08-06 (see <c>QwenImageFp8PrecisionDiagnosticTests</c> and the footnote in
/// <c>benchmarks/results/2026-08-05_multigpu_speeds.md</c>). Two independent controls proved the N-way sharding
/// mechanism itself is correct: (1) two backend instances on the SAME ordinal gave bit-exact output; (2) forcing
/// BOTH the unsharded baseline and the sharded run onto the same fp8 GEMM regime recovers SSIM 0.9922 — better
/// than every other sharded model measured. The real source of the low SSIM at default settings is that this
/// checkpoint's residual reaches ±10M by mid-depth (documented in <c>QwenImageTransformer.cs</c>), and the strong
/// card's native fp8 GEMM path has real per-tensor e4m3 quantization error on those outlier channels — a property
/// of this checkpoint's precision regime on Ada+ hardware, present with or without sharding. Disabling native fp8
/// to "fix" the gate would cost real slowdown on every Qwen-Image generation, single-GPU included, so it stays
/// opt-in, and these two facts carry the regression coverage a regime gap can't provide instead: a same-device
/// split (one GEMM regime, SSIM &gt; 0.99) and a regime-matched cross-device run (<c>HARTSY_FP8_NATIVE=0</c> on
/// both sides, measured 0.9922, gated &gt; 0.95). Same mechanism class as
/// <see cref="MiniMaxH3DitShardingEngineTests"/>'s already-accepted informational gate.</summary>
...
public sealed class QwenImageDitShardingEngineTests
{
    ...
    /// <see cref="DitShardingEngineTests.DitSharding_RealEngine_ProducesCoherentImage_WithinToleranceOfUnsharded"/>
    /// can't provide: identical run, but <c>HARTSY_FP8_NATIVE=0</c> forces BOTH the unsharded baseline and the
    /// sharded run onto the same (non-native) fp8 GEMM regime, removing the SM 8.9 native-fp8 quantization gap
    /// that made the default-regime SSIM informational. The 2026-08-06 investigation measured SSIM 0.9922 in
    /// exactly this configuration, so &gt; 0.95 is a real regression bar with margin. <c>CudaBackend</c> reads
    /// <c>HARTSY_FP8_NATIVE</c> once at construction, so it is set at the very top, before ANY backend (the VRAM
    /// probes included) is constructed — same static-init caveat as the <c>HARTSY_KEEP_MODELS</c> facts: run this
    /// fact filter-isolated in its own process.</summary>
    [Fact]
