...
public unsafe class LanceLatentPatchTests
{
    ...
    {
        // `using` is load-bearing beyond leak hygiene: the pending Dispose roots each Tensor for the whole
        // scope. Without it the object is unreachable once its raw DataPointer has been read, and a GC
        // triggered by a concurrently running test class finalizes it mid-assert — glibc free() then writes
        // tcache metadata into the still-in-use buffer (the 07-17 parallel-suite stomp).
        int t = 1, h = 3, w = 4, c = 48, pt = 1, ph = 2, pw = 2;
        using Tensor latent = Random([t * pt, h * ph, w * pw, c], seed: 7);
        using Tensor tokens = LanceLatentPatch.Patchify(latent, pt, ph, pw);

        Assert.Equal(t * h * w, (int)tokens.Shape[0]);
        Assert.Equal(pt * ph * pw * c, (int)tokens.Shape[1]); // 192

        using Tensor back = LanceLatentPatch.Unpatchify(tokens, t, h, w, pt, ph, pw, c);
    ...
    public void Patchify_FeatureOrderMatchesUpstreamEinops()
    {
        ...
        int c = 2, ph = 2, pw = 2;
        using Tensor latent = new Tensor(new TensorShape([1L, 2, 2, c]), DType.F32); // [Tf=1,Hf=2,Wf=2,C=2]
        ...
        // Rooted via `using` — see PatchifyUnpatchify_RoundTrips for why this is required, not style.
        using Tensor tokens = LanceLatentPatch.Patchify(latent, 1, ph, pw); // [1, 8]
