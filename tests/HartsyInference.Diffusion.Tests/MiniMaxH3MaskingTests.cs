...
public sealed class MiniMaxH3MaskingTests
{
    ...
    [Fact]
    public void VideoMaskUsesContinuousTwoByTwoAmaxInPackedRowOrder()
    {
        float[] mask =
        [
            0f, 0.1f, 0.2f, 0.3f,
        ...
        float[] rows = Assert.IsType<float[]>(
            MiniMaxH3Masking.PackVideoMaskRows(mask, latentFrames: 1, latentHeight: 4, latentWidth: 4));

        Assert.Equal([0.5f, 0.7f, 0.9f, 0.75f], rows);
    }
    ...
    {
        float[] rows = Assert.IsType<float[]>(
            MiniMaxH3Masking.ResampleAudioMask([0f, 1f, 0f], sourceRate: 20f,
                targetAudioLatentFrames: 5));
    ...
    public void AllWhiteMasksAreExactNoOps()
    {
        ...
        Assert.Null(MiniMaxH3Masking.ResampleAudioMask([1f, 1f], 40f, 8));
    }
