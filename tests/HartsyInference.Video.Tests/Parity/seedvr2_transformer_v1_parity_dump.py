"""SeedVR2 v1 NaDiT (7B architecture, models/dit) tiny-config parity dump.

Same harness as seedvr2_transformer_parity_dump.py but against the V1 tree the 7B checkpoint uses:
plain GELU-tanh MLP with biases, no vid_out_norm/tail ada, all blocks fully split (no mm_layers), and
qk_rope=True → NaRotaryEmbedding3d (pixel freqs linspace(1,128,10)·π, 60 of 128 dims rotated, positions
linspace(−1,1) per window axis, VIDEO ONLY). flash_attn is shimmed with per-sequence SDPA before import.

Usage:  <seedvr2-venv-python> seedvr2_transformer_v1_parity_dump.py <SeedVR-checkout> <out-dir>
Then:   SEEDVR2_PARITY_V1_DIR=... dotnet test --filter SeedVr2Tests

Tiny dims must match SeedVr2Tests.TinyV1Config exactly.
"""
import os
import sys
import types
...
# ---- flash_attn shim (must precede models.dit import) ----
...
from models.dit import na  # noqa: E402
from models.dit.nadit import NaDiT  # noqa: E402

OUT = sys.argv[2]
os.makedirs(OUT, exist_ok=True)

# Must match SeedVr2Tests.TinyV1Config exactly.
...
model = NaDiT(
    ...
    norm="rms", norm_eps=1e-5, ada="single", qk_bias=False, qk_rope=True, qk_norm="rms",
    patch_size=(1, 2, 2), num_layers=LAYERS,
    block_type="mmdit_sr", shared_qkv=False, shared_mlp=False, mlp_type="normal",
    window=[(4, 3, 3)] * LAYERS,
