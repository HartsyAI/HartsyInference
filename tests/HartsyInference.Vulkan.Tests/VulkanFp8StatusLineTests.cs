...
/// <summary>The once-per-device fp8 line a Vulkan backend logs at construction: the reason when off, a warning only when the
/// user forced the path on and the device cannot run it.</summary>
public sealed class VulkanFp8StatusLineTests
{
    ...
    [Fact]
    public void Available_NamesTheShape()
    {
